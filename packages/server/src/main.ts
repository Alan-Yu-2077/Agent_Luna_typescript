...
import { builtinRegistry, codeWriteEnabled, messageRegistry, withCodeWrite } from './tools/registry';
...
if (Bun.env['ANTHROPIC_API_KEY']) {
  ...
  const registry = withCodeWrite(messageMode ? messageRegistry : builtinRegistry);
  setRuntime({ provider, registry, dreamLlm });
  console.log(
    `[luna-server] provider: ${Bun.env['LUNA_MODEL'] ?? 'claude-opus-4-8'} via ${Bun.env['ANTHROPIC_BASE_URL'] ?? 'https://api.anthropic.com'}${summarizerKey ? ' (+summarizer key)' : ''}${messageMode ? ' [message-tool mode]' : ''}${writeMode ? ' [code-write]' : ''}`,
  );
