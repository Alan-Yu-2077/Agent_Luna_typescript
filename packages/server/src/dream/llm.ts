...
export type DreamCallResult =
  | { ok: true; text: string }
  | { ok: false; failure: 'rate_limited' | 'content_filter' | 'auth' | 'empty_text' | 'exception'; detail: string };

function classify(e: unknown): 'rate_limited' | 'content_filter' | 'auth' | 'exception' {
  const msg = (e instanceof Error ? e.message : String(e)).toLowerCase();
  if (msg.includes('rate') || msg.includes('429') || msg.includes('overload') || msg.includes('饱和')) {
    return 'rate_limited';
  }
...
export async function dreamCall(
  ...
  for (const provider of attempts) {
    try {
      ...
      last = { ok: false, failure: classify(e), detail: e instanceof Error ? e.message : String(e) };
    }
  }
  return last;
}
...
export const MemoryPatch = z.object({
  ...
  add: z.array(
    z.object({
      category: z.enum(['core_facts', 'preferences', 'key_moments', 'active_threads', 'project_context']),
      text: z.string().min(1),
    }),
  ),
});
