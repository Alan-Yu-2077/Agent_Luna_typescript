...
## ⛔ WHERE THE CODE IS — read before anything else (consolidated 2026-08-01)

**One tree, one remote, one branch: `~/Desktop/Luna-ts`, branch `main`.** It holds the live
`luna.sqlite`, the real `luna.env`, and the `Luna.app` the owner runs. The second working tree
(`Agent_Luna_typescript`), the second remote, and the `oss-prep` branch that older notes mention were
all archived on 2026-08-01 — if you remember them, that memory is stale. Same for the Python original
and its TTS tree: retired the same day, voice runtime now at
`~/Library/Application Support/@luna/desktop/tts/runtime`.

**Always read the live log**: `~/Desktop/Luna-ts/docs/history/DEVELOPMENT.md` (top line =
shipped head). **Always get the test count by running** `bun test` in `~/Desktop/Luna-ts` —
never quote a number from a skill file.

**Posture (v0.41.0, 2026-07-30)**: Luna is a private, single-machine companion. `origin` is the only
remote (`Agent_Luna_typescript.git`, public, branch `main`); `github.com/Alan-Yu-2077/Luna-ts` is an
...
> "cut" claim is presumed stale until you check the live source.** Two known-wrong examples: the
> "Anthropic-SDK-only, `openai_compat` dropped" locked decision (an OpenAI provider shipped in
> Initiative 16 — see `packages/server/src/provider/openai/`), and the "Skills subsystem — cut" entry
> (a skills library shipped in Initiative 23 — see `packages/server/src/skills/`).

## Historical state (as of ~v0.15.4 — see the warning above)
...
- `bun test` (from `~/Desktop/Luna-ts`) — run it to get the real count; do NOT quote a number from this file.
