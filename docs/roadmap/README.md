...
| v0.32.2 | [Dream distillation (dark)](skills-procedural-memory-2026-07/v0.32.2-dream-distillation.md) | **High** | `distill_skills` dream step between `run_diaries` and `rag_refresh` (zero protocol change); SkillPatch Zod + whole-patch rejection + JSON-literal-null default + per-cycle cap (2) + merge-over-duplicate + stale deprecation; `source='dream'` provenance; **never runs bun test**; every write audited + one-call `restoreSkill`; behind `LUNA_DREAM_SKILLS=0`; **live dream A/B gates the flip** | 📋 |
