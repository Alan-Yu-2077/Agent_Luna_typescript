...
| v0.32.1 | [Skills into recall](skills-procedural-memory-2026-07/v0.32.1-recall-source.md) | Low | `'skills'` as a `retrieve()` source (3 unions + candidate loop, importance 0.75, key = name+description — the Voyager pattern); recall-tool scope + output-enum extension; `recall_skill` → `markUsed` (epoch-silent); **fix the rag_refresh `contentHash`→`embedCacheKey` mismatch** (pre-existing dead pre-warm) + pre-warm skill texts | 📋 |
