...
> **Main head: v0.33.2** (branch `mainline`). **Initiative 24 — OSS Readiness (v0.34.0–v0.34.8) 📋 PLANNED** — the public-release gate (LICENSE + PII scrub + de-yunwu + portability + remove yumi/TTS → bring-your-own onboarding + docs; irreversible history rewrite owner-gated last), see [`oss-readiness-2026-07/`](oss-readiness-2026-07/). Since v0.32.3, shipped ad hoc: v0.33.0 desktop native location, v0.33.1 continuation glyph, v0.33.2 continuation-TTS-barge-in fix. **Initiative 22 (the Soul File, v0.30.0–v0.30.3) ✅ SHIPPED 2026-07-04** — the persona is one DB-stored soul file (git-seeded fixed core + a Luna-authored evolving section); `core_memory` retired; LD #12 amended. **v0.31.0 shipped ad hoc 2026-07-04** (owner-maintainable soul: `seedFixedCore` → seed-if-empty, `updateFixedCore` owner-authoritative, the `/_workspace` soul editor). **Initiative 23 — Skills as Procedural Memory (v0.32.0–v0.32.3) ✅ SHIPPED 2026-07-05** — the half-built v0.15.4 skill library became the **fourth memory pillar**: shelf + L1 trigger + lifecycle (v0.32.0), semantic recall + the rag_refresh embed-key fix (v0.32.1), dream distillation dark-launched (v0.32.2), flip + `/_workspace` Skills panel + **LD #12 amendment** after a PASSED 4-run live dream A/B (v0.32.3); 3 adversarial reviews, 25 confirmed findings all fixed pre-commit — see [`skills-procedural-memory-2026-07/`](skills-procedural-memory-2026-07/). **Initiatives 17 (silence ladder) + 18 (companion UI) + 19 (desktop app) ✅ SHIPPED 2026-07-02.** Earlier, shipped ad hoc via `luna-ts-dev` (not roadmapped): **v0.27.0–v0.27.1 settings surface** (pet toggle + a server-driven, auto-rendering settings panel over 16 whitelisted env flags), **v0.27.2** (desktop preload `__dirname` fix — the pet bridge silently never loaded), **v0.27.3** (proactive priming directive no longer leaks into a phantom user bubble). **Initiative 20 — desktop first-run & pet ergonomics (v0.28.0–v0.28.3) ✅ SHIPPED** — a guided setup screen (writes `luna.env`, restarts the sidecar) + pet mode reworked so the *window* moves/resizes while the model is a fixed half-body. See [`desktop-onboarding-pet-2026-07/`](desktop-onboarding-pet-2026-07/). **Initiatives 8–12 all ✅ shipped + merged**: code-agent capability
...
> **Initiative 16 — OpenAI-protocol adapter ✅ SHIPPED** (v0.23.0–v0.23.3, branch `feat/weather-perception`): the chat path was hardwired to
> the **Anthropic protocol** (the canonical history format is `@anthropic-ai/sdk`-typed). Add an **OpenAI
> Chat-Completions provider** behind the `Provider` seam (translate at the boundary; Anthropic stays the
> internal IR) so Luna runs on **any model over the OpenAI protocol** — staged seam→translation→streaming→
> registry. Amends the "Anthropic-SDK-only chat" LD; default path unchanged.
...
| 16 | v0.23.0 – v0.23.3 | **OpenAI-protocol adapter (model compatibility)** — Luna's chat path is hardwired to the Anthropic protocol (the request, the streaming events, and the **canonical history format** are all `@anthropic-ai/sdk`-typed; ~30 files import it). Add an **OpenAI Chat-Completions provider** behind the existing `Provider` seam so Luna runs on **any model over the OpenAI protocol** (OpenAI, OSS via a compatible gateway, the yunwu OpenAI route, vLLM/Ollama). Shape: **translate at the boundary** (Anthropic-shaped types stay the IR; `OpenAIProvider` is a translating adapter), not a neutral-IR rewrite. Staged: provider **seam + capabilities + factory** (v0.23.0), the **translation core + `complete()` + correctness-first chat** (v0.23.1), **real SSE streaming + interleaved tool-use + reasoning→thinking** (v0.23.2), **per-model registry + multi-model E2E + close** (v0.23.3). **Amends the provider LD** ("Anthropic-SDK-only chat") — a fresh TS provider honors per-instance keys, so the cut `openai_compat`'s bug doesn't recur. Default path (`LUNA_PROVIDER=anthropic`) byte-unchanged. | [`openai-protocol-adapter-2026-06/`](openai-protocol-adapter-2026-06/) | ✅ SHIPPED (code; live multi-model E2E pending a restart) |
| 17 | v0.24.0 – v0.24.2 | **Proactive parity restore (the silence ladder)** — the proactive *wake decision* is wrong for this product: Initiative 15's **detector registry** made Luna reach out on a **calendar** (`LUNA_PROACTIVE_SLOTS`) with **no graded restraint** (every fire is one-shot). Restore Alan's original **Python** design — a single-signal **escalation ladder driven by silence**: a phase machine (`ENGAGED→IDLE_WATCH→NUDGED→DORMANT`) through four restraint levels (**ambient** → **idle_nudge** → backoff **renudge**s → **leave_message** → DORMANT + auto-recovery), driven by `effective_gap`, with the model's silent-sentinel veto + `<proactive_meta>{follow_up}` + the full **companion-opener constraint** (陪他不查岗). Staged: **ladder core** behind `LUNA_PROACTIVE_LADDER` (coexists — v0.24.0), **flip default + delete the 5 detectors + slot machinery + migration** (v0.24.1), **`set_proactive_style` activeness/voice self-tuning** clamped inside operator rails (v0.24.2). **Amends LD #15** — reverses the 2026-06-13 Hermes-paradigm direction for the wake decision at Alan's direction (2026-07-01); the agency turn + safety gate + dropped delivery layer stand. Pure-Python replica: the net-new weather/promise/thread/after-night detectors are **deleted**. | [`proactive-parity-restore-2026-07/`](proactive-parity-restore-2026-07/) | ✅ SHIPPED 2026-07-02 |
| 18 | v0.25.0 – v0.25.2 | **Collapsible companion UI (bubble stack + model glide)** — a second way to be with Luna: a control morphs the chat window into a single **bottom input bar**; her replies stop rendering in the (hidden) window and **pop as speech bubbles beside the Live2D model** (a stack — newest at the bottom, previous pushed up, each ~10s then a gradual fade); Luna **glides to screen center** on collapse and back **right** on expand. **Adversarially verified `packages/web`-ONLY** — zero server, zero wire change (`backendTouchpoints: []`): reply text already arrives on `tool.progress{message}`/`tool.finished`→`MessageDelivery.text`, expression rides the same envelope (LD #9), `chat.send` is unchanged, proactive replies ride the same message-tool path (bubbles for free), and the server holds no per-connection UI state. Staged: **speech-bubble stack** (`SpeechStackView`+router over the `BubbleView` seam, speech-gated TTL, barge-in fade — v0.25.0), **collapse/expand morph** (window↔bottom-bar, echo + mobile + reduce-motion — v0.25.1), **model glide** (pixi-ticker rAF ease on a dedicated mode-offset — v0.25.2). Realizes `bubbles.ts:62`'s "real Live2D-framed UI is a later pass." | [`collapsible-companion-ui-2026-07/`](collapsible-companion-ui-2026-07/) | ✅ SHIPPED 2026-07-02 |
| 19 | v0.26.0 – v0.26.2 | **Desktop app (Luna lives on your screen)** — take the frontend out of the browser and make Luna a **desktop application**, ultimately a transparent, always-on-top floating **companion**. Verified to be **mostly packaging, not a rewrite**: Live2D already runs on pixi v7 WebGL + a **local inlined-WASM** Cubism Core over a **transparent** canvas, the frontend couples via **one WS URL**, and the only browser API that breaks is geolocation. **Shell = Electron for v1** (engine certainty for THIS pixi-live2d-display + WASM stack — the adversarial verdict rules Tauri's WKWebView an un-derisked engine surface; Tauri deferred behind a packaged go/no-go). **Single-machine** (Alan's choice): the Bun server is `bun build --compile`'d + spawned as a supervised sidecar. Staged: **port foundation + rendering smoke test** (static build, loopback origin, WS-URL fix — v0.26.0), **single-machine app** (server sidecar + supervisor + app-data SQLite/secrets + packaging — v0.26.1), **desktop-pet window** (transparent/always-on-top/click-through + background-throttling-off + geo shim + premultiplied-alpha — v0.26.2). Home for Init 18's collapsed companion mode. Extends the Frontend LD. Licensing: personal use free; the yumi model art's own license is the only distribution gate. | [`desktop-app-2026-07/`](desktop-app-2026-07/) | ✅ SHIPPED 2026-07-02 |
| 20 | v0.28.0 – v0.28.3 | **Desktop first-run & pet ergonomics** — a guided setup screen (writes `luna.env`, restarts the sidecar) + pet mode reworked so the *window* moves/resizes while the model is a fixed half-body + a 24-agent review remediation. | [`desktop-onboarding-pet-2026-07/`](desktop-onboarding-pet-2026-07/) | ✅ SHIPPED 2026-07-03 |
| 21 | v0.29.0 – v0.29.1 | **Proactive silence-trigger redesign** — the silence ladder measures `effective_gap` from the user's last message + her *proactive* outreach, **ignoring Luna's reactive replies**, so she interrupts active conversations ("系统对'静默'的概念存在误解"). Redesign silence as a single **idle-timer** (`lastActivityMs`, bumped by all conversation activity); keep `lastUserMs` (escalation reset) + `lastProactiveMs` (outreach spacing) as orthogonal clocks. **Core** behind `LUNA_PROACTIVE_SILENCE_TIMER` (v0.29.0), then **tune the ambient amplifiers + retire the old anchor** (v0.29.1). Follows Init 17; stays inside LD #15 (no amendment). | [`proactive-silence-timer-2026-07/`](proactive-silence-timer-2026-07/) | ✅ SHIPPED 2026-07-03 |
| 22 | v0.30.0 – v0.30.3 | **The Soul File** — collapse the persona (hardcoded `default.md` + the degraded `core_memory` table) into ONE **DB-stored soul file** split by authorship: a **fixed core** (dev-authored, git-seeded, Luna-immutable) + a small **evolving section** (self + bond, Luna self-authors via the dream). Retire `core_memory`; fix the `persona_update` freeze (frozen since 2026-06-24) + the no-cleanup gap that grandfathered a fact-ledger into `relationship_status`; `buildSystemPrompt` block 04 renders the whole soul from the DB, block 07's self/relationship folds in (L3 stays). Staged: soul store + migration + seed **dark-launch** (v0.30.0) → render behind `LUNA_SOUL_DB` (v0.30.1) → **dream authors** the evolving section + relationship purge (v0.30.2) → flip default + retire `core_memory` + **LD #12 amendment** (v0.30.3). Owner-driven; disruptive. | [`soul-file-2026-07/`](soul-file-2026-07/) | ✅ SHIPPED 2026-07-04 |
| 23 | v0.32.0 – v0.32.3 | **Skills as Procedural Memory** — finish the half-built v0.15.4 skill library into Luna's **fourth memory pillar** (procedural: "how I do things"). The 2026-07-04 audit: the substrate works but the loop is open — **1 skill saved + 4 recalls in 19 days** (~0.6% of tool traffic) because nothing triggers a save (zero L1/persona mention), nothing surfaces the library (no shelf in the prompt), and recall is lexical-only. Close it: a **skill shelf** (progressive disclosure — name+description in the cached block, body via `recall_skill`; the Anthropic Agent Skills pattern) + an **L1 skills clause** (use-before-redo + when-to-save) + **lifecycle substrate** (audit / usage counters / provenance / deprecation / epoch, migration `0018`) — v0.32.0; skills as a **`retrieve()` source** (semantic recall; + the rag_refresh `contentHash`→`embedCacheKey` fix) — v0.32.1; a **`distill_skills` dream step** (Letta sleep-time-compute on the existing dream engine: AWM variable abstraction, CLIN cause-not-transcript, ACE merge-over-duplicate, capped + audited + restorable, `source='dream'`) dark-launched behind `LUNA_DREAM_SKILLS=0` — v0.32.2; **flip + `/_workspace` Skills panel + LD #12 procedural-memory amendment** — v0.32.3. The write-target substrate for the upcoming code-capability/self-evolution initiative. | [`skills-procedural-memory-2026-07/`](skills-procedural-memory-2026-07/) | ✅ SHIPPED 2026-07-05 |
| 24 | v0.34.0 – v0.34.8 | **OSS Readiness** — the public-release gate. A 21-agent audit confirmed **secrets-in-source = 0**; the blockers are legal + PII + portability, not leaked keys. Scrub the owner's name/handle/hobbies out of runtime prompts, fixtures, comments, the desktop appId + outbound UA; kill the first-run box that funnels new users' API keys to the owner's private `yunwu.ai` proxy; fix the homebrew-only sqlite path that silently kills semantic recall off the owner's laptop; neutralize Shanghai locale defaults; **remove the un-relicensable `yumi` Live2D model** (and de-personalize TTS — the `neoru` voice weights stay private, the integration is kept + made config-driven BYO) and replace them with a **bring-your-own onboarding** (empty-state → drop/pick a model + a zero-setup browser voice + a `services/tts/` GPT-SoVITS config + `docs/SETUP.md`); add **MIT LICENSE + `THIRD_PARTY_LICENSES`**; **remove `docs/` internal diaries + `.claude/`** (extract clean `ARCHITECTURE.md`/`ROADMAP.md`). Staged legal→PII→de-yunwu→paths→locale→docs→asset-removal→onboarding; the **irreversible git-history rewrite is quarantined as a final owner-gated step** (v0.34.8). Verified in a clean clone at v0.33.2; the Phase-A workflow caught the empty-`models/` build trap, the smoke-verdict break, and the net-new BYO desktop plumbing. **Supersedes cancelled Initiative 7.** | [`oss-readiness-2026-07/`](oss-readiness-2026-07/) | 📋 PLANNED |
...
## Initiative 16 — OpenAI-protocol adapter (model compatibility) (v0.23.0 – v0.23.3) — ✅ SHIPPED

Luna's chat path is hardwired to the **Anthropic protocol** — the request, the streaming events,
and the *canonical history format* are all `@anthropic-ai/sdk`-typed (`session.history:
Anthropic.MessageParam[]`; ~30 files import the SDK). Add an **OpenAI Chat-Completions provider**
behind the existing `Provider` seam (`chatStream`/`complete`) so Luna runs on **any model over the
OpenAI protocol**. Shape: **translate at the boundary** — the Anthropic-shaped types stay the
internal IR and `OpenAIProvider` is a translating adapter (cheap blast radius), *not* a neutral-IR
rewrite (deferred). **Amends the provider Locked Decision** ("Anthropic-SDK-only chat"); a fresh TS
provider honors per-instance keys, so the cut `openai_compat`'s `api_key_override` bug doesn't
recur. Default path (`LUNA_PROVIDER=anthropic`) stays byte-unchanged. Folder:
[`openai-protocol-adapter-2026-06/`](openai-protocol-adapter-2026-06/).

| Version | Plan | Risk | Theme | Status |
|---|---|---|---|---|
| v0.23.0 | [Provider seam + capabilities](openai-protocol-adapter-2026-06/v0.23.0-provider-seam-capabilities.md) | Low | `ProviderCapabilities` descriptor + `providerFor()` factory; route Anthropic through it unchanged; `cache_control` gated on `promptCache`; record the LD amendment | ✅ |
| v0.23.1 | [OpenAI translation + complete](openai-protocol-adapter-2026-06/v0.23.1-openai-translation-complete.md) | Medium | `OpenAIProvider`: the pure Anthropic⇄OpenAI translation core (tool_use/tool_result round-trip) + `complete()` + a correctness-first non-streaming `chatStream()`; minimal `fetch` client + test seam | ✅ |
| v0.23.2 | [OpenAI streaming](openai-protocol-adapter-2026-06/v0.23.2-openai-streaming.md) | Med-High | real SSE → `ProviderEvent` (interleaved tool-use, fragmented `tool_calls` reassembly) + reasoning→`thinking_delta`; behind `LUNA_OPENAI_STREAM` | ✅ |
| v0.23.3 | [Model registry + close](openai-protocol-adapter-2026-06/v0.23.3-model-registry-close.md) | Medium | per-model quirk table (token-param, system/developer role, no-tools, reasoning) driven by a model→provider **registry**; multi-model E2E; REWRITE_CONTEXT amendment; **Initiative 16 ✅** | ✅ |

## Initiative 17 — Proactive parity restore (the silence ladder) (v0.24.0 – v0.24.2) — ✅ SHIPPED

The proactive **wake decision** is wrong for this product. Initiative 15 replaced the LLM wake-gate
with a **deterministic detector registry** (after-a-night, scheduled-slot, weather-shift, open-thread,
promised-follow-through) — which made Luna reach out on a **calendar** (`LUNA_PROACTIVE_SLOTS`) with
**no graded restraint** (every fire one-shot: consider once, speak or stay silent). Alan's original
**Python** design is the opposite and the one he wants: a **single-signal escalation ladder driven by
silence** — one input (*how long since anyone spoke*) drives a phase machine
`ENGAGED→IDLE_WATCH→NUDGED→DORMANT` through four restraint levels: a light **ambient** thought → a
low-key **idle_nudge** → progressively lighter **renudge**s on exponential backoff → a
**leave_message** that releases the user and goes quiet, **auto-recovering** after a cool-down. The
model keeps the last word (silent-sentinel veto) and appends `<proactive_meta>{follow_up}>` to steer
the phase; a **companion-opener constraint** (陪他不查岗; self-disclosure / fresh topic / vary every
opener) is the persona payload. **Restore the decision layer, keep the agency machinery** — the
full-tool, message-optional, safety-gated proactive *turn* (Initiative 5) stays; only the layer that
decides *when/with-what-restraint* changes. **Amends LD #15** (reverses the 2026-06-13 Hermes
direction at Alan's 2026-07-01 direction); the dropped outbox/SSE-replay delivery layer stays dropped.
Pure-Python replica → the five detectors are **deleted**. Folder:
[`proactive-parity-restore-2026-07/`](proactive-parity-restore-2026-07/).

| Version | Plan | Risk | Theme | Status |
|---|---|---|---|---|
| v0.24.0 | [Silence-ladder core](proactive-parity-restore-2026-07/v0.24.0-silence-ladder-core.md) | High | phase machine (`effective_gap` → ambient/idle_nudge/renudge/leave_message + backoff + DORMANT-recovery) + 4 scenario framings + full companion constraint + `<proactive_meta>{follow_up}` + anti-repeat, behind `LUNA_PROACTIVE_LADDER` (default off, coexists with detectors) | ✅ |
| v0.24.1 | [Retire detectors](proactive-parity-restore-2026-07/v0.24.1-retire-detectors.md) | Medium | flip `LUNA_PROACTIVE_LADDER` on; delete `detectors.ts` (5 detectors) + slot machinery + `LUNA_PROACTIVE_SLOTS`; migration retiring the slot columns; **amend LD #15** | ✅ |
| v0.24.2 | [Style self-tuning](proactive-parity-restore-2026-07/v0.24.2-style-self-tuning.md) | Low | `set_proactive_style` tool + activeness (aloof/balanced/clingy) lever + voice-notes/exemplars, clamped inside operator floor/ceiling; **Initiative 17 close** | ✅ |

## Initiative 18 — Collapsible companion UI (bubble stack + model glide) (v0.25.0 – v0.25.2) — ✅ SHIPPED

A second way to *be with* Luna. Today the app is a fixed two-column stage (chat window left, Live2D
model right). This initiative adds a **collapsed companion mode**: a control gracefully morphs the chat
window into a single **bottom input bar**; Luna glides to **screen center**; her replies stop rendering
in the (hidden) window and **pop as speech bubbles beside her** — a stack where the newest bubble sits
at the bottom and pushes the previous one up, each living ~10s then fading. Expanding reverses it
(window unfolds, Luna glides **right**). **Hard constraint: `packages/web` ONLY — zero `packages/server`
and zero `packages/protocol` change**, adversarially verified feasible (`backendTouchpoints: []`): reply
text already arrives on `tool.progress{message}` + `tool.finished`→`MessageDelivery.text`, the model's
expression/emotion ride the same envelope (LD #9), user input reuses `chat.send` verbatim, proactive
replies ride the same message-tool path (bubbles for free), and the server holds no per-connection UI
state (`ws.ts:89-140`). Everything feeds the existing `controller → BubbleView` seam (`bubbles.ts:40`),
so no wire touch and no silent-drift. Realizes the `bubbles.ts:62` note that the minimal view is
deliberate and "the real Live2D-framed UI is a later pass." Folder:
[`collapsible-companion-ui-2026-07/`](collapsible-companion-ui-2026-07/).

| Version | Plan | Risk | Theme | Status |
|---|---|---|---|---|
| v0.25.0 | [Speech-bubble stack](collapsible-companion-ui-2026-07/v0.25.0-speech-bubble-stack.md) | High | beside-model `SpeechStackView` (`BubbleView` impl: newest-bottom, push-up, speech-gated ~10s fade, overflow cap) + `RouterBubbleView` over `deps.view` keyed on a live flag; barge-in fast-fade; testable standalone via a toggle | ✅ |
| v0.25.1 | [Collapse morph](collapsible-companion-ui-2026-07/v0.25.1-collapse-morph.md) | Medium | client `.collapsed` state + button + localStorage; window↔bottom-bar CSS morph (FLIP/cross-fade); `send()` reuse + echo suppression; flip routes replies to the stack; mobile-breakpoint + reduce-motion reconciliation | ✅ |
| v0.25.2 | [Model glide](collapsible-companion-ui-2026-07/v0.25.2-model-glide.md) | Medium | center↔right glide on collapse/expand — pixi-ticker rAF ease (exported `easeInOutSine`/`lerp`) on a dedicated mode-offset that never clobbers the persisted drag; reduce-motion snap; **Initiative 18 close** | ✅ |

## Initiative 19 — Desktop app (Luna lives on your screen) (v0.26.0 – v0.26.2) — ✅ SHIPPED

Take the web frontend out of the browser and make Luna a **desktop application** — ultimately a
floating, always-on-top, transparent-background **companion** who lives on the screen. Verified to be
**mostly packaging, not a rewrite**: Live2D already runs on `pixi.js v7` WebGL + a **local, inlined-WASM**
Cubism Core over a **transparent** canvas (`cubismRuntime.ts:9-45`), the frontend couples through **one
WS URL** (`app.ts:23-24`), models + core are local (`public/`), WebAudio is already gesture-unlocked
(`webAudioSink.ts:31-37`), and the only browser API that genuinely breaks is geolocation (`geo.ts`).
**Shell = Electron for v1** — the adversarial verifier could rule Live2D rendering fine in Chromium
(the engine the app already targets) but could **not** de-risk THIS pixi-live2d-display 0.5.0-beta +
WASM Cubism Core + transparent context under macOS **WebKit**, so Tauri v2 (~5MB vs ~150MB) is deferred
behind a packaged-build go/no-go gate. **Single-machine deployment** (Alan's choice, 2026-07-01): the
Bun server is `bun build --compile`'d and spawned as a **supervised sidecar**, so Luna is one
double-clickable app. This is the **home for Initiative 18's collapsed companion mode**. Extends the
Frontend-scope LD (web controller → packaged desktop shell; Live2D/audio internals untouched).
**Licensing:** personal use is free and clear; the only distribution gate is the **yumi model art's own
license** (unknown provenance — confirm before any distribution) and keeping Luna a single-fixed-model
app (not an "Expandable Application"). Folder: [`desktop-app-2026-07/`](desktop-app-2026-07/).

| Version | Plan | Risk | Theme | Status |
|---|---|---|---|---|
| v0.26.0 | [Port foundation + smoke](desktop-app-2026-07/v0.26.0-port-foundation-smoke.md) | High | static build (`bun build` + copy `public/`), fixed loopback-origin serve, config-driven WS URL; plain Electron window proves **yumi renders + drives + WS connects** (server hand-started) | ✅ |
...
*model* is the draggable/zoomable thing — the wrong lever). Give first-run a **guided setup screen** (writes
`luna.env`, restarts the sidecar in place — keys stay file-only, never on the settings wire) and rework pet
interaction so **the window** is what you move + resize while **the model** is a fixed half-body portrait that
fills whatever size you pick. Shell + web-frontend UX only; no turn/memory/proactive-core change. Folder:
[`desktop-onboarding-pet-2026-07/`](desktop-onboarding-pet-2026-07/).
...
## Initiative 23 — Skills as Procedural Memory (v0.32.0 – v0.32.3) — 📋 PLANNED

Finish the half-built v0.15.4 skill library into Luna's **fourth memory pillar** — procedural memory
("how I do things"), beside episodic (L2), semantic (L3), narrative (diaries) and identity (soul).
The 2026-07-04 live audit: the substrate works but the loop is open — **1 skill saved + 4 recalls in
19 days** (~0.6% of tool traffic) because nothing triggers a save (zero L1/persona mention), nothing
surfaces the library (no shelf — Luna cannot see it exists), and recall is lexical-only. Luna's own
diagnosis (L2, 2026-07-03): *"recall_skill is lexical search, not embeddings — a tool I have to
choose to call, nothing auto-injects it into context yet."* Grounded in an external SOTA review
(Anthropic Agent Skills progressive disclosure; Voyager verification-gate + description-as-key; AWM
variable abstraction; CLIN cause-distillation; ACE incremental deltas; Memp lifecycle; Letta
sleep-time compute — Luna's dream engine is exactly that organ). Also the write-target substrate for
the upcoming code-capability/self-evolution initiative. Folder:
[`skills-procedural-memory-2026-07/`](skills-procedural-memory-2026-07/).

| Version | Plan | Risk | Theme | Status |
|---|---|---|---|---|
| v0.32.0 | [Shelf + trigger + lifecycle](skills-procedural-memory-2026-07/v0.32.0-shelf-and-trigger.md) | Low-Med | migration `0018` (skills_audit + used_count/last_used_ms/source/deprecated_ms); `saveSkill` audit-first + no-op-guarded + **epoch-bumped**; the **skill shelf** in the cached block (name-ordered, capped, deterministic — the cache invariant); `isSkillsMode` + the **L1 skills clause** (8th mount flag; shipped two-variant w/ a 9th shelf-visible flag); `save_skill` description rework (what+when, "pushy"); `skills.enabled` into the settings panel | ✅ |
| v0.32.1 | [Skills into recall](skills-procedural-memory-2026-07/v0.32.1-recall-source.md) | Low | `'skills'` as a `retrieve()` source (3 unions + candidate loop, key = name+description — the Voyager pattern; shipped **relevance-gated, no recency term**, boot-frozen mount truth); recall-tool scope + output-enum extension + honest disabled error; `recall_skill` → `markUsed` (epoch-silent); **fix the rag_refresh `contentHash`→`embedCacheKey` mismatch** + pre-warm skill texts + migration `0019` cache reset | ✅ |
| v0.32.2 | [Dream distillation (dark)](skills-procedural-memory-2026-07/v0.32.2-dream-distillation.md) | **High** | `distill_skills` dream step between `run_diaries` and `rag_refresh` (zero protocol change); SkillPatch Zod + whole-patch rejection (incl. **no-resurrection** via new AND merge + in-patch dupes) + JSON-literal-null default + NaN-guarded per-cycle cap (2) + merge-over-duplicate + stale deprecation; `source='dream'` provenance; **never runs bun test**; every write audited + one-call `restoreSkill`; description-injection into the cached block closed at write+sink (review HIGH); behind `LUNA_DREAM_SKILLS=0`; **live dream A/B gates the flip** | ✅ |
| v0.32.3 | [Flip + owner surface + close](skills-procedural-memory-2026-07/v0.32.3-flip-owner-surface.md) | Medium | flip `LUNA_DREAM_SKILLS` default ON (call-time, restart-free panel toggle); `/_workspace` **Skills panel** (view/edit/deprecate/restore, dev-tools-gated, audited writes; the v0.31.0 soul-editor pattern); **LD #12 procedural-memory amendment**; initiative close | ✅ (live A/B passed: null-restraint + positive distillation live; one shape bug caught+fixed) |
