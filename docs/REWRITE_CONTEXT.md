...
2. **Self-continuation default behavior**: Locked to mechanical-only (no LLM call). Open: the concrete fresh-design shape in Initiative 5 — on this architecture a continuation is a **delayed normal turn** (timer + runTurn + user-role stage direction, the v0.6.2-validated injection pattern), so the open question narrows to trigger policy (mechanical brake + what, if any, bounded judgment) and its default. Python's 40%-coin-flip and `parse_continuation_line` machinery are explicitly not the starting point.
