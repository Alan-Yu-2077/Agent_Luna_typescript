...
2. **Self-continuation default behavior**: Locked to mechanical-only (no LLM call). **Now owned by Initiative 5 (lands v0.11.0)** as a *delayed proactive micro-wake* (the scheduler enqueues a near-immediate wakeup with `continuation` intent; reuses the proactive `runTurn` path — LD #15). Open only: ship-on-by-default vs opt-in, decided at v0.11.0 against the soak. Python's 40%-coin-flip and `parse_continuation_line` machinery are explicitly not the starting point; the v0.27.1 (user-role stage direction) and v0.28.1 (no model-emitted "more to say" flag) lessons carry.
