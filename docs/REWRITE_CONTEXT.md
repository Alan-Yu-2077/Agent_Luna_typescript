...
| LLM provider | **Anthropic via official SDK (default)**; **OpenAI Chat-Completions provider** behind the `Provider` seam — Initiative 16 (v0.23.x) | **Amended v0.23.0.** Originally "Anthropic-only; drop `openai_compat`" — but that cut was specifically for the Python adapter's broken `api_key_override`. A *fresh* TS `OpenAIProvider` takes `apiKey` per-instance (like `AnthropicProvider`, `provider/anthropic.ts:40`), so the bug doesn't recur and the dream summarizer-key isolation holds; chat is therefore no longer Anthropic-only. Resolved by `providerFor()` via a model→protocol **registry** (`provider/registry.ts`, v0.23.3) keyed on `LUNA_MODEL` (built-ins + `LUNA_MODELS_JSON` override; per-model quirks: token-param name, system/developer role, reasoning, tool support); `LUNA_PROVIDER` forces the protocol. Default path byte-unchanged; `cache_control` is stripped on the OpenAI path (auto-cache, not explicit); SSE streaming behind `LUNA_OPENAI_STREAM`. Embeddings (LD #13) unaffected. The runtime `mock` provider stays cut; `MockProvider` is a test-only seam. |
| Frontend scope | **TS-port `agent-app.js` controller only** | Live2D rendering + audio pipeline + GPT-SoVITS proxy stay as-is (cost/benefit). **AMENDED v0.26.1 (2026-07-02, Initiative 19):** scope extends from "web controller" to a **packaged desktop shell** — `packages/desktop` (Electron) spawns the `bun build --compile`'d `luna-server` as a supervised sidecar (kill-on-quit, bounded crash-restart), serves the `packages/web` production build over a pinned loopback origin (`:5177`), and owns app-data (`userData/luna.sqlite` via the existing `LUNA_DB_PATH`; keys in `userData/luna.env`, never bundled). The Live2D/audio/TTS internals remain untouched; the desktop app's server runs on its own `:8790` so it coexists with a dev instance on `:8787`. GPT-SoVITS stays an external optional process. |
...
| Memory layer model | **3 layers** — L1 (会话级活跃上下文: live thread + rolling summary) / L2 (全量化存档 + 日记周记: full-text archive, diaries generated from it) / L3 (长期记忆: durable facts Luna remembers/forgets). **Diary (day/week/month) is a dream OUTPUT living in L2, not a 4th layer.** Plus a prose **core memory** (self_state + relationship_status), always-injected, dream-updated, with a lightweight SQLite audit + `restore(n)`. | **LD #12.** Alan-locked layer defs (Python `memory-redesign-dream/README.md:29-34`); supersedes the earlier "four-layer (L1/L2/L3/diary)" wording. **AMENDED v0.17.0 (Initiative 10, owner correction per `MEMORY_DESIGN_DIVERGENCE.md`):** L1 active window = **~100 verbatim *turns*** (env `LUNA_L1_RECENT_TURNS`, 40–150), not a message cap; older history compresses to a **structured, size-bounded digest with importance anchors** (LLM-rated salience), replacing the unbounded `rolling_summary`. Supersedes v0.4.1's "12 pairs" default + messages unit. **AMENDED v0.17.1:** the **diary** is no longer "an L2 archive output, not injected" — it is an **injected long-range memory layer**: a standing day/week/month digest in the cached system block (`renderDiaryDigest`) **and** a recall candidate source (`collectCandidates` += `'diary'`), ranked by the Generative-Agents recency×importance×relevance score. Monthly diaries are generated. Closes `MEMORY_DESIGN_DIVERGENCE.md`. **AMENDED v0.30.3 (Initiative 22, The Soul File):** the **prose core memory** is no longer a standalone `core_memory` table — it is now the **evolving section of the DB "soul file"** (`soul.evolving_self`/`evolving_bond`), Luna-authored via the dream with the same audit + `restore(n)` (over `soul_audit`). The persona itself moved **file → DB**: `persona/default.md` is a **git-versioned seed** re-synced (hash-gated) into the soul's dev-authored, Luna-immutable **fixed core** on boot; runtime reads the DB. `core_memory`/`core_memory_audit` are dropped (migration `0017`). The always-injected + dream-updated properties hold; only the storage + authorship split changed. **AMENDED v0.32.3 (Initiative 23, Skills as Procedural Memory):** the memory model gains an injected **procedural layer** — the skill library (`skills` + `skills_audit`, migrations 0009/0018): a **skill shelf** (names + one-line descriptions, name-ordered, capped) rendered in the cached system block; `'skills'` as a fourth recall candidate source (relevance-gated, no recency term); dream-distilled via the `distill_skills` step (whole-patch-rejected, capped, never resurrecting deprecated skills, audited — every write is one `restoreSkill` call to undo); provenance-split by author: `saved` = the verify-gated awake tool, `dream` = distilled, `owner` = the `/_workspace` skills panel. The CUT Python "Skills subsystem" (减负 list — a tool-filter `list_skills`/`activate_skill`/`exit_skill`) stays cut and is unrelated to this library. |
