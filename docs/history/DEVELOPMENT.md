...
| `v0.25.2` | 2026-07-02 | **Collapsible companion UI — model glide + head-anchored comic bubbles → Initiative 18 ✅** : FLIP glide on the pixi beat (NEW `ease.ts` + pure `glide.ts` tween; `ModelDriver` composes base+drag+mode; `glideLayout(mutate)` eases the mode offset → 0; live-sampled: textbook ease-in-out, settles exactly at center). Alan's design review: bubbles **anchor beside her HEAD** (sink publishes `--luna-head-x/y/gap` per frame — drag/zoom/glide tracked; clearance `0.26×width` so they never cover her), **comic tails** (`::after` on `.latest` pointing at her face; the previous bubble's tail transitions away the moment it becomes history), bounded **random jitter** per bubble. Adversarial review: **3 CONFIRMED fixed** (fit() re-clamps + heals a stranding persisted drag; `glide.stop()` on the reduce-motion snap; OS `prefers-reduced-motion` honored in JS). +9 tests; **919 green, `tsc` ×3 clean**; browser-verified on the isolated :5273 preview. **Initiative 18 complete (3/3), zero server/protocol change.** | _branch_ |
| `v0.26.0` | 2026-07-02 | **Desktop app — port foundation + Electron rendering smoke** (Initiative 19 opens): `packages/web` gains a **production build** (`bun build` → a self-contained 8.4MB `dist/` incl. Cubism core + yumi); NEW `wsUrl.ts` fixes the #1 desktop break (fixed `ws://127.0.0.1:8787` + `?ws=` override — no more `location.hostname` derivation). NEW **`packages/desktop`** (Electron ^33): a pinned-`:5177` path-jailed loopback static host (`serve.ts`, sidecar-ready), a minimal shell window (`contextIsolation`, `backgroundThrottling:false`), and an automated **rendering go/no-go** (`smoke.ts`: hidden window + dead-port WS + DOM probe + PNG). **Smoke PASSED** — `headX` set ⇒ yumi renders AND animates in Electron's Chromium from the production bundle. +3 tests; **922 green, `tsc` ×4 clean**. Live chat round-trip deferred to Alan's supervised run (isolation policy). | _branch_ |
...
| `v0.26.2` | 2026-07-02 | **Desktop app — the pet window → Initiative 19 ✅** : `LUNA_PET_MODE=1` → a transparent/frameless/always-on-top window with `?pet=1` (the web strips the room + forces the companion layout); **region click-through** (a pure `petHitTest` over the sink-published model bbox + the bar/buttons → `preload` contextBridge → `setIgnoreMouseEvents`); geo via `LUNA_LAT_LON` in the template. **Packaged pet smoke PASSED** (`pet:true, bodyBgImage:"none", wsStatus:"open"` + a real-alpha PNG — yumi over pure transparency, no edge halos). +4 tests; **932 green, `tsc` ×4 clean. Initiative 19 complete (3/3).** | _branch_ |
...
| `v0.27.3` | 2026-07-03 | **Proactive directive leaked into a phantom user bubble**: a proactive turn's `userText` is the internal stage direction (`[System proactive trigger …]` / self-continuation priming), not a real user message — but `runTurn` persisted it verbatim as L2 `user_text`, so the frontend rendered the raw directive as a user bubble (visible since ~2026-06-16; grok's rambling just made Alan scroll up and notice). Fix: `userText: opts.proactiveTurn ? '' : opts.userText` (the directive still lives in `raw_json` for context). +1 regression test; **956 green, `tsc` clean.** Cleaned 7 already-leaked rows in the live DB (user_text cleared, replies + context intact). | _branch_ |
...
- Resolves a live confusion: Alan read the persistent proactive interrupts as the silence-ladder
  ...
  interrupted." Left the continuation behavior itself unchanged — once distinguishable, Alan judged
  it fine ("既然是补一句那就没问题了"). See [[proactive-ambient-interrupts-residual]] for the ladder side.
...
- Installed `corelocationcli` (Homebrew, 4.0.7) on the dev Mac. Validation: **1141 tests pass, 0
  ...
  `Asia/Shanghai`→Shanghai via the timezone tier (Location Services currently off).
...
- **The live dream A/B (the roadmap's hard gate) passed, 4 runs against yunwu on a copy of the
  ...
  (post-exam chat, funny animal facts) → the model returned the null patch, "nothing to distill";
...
- The A/B discipline (dark-launch → live A/B → flip) caught a shape bug unit tests could not —
  ...
  found real defects before they reached Alan's instance.
...
- Closes the gap Alan named: the fixed core was git-`default.md`-authoritative and Luna-immutable, so the
  human **owner** had no way to customize the persona short of editing code — "固定死". The soul is now a
...
- Realizes the "one Luna" Alan asked for (2026-07-03): whenever `bun run dev` is up (his usual state) the desktop window and the browser tab are literally the same running brain on the same memory; when it isn’t, the app is still self-contained (spawns against the shared repo DB). Attach-or-spawn keeps Initiative 19’s single-machine self-containment while removing the second-Luna divergence — without the two-servers-one-DB race (only one backend ever owns the port + the DB).
...
- **NEW `packages/desktop/src/tts.ts`** (~35 LOC): `resolveTtsConfig(env, repoRoot, existsFn)` → `{ dir, port, available, upstream }`, resolving the **same `LUNA_TTS_DIR` (default sibling `../Agent_Luna/TTS`) / `LUNA_TTS_PORT` (8788)** knobs `scripts/dev-all.ts` uses; `available` mirrors dev-all's probe (`<dir>/server/gpt-sovits-service.js` present). `ttsProxyScript(repoRoot)` → `scripts/tts-proxy.cjs`.
...
### `v0.28.4` — 2026-07-03 — EMERGENCY: L1 fold stall → unbounded context (~$1/turn)
...
- **Incident**: Alan's cost dashboard showed plain chat turns billing **~390K input tokens** (~$1/turn, cache-write 350K+). Diagnosis on a live-DB copy: the rebuilt session history held **1941 messages**, `window_low_water` was stuck at **498**, so `buildActiveContext` sent messages 498→1941 — **1444 messages ≈ 294K tokens — on every turn**, chat or not.
...
- **Initiative 20 ✅ complete (3/3).** Deferred to Alan's eyes (same standing desktop caveat): a real windowed run to drag + resize the pet over the desktop.

Inference:
- Closes Alan's "桌宠模式 无法拖动或缩放窗口，这点还是很伤": the move/resize affordance moved from the model (removed in v0.28.1) to the window itself. The per-pixel pass-through was a v0.26.2 nicety; trading it for real move + resize is what "整体可缩放" asked for. The hit-test seam is preserved, so the decision is reversible.
...
- **Single-user hardcoding (#12) + person unification (#26).** The literal "Alan" is removed from `buildWeatherBlock`/`weatherNoteFor`/`WEATHER_CLAUSE`/`TIME_CLAUSE` ("where the user is" / "the user feel they owe you presence"); proactive person-references unified to he/him (the `ambient` scenario + the felt-absence line + the `spontaneous` directive).
...
- The structural findings deferred here (redundancy de-duplication, single-user "Alan" hardcoding, person/voice consistency, priority hierarchy) remain for a later round.
...
- **Long-standing** (not a grok artifact): leaked rows dated back to 2026-06-16 — grok's repetitive proactive rambling just made Alan scroll the history up far enough to see it. Confirmed present on Claude (turn 314, 12:12, before the grok switch).
...
- This is a display/persistence-contract bug, not a model bug — the directive was always meant to be system-side priming, never a timeline entry. It stayed invisible because proactive turns usually scroll off-screen fast; the grok detour (empty turns + repetition) is what surfaced it.
...
- First slice of the settings surface Alan asked for ("开关不要只是丢一个 ENV 代码"): the most user-visible env flag became a click. The settings.json/luna.env split keeps luna.env as the user-edited *config* file (keys) while UI choices get their own shell-owned store — the same operator-vs-UI precedence model v0.27.1 generalizes server-side.
...
- The collapsed companion mode is now the full experience Alan described: she glides to center, you talk through a floating pill bar, and her replies pop as comic bubbles beside her face — newest at her mouth with the tail, history drifting up and shedding its tail. All of it rides the existing controller→BubbleView seam and the model's own render beat.
...
- **Browser-verified on an isolated preview** (port **5273**, `/` → `/app?ws=8899` — a dead WS port so the page can never touch the stable `:8787`, whose ws-reconnect hook could fire a proactive turn): collapse click → fixed pill bar + full-width centered yumi + bubbles beside her; expand click → panel restored (header/log visible, `position:relative`); **reload boots collapsed** (persistence); mobile 375px → pill clamps to 92vw, no breakpoint fight, model fills 738px. Screenshots taken of both states.
...
- **Not yet browser-verified visually**: a running preview would collide with the stable `:5173`/`:8787` instance, so the live "bubble beside the real model" check is deferred to Alan's run (or an isolated-port pass). The logic is unit-covered; the CSS reuses the established theme vars + animation patterns.
...
- Closes the proactive-parity-restore initiative: the wake is Alan's Python silence ladder, and she can now tune her OWN outreach personality (as in the Python design) without ever exceeding the operator's safety rail. `balanced` is the default, so nothing shifts until she moves her activeness.
...
- **Amended LD #15** in `REWRITE_CONTEXT.md` (2026-07-02): the detector-registry framing of the wake decision is reversed at Alan's direction; the ladder is the DECISION, detectors deleted; the agency turn + safety gate + dropped delivery layer stand.

Inference:

- Completes the reversal LD #15 recorded: the proactive wake is now purely Alan's Python silence-driven design — no calendar slots, no event detectors. This is the pure-Python-replica scope he chose, accepting the loss of the net-new weather-shift / promise-follow-through / open-thread / after-a-night triggers, and the behavior change that she **no longer proactively greets after a long absence** (long-absence → `sleeping`, she waits for the user).
- On Alan's next restart the ladder goes live (default-on); his `.env`'s temp `LUNA_PROACTIVE_SLOTS` becomes a harmless no-op (can be removed).
...
- **`proactive/proactiveTurn.ts`** — the four restraint-graded scenario framings (`SCENARIO_BODIES`) + the full `COMPANION_OPENER_CONSTRAINT` (陪他不查岗: self-disclosure or a fresh topic, never a 在吗/status opener, vary every opener) + per-session anti-repeat of recent spoken openers; `runProactiveTurn` gains a `scenario` path (silence is native — no Python SILENT sentinel).
...
- Restores Alan's original Python proactive design as the wake DECISION (amends LD #15's detector-only framing — fully resolved at v0.24.1). The default path is byte-unchanged with the flag off, so nothing in production shifts until the v0.24.1 flip.
