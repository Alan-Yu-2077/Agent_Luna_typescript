...
- Tests: **+10** — `tts.test.ts` (config defaults / env overrides / availability probe / proxy-script path) + `serve.test.ts` (integration: GET + POST forwarding against a fake upstream, binary passthrough, 502 on no-upstream, 502 on unreachable upstream, static-serve + path-traversal guard). **1006 all-package green.** Desktop-package `tsc` is red on **pre-existing** electron-devDep-unavailable errors only (11 on clean HEAD, unchanged by this change — verified by stash); protocol+server `tsc` clean; full suite green.
