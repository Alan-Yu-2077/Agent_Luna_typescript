...
| `v0.45.2` | 2026-08-08 | **A word when the song changes (Initiative 32, 3/5)** — the moment, disciplined. `proactive/musicMoment.ts`: `freshTrackMoment` (pure — current track changed ≤120s ago AND still playing; the window is ≥2× the 60s tick so every change gets 1–2 evaluations then EXPIRES, and rapid skip-skip-skip naturally collapses to the last track), sub-gate state shaped like Cadence (cooldown anchor + local-dated daily quota; migration 0020 adds the three session columns), `passesMusicGate` (45min / 3-a-day defaults, `LUNA_MUSIC_PROACTIVE_COOLDOWN_MIN`/`_DAILY`), commit split mirroring cadence (cooldown stamps every consideration, quota counts only messages). Wired INSIDE `maybeFireProactive`'s existing flow (M6: the tick is the only wake path — no new trigger): global rail already passed, ladder said nothing THIS tick, phase must be present (engaged/idle_watch — dormant/sleeping stay asleep, nudged keeps its backoff), THEN the sub-gate; a fire runs the spontaneous framing with `musicSeedFor` through the v0.22.0 seed channel (OQ3 → seed, no fourth intent) and consumes the GLOBAL account too (commitProactive/Silent on top of the phase commit — no exemption). `LUNA_MUSIC_PROACTIVE=0` kills only this surface. Tests 1861→1878: THE unbypassable-rail trio (global cooldown/quota/quiet-hours veto with a valid moment → zero provider requests), phase trio, sub-cooldown/sub-quota independence + next-day recovery, both-accounts consumption, seed carries track never artwork, flag-off death, rapid-skip at-most-one. | _working tree_ |
| `v0.45.1` | 2026-08-08 | **She knows without asking (Initiative 32, 2/5)** — ambient music context. `turn/nowPlayingContext.ts` mirrors `weatherContext.ts`: `musicAmbientEnabled()` (default on riding the provider, `LUNA_MUSIC_AMBIENT=0` the off switch) + pure `buildMusicBlock` over a `MusicFacts` scalar type — the artwork firewall is the TYPE (no field to leak tens-of-KB base64 through), progress is three coarse phases (start/middle/end via `positionAt` extrapolation; second-precision would churn prompt bytes for nothing), paused states say so, no track → no block at all. Injected in `runTurn` right after the weather block in the UNCACHED user tail with the same try/catch-warn-omit shell; `buildSystemPrompt` asserted byte-identical across track changes (the volatile-never-cached invariant). Proactive turns run the same engine, so she knows what's playing when SHE opens her mouth too — zero extra code (M5), the quiet listening-together baseline before v0.45.2's moments. `trackPhrase` exported for v0.45.2's seed. Tests 1845→1861: phase boundaries, block wording incl. singles/no-duration, firewall (key enumeration + no-base64), enable matrix, exactly-one-block injection, idle/dormant/flag-off zero-residue, cache boundary, torn-state isolation. | _working tree_ |
| `v0.45.0` | 2026-08-08 | **She can hear the room (Initiative 32, 1/5)** — the music base. `~/Desktop/luna-music-cli` (built + live-verified 2026-08-07) vendored as `packages/music-cli` — 16 payload-based tests ride along, platform-independent, so both CI legs run them; imports de-extensioned to house style, provenance headers stamped. `tools/media/nowPlaying.ts` is the resident provider on the weather-snapshot pattern: ONE `watch()` subscription (one `media-control stream` child, push not poll) keeps `{track, playing, changedAtMs}` in memory for synchronous reads; dormancy trio (LUNA_MUSIC=1 && darwin && doctor) — any gate failing = one log line, zero subscriptions, boot untouched; a dying stream gets ≤5 exponential-backoff restarts then dormancy with one warn, and shutdown kills the child (verified live: SIGTERM → zero orphans). Tools: `music_now` (safe-parallel, **proactiveRisk:'safe'**) with positionAt-extrapolated progress; `music_control` (play/pause/toggle/next/prev zod whitelist, global-serial, **deliberately unmarked** → fail-closed 'surface': she must speak before touching his playback in a proactive turn — the tripwire test pins both marks). Mounted via `withMusic` at the same boot-frozen instant as `withWeather` (flag+platform sync gate; async doctor gates the subscription, not the mount — dormant tools answer recoverable errs, never vanish). `LUNA_MUSIC` in ENV_TEMPLATE. `changedAtMs` recorded-not-consumed (v0.45.2's raw material). Suite 1807→1845, five packages tsc clean, real-machine lifecycle verified both ways. | _working tree_ |
| `v0.44.8` | 2026-08-02 | **No seams in her room (owner correction)** — the menu-mode canvas was the session layout's right-hand slot, so a model zoomed past the slot width was CLIPPED at the slot's left edge — a hard vertical seam through her sleeve, naked in the open menu room (in session view that same edge hides flush against the chat panel). Now `.menu-wide` spans the canvas across the whole stage — she extends naturally, only the WINDOW edge crops her — while the sink's new frame RECT (`setFrame`, `framedBaseX`) keeps her base position exactly where the session slot puts her: measured Δ0.16px menu→Talk and Talk→menu (sub-pixel `clientWidth` rounding), so D4 holds and the re-clip lands under the chat panel's fade-in. The frame is a rect, not a left inset — the slot also stops 22px short of the window's right padding, and centring to the canvas edge would have shifted her 11px at Talk. Drag clamps reason about the frame too, so a menu-time drag can't strand her outside the slot she returns to. Plus the owner's second note: the zzz doubled (26/33/42px, drift and headroom scaled, ambient 3.2s clock unchanged). `frame.test.ts` pins plain-frame ≡ old centring bit-for-bit, slot-frame ≡ slot-canvas position, and the 22px-padding case by name. Suite 1807 green, four packages tsc clean, verified live at zoom 1.8/1.25 against a throwaway 8901 sidecar. | _working tree_ |
| `v0.44.7` | 2026-08-02 | **The close-out, and CI turns green (Initiative 31, 8/8)** — the owner's named addition first: every push since 7/31 was red and mailing him, all environmental, root-caused to this very skill's stale \"CI no longer runs\" line. Model-file tests now `skipIf` the gitignored model (no fixture — they exist to check the REAL one), the retirement guard normalises Windows backslashes (the Windows leg caught the guard's own portability bug — why that leg stays), and the skill now says what is true: CI runs on every push and local green is not the finish line. D10 becomes an audit (every CSS duration in a declared band, strays listed), `prefers-reduced-motion` lands as a deliberate AMENDMENT to the v0.36.0 constitution — the manual-switch ban stands and is still tested, the OS accessibility preference is honored for UI stages only, and the media block provably never reaches her engine aliveness. The menu gets its version line; ARCHITECTURE.md gets The front door; the old panel + escape hatch stay for an observation week. Suite 1802 green, four packages tsc clean, **CI success — first green since 7/31**, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.44.6` | 2026-08-02 | **Modules and the persona editor (Initiative 31, 7/8)** — four uniform module cards (Chat LLM / Embedding / Web search / Weather) as DECLARATIONS over the wizard's own machinery: prefill (secrets as names, masked `sk-gt6U…wxyz`, plaintext only on focus), the vendor-hinted probes (advice, never a gate), and a new `luna:save-config` IPC that writes `luna.env.bak` first then per-field merges — README OQ2 lands as *restart to take effect*, with a one-click `relaunch()`. The persona editor puts the v0.31.0 owner capability behind a product door: her 4908-char fixed core in plain text, a line-diff preview that is PART of the save, empty writes refused twice, evolving read-only — and her `self-edit` tool still cannot touch fixed, the firewall tests running unchanged. Settings grows five→seven categories (Persona and Modules each earn a rail entry). Verified against a DB copy through the audited store, the real `luna.sqlite` byte-untouched. +10 tests. Suite 1799 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.44.5` | 2026-08-02 | **Settings, by which part of her (Initiative 31, 6/8)** — the three-tab slide-in panel becomes five categories behind the menu (Voice / Expression & Motion / Appearance / Behaviour / System), and the migration is an ADOPTION, not a rebuild: the page moves the old panel's live DOM rows — same elements, same listeners, same keys — so the live-read contracts physically cannot drift, and the risk shrinks to placement, which the reconciliation test pins (the exact eleven-key inventory, every PERF_FLAGS key with exactly one home). Lobby boots retire the ⚙ entirely — one hierarchy, no back doors — and Esc in chat returns to the menu; `luna:menu='0'` keeps the old panel byte-identical until the initiative closes. Voice gains a live health line; Behaviour is a deliberate placeholder (surfacing proactive knobs is its own decision, not a side effect of moving furniture). +6 tests. Suite 1789 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.44.4` | 2026-08-02 | **The skills page: what she learned (Initiative 31, 5/8)** — a growth record, not a manual browser: cards with use counts and relative last-use, the retired folded grey below, and the mark that carries the page's meaning — ✦ on the ones she taught HERSELF ('saved'/'dream' sources), because "one of these three she built out of a dream" is a companion growing. Bodies expand in place as plain text, one at a time; read-only by decision. Verified against her real DB (`✦ discover-latest-build` 8 uses / `self-architecture` 6.7KB body / `search-then-fetch`). +6 tests. Suite 1783 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.44.3` | 2026-08-02 | **The diary book (Initiative 31, 4/8)** — 28 diaries and 59 dream reports become a two-page book: calendar left where ONLY real days light (purple dot = diary, crescent = that night's dream), content right, a 1.6s page turn around the spine with paper-bend shadow, a lighter half turn for same-day Diary|Dream switches, latest-wins click queueing, and keyboard travel over lit days only. Dreams share the book (D7) through a translation layer that renders the consolidation pipeline as what she did that night — counts extracted from real detail strings, timings never surfaced, unknown steps degrading raw, aborted dreams reading 「这个梦断掉了。」 in italics. Read-only by decision: it is her writing. Zero WS anywhere — reading her diary cannot wake her, by architecture. Verified against a copy of her real DB, her actual 7/31 entry and that night's two dreams included. +16 tests. Suite 1777 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.44.2` | 2026-08-02 | **Her memory becomes something you can fetch (Initiative 31, 3/8)** — 28 diaries, 3 skills and 59 dream reports lived only in SQLite with zero front-end doors; `GET /api/data/{diaries,skills,dreams,soul}` (+ `POST soul/fixed`, the existing audited owner capability re-addressed) are the doors. Product surface, deliberately NOT behind the dev-tools gate; loopback is the boundary, same principle as the WS. Zod schemas in a new `protocol/data.ts` guard both ends; `report_json` is flattened server-side so the client never touches the raw string and a malformed row degrades instead of 500ing the list. The desktop forwards `/api/data/*` with the `/api/tts` discipline — an exact allowlist chooses the upstream target, so traversal matches nothing — and the packaged smoke asserts the forward end to end. Verified against a copy of her real DB: 28/3/59 all flow, the workspace aliases stay byte-identical, and Luna's `self-edit` still cannot reach the fixed core. +15 tests. Suite 1761 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.44.1` | 2026-08-02 | **Talk: she wakes for you (Initiative 31, 2/8)** — Talk is not a navigation: the menu's text column fades left, the chat reveals in its place, and she wakes IN PLACE through a four-beat sequence composed entirely of v0.43.x surfaces (state switch whose slow posture smoothing raises her head on its own, the sleeping→neutral eyelid handback so her first blink is the blink awake, `headLiftAlert`, a brow pulse, a light `tender` landing). **The wake gate is the wake itself** — no overlay, because an overlay would hide the animation that replaces it; the TTS warms underneath and the input never strands. ← Menu disconnects POLITELY: mid-turn it waits for `turn.result` — and for `proactive.finished`, since proactive turns emit no `turn.started` and forgetting them would cut her off exactly when she chose to speak unprompted. Dream enters from sleep without waking (a sleeper does not wake to fall asleep — the skipped wake plays when the dream ends). `activateSession` is re-entrant on the same client; the packaged smoke now CLICKS Talk, so `wsStatus 'open'` proves the real path. +12 tests. Suite 1746 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.44.0` | 2026-08-02 | **The cold lobby (Initiative 31, 1/8)** — opening the app no longer drops the owner mid-conversation: boot split into a lobby half (layout, sink, her asleep, the floating-text menu) and `activateSession()` (WS, controller, voice, geo), which in menu mode simply does not run — the backend is untouched until Talk, cold by construction since everything proactive rides the socket. The menu overlays the chat panel's own grid slot, which is the D4 mechanism: she already stands where the session layout puts her, so waking never moves her. Spring hover from a real damped spring sampled into CSS `linear()`; transition I orchestrated 600ms+900ms=1.6s; Quit lands on the desktop bridge; the packaged smoke goes two-phase (menu asserted cold, then `?menu=0` re-runs every old assertion incl. wsStatus open). pet / agent-only / `luna:menu=0` boot byte-identical to yesterday. +16 tests. Suite 1734 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.15` | 2026-08-01 | **Intensity stops being a volume knob (Initiative 30, 15/15)** — the owner noticed the model plays `emotion` safe, and measurement agreed: at 0.3 a `bright_delight` moved `mouthPucker` by 0.147 while `defaultIdleV1` alone swings `mouthForm` by ±0.22 — **the expression was smaller than her breathing**. v0.42.4's lesson (measurable ≠ visible) recurring, with a value the model chose rather than a coefficient as the culprit. The pose lerp and the overlay concentration both drop their intensity factor; every clip now plays at full amplitude, identical at 0.3 / 0.5 / 1.0 / omitted. **The version NOT shipped is the interesting one**: locking the wire value to 1 would have forced the ≥0.7 branch permanently high, making `annoyed`, `skeptical` and `awkwardV2` unreachable and taking the swirl-eyes overlay with them — 14 clips back to 11, half of v0.43.2 undone, and a character who meets every irritation with maximum sulk. So amplitude and branch SPLIT, and `emotion` narrows from *how loud* to **which of the two performances** (it still sets how long the mood lingers — that one accumulates across messages, so a small value there is brief rather than invisible). `Playback.intensity` is deleted rather than left stored-and-unread, cascading through `triggerEmotion`, `previewPose`, the sink interface and the bench readout; the `message` tool's describe is rewritten because `0 = barely visible, 1 = full` became a lie on landing. Two tests deleted (not loosened) and named in the record; +7 added, including a visibility floor and a **widened KNOWN LIMIT** enumerating the twelve brow channels across four clips that now sit permanently on their clamp — recorded rather than left to be discovered. The adversarial audit could not run (session limit); the three checks it was aimed at were done by hand and are listed. Suite 1718 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.14` | 2026-08-01 | **She has one voice, or none (归一化 runbook 第 1 步)** — the Web Speech browser voice was the distribution era's zero-setup fallback; once Luna became one person's single-machine companion (v0.41.0) its only remaining behaviour was cutting in with a stranger's voice every time api_v2 hiccuped. Deleted: `webSpeechSink.ts`, and with it `voicePick.ts` (which existed solely to choose whose system voice to borrow). The backend union narrows to `'none' \| 'http'`, the default flips from `'browser'` to **`'none'`**, and a persisted legacy `'browser'` resolves to silence — never promoted to `'http'`, which would point an install with no server at a backend that isn't there. Failure semantics change from *speak it anyway* to **skip it and log it**. **An adversarial audit of the first pass returned twenty confirmed defects, three of them blockers, all in the half I had treated as cosmetic**: the desktop's first-run luna.env template was still minting `LUNA_TTS_BACKEND=browser` into every new install, `.env.example` and `docs/SETUP.md` still called it the default, and the wizard default I had "fixed" to `'http'` would have written a backend the owner might have no server for — the exact promotion the resolver refuses, undone one layer up. Narrowing a type deletes the code that uses a feature; it does not delete the promises. So the guard test now sweeps the whole repo and greps prose, not just identifiers — its first draft passed while four live promises shipped. One behavioural addition: `speak` now reports whether the words reached the room, because v0.43.13 fires a question's head-tilt when `speak` resolves, and with the fallback gone that also covered "this line never happened" — she was tilting her head after questions the room never heard. Suite 1712 green (1713 − 14 + 1 + 6 + 4 + 2, every test's disposition listed in the detailed record), four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.13` | 2026-08-01 | **Punctuation gestures, and Initiative 30 closes (Initiative 30, 14/14)** — the audio told v0.43.12 *where the stresses are*; the text tells this version *what kind of sentence that was*. A sentence's final mark maps to a gesture — `？` tilts the head, `！` leans in, `……` lets the gaze drift off, `～` sways — and the timing is the whole content of the version, which is why the hook lives in the controller rather than the engine: only the controller knows when a sentence starts and finishes playing. Emphasis fires on the audio's `onStart` and runs through the utterance; a question's tilt fires when the `speak` promise resolves, because that is where the rise lands in speech. **Two independent signals on purpose**: the envelope carries no pitch, so a rising question is invisible to the stress detector no matter how it is tuned, and punctuation is the honest approximation available without an F0 tracker. Consecutive identical gestures alternate direction so three questions do not read as a stuck animation — but only the LATERAL channels mirror, since negating a brow lift would turn emphasis into a frown. **It adds a trigger, not a mechanism**: v0.43.12's pulse layer took these unchanged, which is the test of whether that layer was built at the right altitude. The gate is `pulseSpeech`, while `addPulse` stays open, because the workbench is a debugging surface and a product toggle must not make it lie. Closes the initiative: `ARCHITECTURE.md` gains a Speech-and-listening passage covering the three signals, the one mechanism and the unchanged arbitration rules, and the whole-initiative regression rides in the suite — v0.43.0's blink probe, v0.43.2's outro monotonicity, v0.43.3's zero-frozen-frames and v0.43.5's 60 fps equivalence gate all still green. +30 tests. Suite 1713 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.12` | 2026-08-01 | **She nods on the stresses (Initiative 30, 13/14)** — the gap Initiative 30 named at its founding and then did not touch for twelve versions: *"六个版本做完，她说话的样子和今天一模一样。"* It needed no new data source. `lip.open` has been a per-frame energy envelope flowing through `setMouth` since the lip-sync shipped, and nothing but the mouth ever read it. A new `AccentDetector` keeps an exponential baseline over it and calls a stress when a frame runs 1.6× above baseline and over an absolute floor, with a 550 ms refractory window and a hard cap of six per sentence — a rhythm fuse, because however emphatic the delivery she should not peck. Each stress becomes a **pulse**: v0.43.11's constant bias generalised to a time envelope, a pose sampled under a half-sine and added last, so a 1.2–2.8° nod plus a faint brow lift rides on top of whatever is already playing. **Head pitch is written in the pre-physics pass (F26), which is exactly why the hair follows for free** — the physics does the expensive half and it costs nothing. Two things kept it honest, and both began as bugs in the first draft: the peak is compared against the baseline *before* the current frame folds in (a loud frame drags the baseline up and would quietly discount the peak being tested), and the tests measure **differentially** against a flag-off twin, because the idle drives `headPitch` too and an absolute reading measures the sway. No pitch detection — a rising question is invisible here and v0.43.13 approximates it from punctuation instead, two independent signals rather than one overloaded one; and the browser fallback voice has no envelope, so this stays silent there rather than faking it. The workbench gains a stress lamp, because the constants are a feel judgement and calibrating them by ear against his own voice model is the owner's part. +15 tests. Suite 1689 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.11` | 2026-08-01 | **The half of the conversation where she is not talking (Initiative 30, 12/14)** — the initiative's own open question named this gap: six versions made her vivid **while quiet**, and a conversation is half waiting. Two additions, both additive, both reusing machinery that already existed. **(1) She notices you typing.** `setListening` is a LEVEL, not an event — held true while there is input, dropped on a 1.5 s debounce and on blur and on send — so per-keystroke jitter is impossible by construction rather than by tuning; the engine only ever sees yes or no. The bias itself is tiny (`headYaw −3°`, `bodyYaw −1.5`, a 0.05 brow lift) and is ADDED beneath everything else, so it rides under a playing clip instead of replacing it. The v0.13.x gaze arbitration is deliberately untouched: turning toward you is a head-and-body act, and letting attention grab the eyeballs would fight the focus controller for the one channel the user's own mouse is already driving. **(2) `thinking` becomes thinking.** v0.43.4's spontaneous scheduler already let `thinking` through its gate — what was missing was never scheduling, it was anything worth playing there. Now it draws from a thinking pool at 3.5–8 s instead of the idle 8–20 s: `lookAwayThenBack` reused, plus two new `ActionDef` rows — `gazeUpRecall` (gaze up-and-right, head following: the posture of retrieval) and `browKnit`. Data only, no new mechanism. One real trap found: the no-repeat memory is 3 and the thinking pool has exactly 3 entries, so applied literally she would freeze mid-thought after three gestures — the memory is now a preference with a fallback. +8 tests, and three existing assertions corrected: two hardcoded gesture counts became derived, and a claim that no thinking gesture touches an eyelid was simply wrong (`lookAwayThenBack` does, legitimately — v0.43.4 rejected stripping it), so it now asserts the release instead. Suite 1674 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.10` | 2026-08-01 | **What she wears is his to decide, and it stays decided (Initiative 30, 11/14)** — v0.43.8 made the model's props try-on-able for a session; the owner's ruling was that props stay walled off from the EMOTION system and open to HIM, which means wearing has to survive a restart. A `COSTUME` table (eyepatch / mic / puppy / two mutually-exclusive hairstyles), a `luna:costume` storage module, a Costume card on the Avatar tab, and **zero new engine code** — v0.43.8's per-frame `setManualParam` was already the runtime, so this version is a table, a storage module and a card. Two invariants carry it, both now tests rather than prose. **`COSTUME` and `ALL_OVERLAY_PARAMS` must be disjoint**: overlays are zeroed every frame by the clip layer, so an id in both would mean a clip ending quietly undresses her — and it would read as a persistence bug rather than a table one. And **the write set is total**: every catalog item gets an explicit value on every change, because Add-blend params never decay, so a state change has to write what is now false as well as what is now true. `ParamarmupL/R` is excluded on purpose (a raised arm is gesture material, not clothing). The bench's costume rows now write through the same persisted state, so there is one answer to what she is wearing. This closes the three-way wall in testable form: the emotion system never chooses a prop, the LLM cannot either (the `message` schema has no such field), the owner does as he likes. Verified live across a reload, including a clip's heart-eyes overlay running at 0.95 without disturbing either worn param. +13 tests. Suite 1666 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.9` | 2026-08-01 | **A new face gets looked at before it gets committed (Initiative 30, 10/14)** — the owner's ask, verbatim: *"哪怕是后面要新加情绪资产，也可以先走我一个人类视角看看离不离谱。"* This is that pipeline. `FaceVm.setComposeMode(pose)` freezes every living layer — idle, state bias, mood, clip, gestures, residue, microsaccades — because all of them write each frame and would drag an edited channel back out from under the slider. **What it does NOT freeze is the write path**, and that is the constraint the whole version hangs on: the sliders edit PRE-GAIN state values because that is what `faceData.ts` stores, so a preview that skipped `FACE_PARAM_GAIN` would look right and export numbers meaning something else. 35 sliders in collapsible groups, bounds read out of `clampStateValue` rather than re-typed; load any of the 14 clips in as a starting point, because authoring a new face means editing a nearby one, not filling 35 zeroes; **A/B play** runs the composed pose as a real clip (via a transient `EmotionDef` and a new `defOf` indirection, under an id that `triggerEmotion`'s guard can never reach) at the SHORT timeline, since a pose that reads as a still is not the same as one that reads arriving and leaving; **Copy EmotionDef** emits JSON that pastes straight into `faceData.ts`, with `owns` inferred from which groups the pose actually touches and a `"//"` line admitting that `entryState` is a 0.6 approximation to tune by hand. Compose deliberately writes the eyelids — that is v0.43.0's invariant working as written (*persistent* layers never, *time-bounded* ones may and must release), and the release is tested rather than assumed. Verified live end to end: `disappointed` → three edited channels → a 25-channel export → A/B play. No new clip was added to `faceData.ts`; this ships the pipeline, the first faces through it are his. +16 tests. Suite 1653 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.8` | 2026-08-01 | **Nine gestures and seventeen assets, reachable by hand (Initiative 30, 9/14)** — the bench skeleton could reach affects, clips, states and idle profiles; it could not reach the two things that were never manually triggerable at all. `FaceVm.playAction(name)` fires one of the nine authored gestures on demand — until now they existed only as a clip's `actionRefs` or as v0.43.4's spontaneous scheduler, so there was no way to simply watch one; it keys the instance `manual:<name>:<t>` so it coexists with a scheduled one and reuses `applyActions` unchanged. `FaceVm.setManualParam(pid, value \| null)` wears any raw model param, which is how the 17 shipped `.exp3.json` assets — costume and props included — finally become try-on-able. **The per-frame write loop is the mechanism, not caution**: prop params sit outside `ALL_OVERLAY_PARAMS` so nothing zeroes them, but pixi-live2d-display's motion update has a save/restore cycle that discards a one-shot write, so a "set it once" API would pass its tests and flicker on screen. Written LAST, after the overlay loop, so wearing an overlay id beats that loop's zero; releasing queues exactly one 0, because Add-blend params do not decay and a forgotten entry means she wears the eyepatch forever. The catalog is hardcoded (the model tree is gitignored, the bundler cannot enumerate it) and checked against the real `yumi.cdi3.json`, the same guard v0.43.1 used. The prop wall is unchanged and re-asserted: the rule was never "these are forbidden" but "the emotion system must never CHOOSE them" — the bench is the owner's hand, a different actor. Emotional asset rows now light up while the clip layer drives them, making the `overlayRefs` table observable instead of source-only. +15 tests. Suite 1637 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.7` | 2026-08-01 | **A bench for the face (Initiative 30, 8/14)** — the expressive stack reached six layers, five flags and three trigger sources with exactly one observable surface: her final face. The owner's words were "我实在有点乱了 … 需要一个专门的 live2d-debug 工作台". `?workbench=1` is that bench — an early-exit URL mode beside `?setup`, mounting the **real** `createPixiLive2DSink` on the shipped bundle (a bench with its own renderer would let him tune a face the app then performs differently, which is worse than no bench). Left: the model. Right: every one of the 15 wire affects, 14 authored clips, 4 states and 5 idle profiles as a button — all **derived from the engine's own tables**, so a new affect appears here without anyone remembering to add it — plus the five flag toggles and a 2 Hz readout of mood / clip+phase / running gestures. The intensity slider draws its tick mark at the imported `HIGH_INTENSITY`, so v0.43.2's escalation branch stops being folklore: 0.69 performs `annoyed`, 0.70 performs `poutyAnnoyed`, verified live against the real renderer. Engine changes kept to two, both read-only: the `__lunaDbg` gate widens to `dev` OR `workbench` (the bench is a consumer of the bridge, so `Live2DSink` gains no method), and `FaceVm.currentPlayback()` joins v0.43.4's `activeActionIds` as the clip layer's observation point — phase-honest, so a finished clip reports `null` instead of lingering as a label. The five per-tick flag keys move into one `perfFlags.ts` shared by the engine, the settings card and the bench: three writers of the same string literals is exactly how a toggle ends up controlling nothing. +20 tests. Suite 1622 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.6` | 2026-08-01 | **Telling her which levers she has (Initiative 30, 7/14)** — six versions built her a face that can do far more than it used to, and none of it was visible to the only participant who has to choose: the `message` tool's two performance fields described themselves as "pick the closest key" and "0 = barely visible, 1 = full". The live test on 2026-07-31 caught the cost in her own words — she told the owner she could not trigger heart eyes (she can: `bright_delight`) and that she carries no state between messages (untrue since v0.42). Both describes rewritten. `expression` now names the four affects carrying a drawn asset and states that a strong affect keeps tinting her resting face for tens of seconds after the clip ends; `emotion` states the real semantics — **≥ 0.7 escalates** four affects into a genuinely different clip, and **omitting it never escalates**. The old wording was worse than thin, it was misleading: a "0 → 1 dimmer" reads as safe to leave off, which under v0.43.2's branch keeps the four escalated clips — the most distinctive poses in the set, only just made reachable — unreachable in practice. Zero behavior change: no enum variant, no threshold, no `superRefine` edit. +3 tests asserting keywords and the four escalating affects **by wire key**, not prose snapshots, since the wording will be re-tuned against the workbench. Suite 1602 green, server tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.5` | 2026-07-31 | **Frame-rate independence, per-organ pacing, and a re-tune that measurement said not to do (Initiative 30, 6/6 — closes it)** — the smoothing was a fixed per-frame lerp coefficient, i.e. correct at exactly one frame rate: at 30 fps it converged the same *fraction* per frame with half as many frames, so the same expression settled differently under load. It becomes `1 − exp(−rate·dt)` over the real elapsed time, with `rate` derived so that **`approach(sm, 1/60) === sm` to 1e-12** — the gate this whole version hangs on, asserted directly rather than sampled, because if the two forms are not equal at the rate everything was tuned at then this is a re-tune wearing a refactor's clothes. Only after that gate: per-organ speeds (eyes 1.5×, mouth 1.4×, brows 1.2×, gaze 0.9×, posture 0.6× — the SDK's eye-26/mouth-24/head-8/body-6 table as NUMBERS, four values rather than another 33 knobs), applied as a multiplier so v0.42.2's arousal modulation survives underneath. A 0.25 s dt clamp means a backgrounded tab cannot teleport her face on refocus. **The planned `GAIN` recalibration was measured and then NOT done**: the prediction was that shorter clips plus the thaw would leave the mood over-exposed, but re-measuring the settled magnitudes gave eye-smile 0.406 (v0.42.4: 0.371), brow 0.255 (0.229), mouth 0.206 (0.337) — within ±10%, i.e. noise against an idle that swings the mouth ±0.22. Changing a hand-tuned constant to satisfy a plan rather than an observation is how the v0.42.x drift started. `ARCHITECTURE.md` now documents the thaw and the three arbitration rules (eyelids belong to the blink, head/body are physics inputs, smoothing is time-based) that none of the code makes guessable. +6 tests. Suite 1599 green, four packages tsc clean, repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.43.4` | 2026-07-31 | **A beat instead of a sit, and a face that does things unprompted (Initiative 30, 5/6)** — three changes, all about time. **(1) Performances are ~2.5 s, not ~6.** The authored `performMs` ran 5200–6800 ms, correct when a clip ending meant the face snapped back to exact baseline with nothing left over; since v0.42.x the mood outlives the clip (6 s hold floor, ~38 s half-life), so the aftertaste is the field's job and the performance should be a beat. Mapped rather than re-authored (`timelineFor`) so the relative ordering the poses were tuned with survives and both sets coexist behind a flag. The floor is **2200 ms, not the 1600–2200 the plan proposed**: two tests sample at t=3000 ms to prove a clip reached its pose, and at 1900 that sample lands after the clip is over — measured 0.018 where >0.5 is required. Retuning the assertion to match the code would have discarded exactly the guarantee it exists for. **(2) Nine authored gestures finally fire on their own.** They were reachable only as a clip's `actionRefs`, and six of the ten reachable clips list none — left alone, she did nothing but breathe and sway. Now one every 8–20 s, drawn with mood-weighted probability from the same VAD field that tints her face (warm → leans in, guarded → leans back), never repeating within three, and **only while no clip is playing** — `applyActions` hard-sets without consulting `owned`, so an overlapping gesture would punch straight through a performance. **(3) `physicsPassthrough` finally has a reader.** `adorable` has declared `['eyeOpenL','eyeOpenR',…]` since it was authored and nothing ever looked at it — the field WAS the author's note that this clip should hand its eyes back. Unread, it meant her biggest, cutest expression was performed with a 6-second unblinking stare. Resolves Initiative 30's open question by implementing the intent rather than deleting the field. The plan's recommendation to strip `eyeOpen` from `slowBlinkAffection`/`lookAwayThenBack` was **rejected**: under v0.43.0's corrected invariant those are time-bounded and legitimate, and stripping them would have deleted authored character to satisfy a rule that no longer said what it used to. +13 tests. Suite 1593 green. | _working tree_ |
| `v0.43.3` | 2026-07-31 | **The peak stops being a still photograph (Initiative 30, 4/6)** — `ownedKeys` gives a playing clip exclusive use of its channels, so for the whole perform window the face was *literally* a frozen image: measured across `adorable`'s perform frames, `ParamBrowAngleL`, `ParamMouthShrug`, `ParamMouthpucker` and `ParamBrowYL` each changed by **0.00e+0 per frame** while `ParamAngleZ` moved 0.116 — body swaying, face a mask, for up to 6.8 s. The one idea worth taking from soullink-emotion-sdk's `MotionMixer` is that layers should ADD rather than one winning outright, so a fraction (0.22) of the idle's *deviation from rest* is now added back on top of whatever the clip set — the authored shape is kept, it just breathes. **Measurement then forced a second lever**: the DEFAULT idle profile drives only `mouthForm` and `mouthOpen` on the face, so brows, pucker, shrug and cheek had no idle motion to leak and would have stayed exactly as frozen. Hence a per-channel out-of-phase wobble (±0.035, one cycle per 3.3–10.5 s, incommensurable rates so nothing pulses in unison) scaled by how far the channel sits from rest — a channel the clip left alone gets nothing, so a calm face is untouched. Also lands `pupilX/pupilY` → `ParamhitomiX/Y`, two channels present on the model since it was drawn and never driven by anything: a target-and-dwell microsaccade inside the eyeball, independent of where the gaze points and owned by no layer. Behind `luna:live-peak` (default on, **Living expressions** in settings). The metric was rewritten during implementation — a per-frame magnitude threshold punishes gentle motion (a channel breathing 0.018 over a 5 s cycle moves 7e-5/frame and is perfectly visible), so the assertion is **zero frozen frames** plus a visible-range bound, which is the exact inverse of the bug. +10 tests. Suite 1580 green. | _working tree_ |
| `v0.43.2` | 2026-07-31 | **Fourteen faces instead of ten, and the blush stops flaring goodbye (Initiative 30, 3/6)** — two unrelated bugs, both one-liners in effect. **(1) The vocabulary.** `AFFECT_TO_EMOTION` collapsed 15 wire affects onto **10** clips: `curious` swallowed three (including `alert_surprise` — surprise was being performed as mild curiosity) and `tender` swallowed three, while `fakeFierce`, `embarrassed`, `poutyAnnoyed` and `disappointed` — four of the most distinctive hand-authored poses — **could never be reached at all**. The fix uses a field already on the wire and previously ignored by this lookup: the optional 0–1 `emotion` intensity now selects between a low and a high form, so `guarded_distance` leans back at 0.3 and bristles at 0.9, `annoyed_resistance` goes flat-look → full cheek-puffed sulk, `awkward_lightness` → properly flustered, `gentle_concern` → visible dismay. **All 14 clips reachable**, no protocol change. A subtlety that would have inverted the whole feature: `emotion` is *optional*, and the frontend's 0.95 pose-strength fallback must not leak into the branch decision — otherwise every message without an explicit intensity performs the intense clip. `affectToEmotion` therefore takes the raw possibly-undefined value; a test asserts undefined behaves as 0. **(2) The flare.** `stage.weight` is direction-relative — during the outro it counts progress *back* toward baseline — and `overlayParams` read it as absolute strength. Every overlay therefore FADED IN as its clip wound down, hit full value, and hard-cut to zero on the frame the stage went inactive. Measured on `shy`'s blush: 0.000 at 6600 ms → 0.449 at 7200 → 0.990 at 7800 → 0 at 7900, on six of the ten reachable clips. Overlays now also scale with intensity, matching the pose's existing `lerp(base, raw, intensity)`. +13 tests including a whole-outro monotonicity trace. Suite 1570 green, web + server tsc clean. | _working tree_ |
| `v0.43.1` | 2026-07-31 | **The artist's peak assets finally reach the screen (Initiative 30, 2/6)** — Yumi ships **17 `.exp3.json` files and not one had ever played**: `yumi.model3.json`'s `FileReferences` has no `Expressions` key at all, so nine hand-drawn emotional peaks sat dark in the folder. The plan called for an exp3 loader plus `suppressParamIds` ownership hand-off; **reading the files killed that design** — every single one is one `{Id, Value: 1.0, Blend: 'Add'}` toggle on a custom parameter, which is precisely what `OVERLAYS` already drives. So this is four table rows, not a subsystem: `爱心眼`/`星星眼`/`眼泪`/`蚊香眼` → `adorable`/`playful`/`disappointed`/`awkwardV2`. Of the other five "unused" assets, three (`猫猫嘴`/`歪嘴`/`舌头`) were already `paramMap` channels and two were already overlays — the honest count of genuinely dark emotional assets was **4, not 9**. The remaining eight are **costume and props** (mic, floating dog, eyepatch, two hairstyles, both raised arms, the bow-press), and nothing in the file format distinguishes those from heart eyes, so the boundary is now a hard test rather than a convention. `ALL_OVERLAY_PARAMS` became derived from the table instead of a hand-kept parallel list — it is the set `flushPose` zeroes each frame, so a forgotten entry would latch an asset on permanently. +11 tests: param ids checked against the real `yumi.cdi3.json`, prop-leak detection, dangling-ref integrity, a ≤3-overlays restraint bound, and end-to-end "the heart eyes light up mid-clip, then release to exactly 0". Suite 1557 green, web tsc clean. | _working tree_ |
| `v0.43.0` | 2026-07-31 | **She blinks again — a v0.42.3 regression, found by measuring instead of asking (Initiative 30, 1/6)** — the continuous-affect layer shipped an `eyeOpen` term, and because `FaceVm` writes at `'beforeModelUpdate'` — deliberately *after* the built-in `CubismEyeBlink`, so expressions win — that term pinned `ParamEyeOpenL` on **599 of every 600 frames at a constant 1.000**. With the flag off: 0 of 600. Her model's `Groups.EyeBlink` names exactly those two parameters, so for eight days the default build simply did not blink. The `IDLE_EYE_SKIP` guard that was supposed to prevent this (`faceVm.ts:84`) is checked only inside `applyIdle`, and no idle profile writes an eye key — **it had never fired once**. Two deletions, ~5 lines: the `eyeOpen` projection leaves `affectPose` (it also bought nothing — the default state already sits at 1, so `clampStateValue` discarded the whole positive-arousal half and kept only the half that *widened* her eyes in low moods), and `eyeOpenL/R: 0.85` leaves `STATE_BIAS.thinking` (measured 501/501 frames pinned — a path that predates the affect work entirely: she stared while waiting for a reply since v0.13.1). `sleeping`'s `0` stays; shutting her eyes is that state's whole point. **Corrected mid-implementation**: the plan's invariant said "no layer may write eyeOpen" — but all 14 clips do, carrying authored asymmetries (`playful` 0.62/0.94, `skeptical` 0.62/0.86) that are exactly the character this initiative exists to protect. The invariant is therefore *persistent layers never, time-bounded layers may — and must release*. +11 tests, including both halves of that and the 600-frame write-count probe a last-value assertion could never have caught. Suite 1546 green, web tsc clean. | _working tree_ |
| `v0.42.4` | 2026-07-31 | **Perceptibility — the mood was real but invisible (Initiative 29 follow-up)** — the owner looked at the shipped build and said 我怎么感觉没什么区别. He was right, and the cause was mine: the initiative README explicitly said the SDK's coefficients "must be re-tuned against *her* 33 channels", and then v0.42.1 shipped them **verbatim**. Measured on the real pipeline, a full-intensity `bright_delight` settled at VAD `{0.73, 0.47, 0.09}` and displaced `ParamMouthForm` by **0.130** of full range, `ParamEyeSmile` by 0.143, `ParamBrowY` by 0.089 — while `defaultIdleV1` alone swings `mouthForm` by ±0.22, i.e. the mood was quieter than the idle it was riding on. Those magnitudes are correct for the SDK, where the undertone is the *only* emotion channel; Luna layers it beneath 14 full-amplitude clips, so the same numbers read as nothing. Fix is one constant: `GAIN = 2.6` applied in `put()` before the ceiling, so every relative weight, interaction term and the 0.46 ceiling are untouched — only the read changes. Re-measured: mouth **0.337**, eye-smile **0.371**, brow **0.229**; the ceiling now genuinely binds at the VAD corners (as designed) instead of never being reached. +2 tests, including the regression that would have caught this: *a strong mood must move the mouth by a visible fraction of full range*, not merely a measurable one. Suite 1535 green (3 consecutive runs), repackaged, smoked `ok:true`, delivered. | _working tree_ |
| `v0.42.3` | 2026-07-31 | **Continuous affect ships ON — Initiative 29 ✅** — the default flips (`luna:affect = '0'` is now the escape hatch, matching how `LUNA_SHELL`/`LUNA_REPO_MAP`/`LUNA_PROACTIVE` are gated), a **Mood memory** toggle lands in the settings panel's Avatar card (client-side like the idle-profile selector; the sink re-reads the key every tick so it takes effect live with no reload), and `describeAffect(vad)` turns the invisible state into something checkable — `"warm · calm · yielding (v+0.55 a−0.15 d−0.40)"` — exposed on the `?dev` bridge as `affect` + `mood()`. `ARCHITECTURE.md` no longer describes a pre-affect stack: it now documents the real layer order (idle → state bias → **mood undertone** → clip → actions → smoothing → gain/clamp) and the ownership rule that lets lip-sync and gaze-follow coexist with it. **Deviation from the plan, recorded**: the 0–2 expressiveness slider was NOT built. The settings panel is server-registry-driven and has no client-side slider helper, so it would have meant a new layout primitive + CSS + tests for a knob on a feature nobody has lived with yet; `reactivity` is already an `AffectOptions` field, so it can land later without redesign. The toggle — which is also the escape hatch for a flipped default — was the part worth shipping now. +4 tests (band boundaries, extremes, the signed two-decimal tail, monotonicity). Suite 1533 green, web tsc clean. Packaged, `smoke:packaged` → `ok:true`, new `Luna.app` delivered to the Desktop (previous kept as `Luna-prev.app`). | _working tree_ |
| `v0.42.2` | 2026-07-30 | **The living baseline — she is *in* a mood, not merely tinted by one (Initiative 29, 3/4)** — three absorptions, each separately observable. (1) **Mood-shifted neutral** (`deriveNeutralParams`): `tick()` no longer starts from the constant `FACE_VM_DEFAULT_STATE` but from `restingState(mood)` = default + undertone × `REST_SHARE` (0.6), clamped per key. This substitution also moves `def` — the value the smoother pulls toward AND the write-gate compares against — so it changes resting behaviour, not just peaks. At a zero mood it returns the old default *exactly*, which is what keeps the disabled path bit-identical. (2) **Arousal-dependent smoothing** (`deriveParameterSmoothing`): the constant 0.18/0.24/0.34 becomes `base × (1 + 0.55·arousal)` bounded to 0.6×–1.6×, so an agitated face moves snappier and a becalmed one languid. (3) **Ambient drift**: a wander of ±0.05/±0.07/±0.03 re-picked every 1.7–5.9 s, added to what `current` chases but never into `target` — so it can never accumulate into a mood that walks away. New `ambient` + `rng` options keep it injectable; `ambient: 0` is exactly still, which is what the byte-identity regressions now use. Two v0.42.1 tests were correctly invalidated by this version and rewritten rather than patched: arousal-dependent smoothing changes every channel's convergence *path*, so an owned clip channel is now asserted to land on the same **value** rather than trace the same trajectory. +17 tests, including the initiative's headline: after `bright_delight` and **25 s** — far past the longest clip timeline (~8.5 s) — the mood-enabled face is still measurably warmer on `ParamMouthForm` than the same run with the layer off, and a sour mood rests visibly differently from a bright one. Suite 1529 green. Flag still `luna:affect` (off). | _working tree_ |
| `v0.42.1` | 2026-07-30 | **The undertone layer — the mood reaches the face (Initiative 29, 2/4)** — NEW `affectPose.ts`: a pure `Vad → Pose` projection adapted from soullink-emotion-sdk's `VADExpressionMapper` (rectified components + multiplicative interaction terms), retargeted onto Luna's 33 channels. Two forced departures from that design, both found by reading her rig rather than the plan: (1) the layer is **additive, not assigning** — her idle writes real motion into the same channels (`headRoll: sway*40`), so a `applyPose`-style layer would have flattened the sway; a new `addPose` accumulates instead. (2) Channels are emitted in **native units** — her angle channels are degrees (~±30) while eyes/mouth/brows are normalised, so one ceiling would be either invisible on the head or violent on the mouth; the SDK's 0.46 ceiling holds for normalised channels and is scaled ×8 for angles (a mood can lean her head ~3.7°). Wired into `FaceVm` between the state bias and the clip, applied through the same ownership set, so lip-sync and gaze-follow need no new logic. `setExpression` now feeds both systems from one wire event: the clip (still via the 15→11 lookup) and the field (all 15, no fan-in). `DEFAULT_BASELINE` moved to dead zero — a warm resting temperament is v0.42.2's subject, and shipping it here would have cost the "enabling this is provably safe" guarantee. Behind `luna:affect` (opt-in, read per tick). +18 tests including three exact-stream regressions: flag-off is byte-identical to a FaceVm with no affect at all; flag-on-at-rest is byte-identical too; and 30 s after a clip has fully ended she is still measurably warmer than the old engine, which had returned to flat. Suite 1517 green. | _working tree_ |
| `v0.42.0` | 2026-07-30 | **The affect core — continuous mood, no rendering yet (Initiative 29, 1/4)** — the owner's complaint ("映射预设表情,未免有点僵硬") turned out to be architectural, not renderer-level: `FaceVm` already writes 33 face channels per frame with idle/state/emotion/action layering and channel ownership, but the EMOTION layer is *event*-shaped — `setExpression` picks one of 14 hand-authored clips through a lossy 15→11 lookup, plays a ~7–8 s intro/perform/outro, then returns to exact baseline with **zero residue**. Nothing carries between messages. NEW `packages/web/src/live2d/affect.ts`: a 3-axis VAD field (valence/arousal/dominance, each −1..1) where an affect is an **impulse**, never an assignment. Two timescales, both frame-rate-independent (`1 − exp(−dt·rate)`): `current` chases `target` at 1.35/s (τ≈0.74 s) while `target` decays to a temperament baseline at 0.018/s (half-life ≈38 s), with an intensity-scaled hold (6 + i·9 s) that suppresses decay so a fresh mood *sticks* before it fades. A 0.25 s dt clamp means a backgrounded tab cannot teleport her mood on refocus. NEW `AFFECT_VAD`: all 15 affects as distinct coordinates — the three that all collapse to the `tender` clip are now pairwise separated by >0.2 on an axis. Design adapted from soullink-emotion-sdk's `EmotionStateController` (MIT) — **ideas only, zero new dependencies**; see the initiative README for the measured, honest reasoning (the engine is ~29.9 KB gzip and *passes* Luna's 35 KB gate — it was rejected for output-shape mismatch with her layered pipeline, not for weight). Module is pure: zero pixi/live2d imports, so the same state can later drive voice pacing or proactive tone. +18 tests (rest-is-exactly-baseline, fast attack ≥90% in 2 s, slow release >40% of peak at 20 s, hold semantics, history-dependence, clamping, determinism, dt clamp). Suite 1500 green. Nothing on screen changes — nothing imports it yet. | _working tree_ |
| `v0.41.0` | 2026-07-30 | **The distribution layer is retired — the repo becomes readable reference engineering** — an owner decision, reasoned through rather than drifted into. The install funnel (unsigned binary → bring-your-own paid API key → a GB-scale voice runtime) filters out everyone except developers, and developers clone rather than install; so the whole cross-platform distribution effort was being paid for an audience that never wanted it. Meanwhile the portfolio value of this project lives in the *readable* repo (140+ Fact/Inference version entries, honest gap-recording, a typed wire contract, 1482 tests) and in the Luna TV showcase — neither of which needs a single download. **Removed** (distribution surface only): `.github/workflows/release.yml` (the whole tag→installer→Release pipeline), `pack:win`/`pack:mac`/`compile:server:win`, the electron-builder `win`/`nsis`/`dmg` target blocks, `scripts/app.ts --win`, the CI cross-compile step, both READMEs' download buttons + SmartScreen/Gatekeeper install prose, and the PRs-welcome badge. **Kept deliberately**: every win32 branch in the CODE (process tree-kill, `path.delimiter`, `.exe` naming, `Scripts\python.exe` venv layout) and the `windows-latest` CI test leg — that is cross-platform hygiene, unit-tested and zero-maintenance, and it reads *well* in a reference repo; ripping it out would be a large risky diff for no gain. Repositioned: the hero now states plainly that this is the engineering published as a reference and that Luna is one person's companion; Quick start → "Run your own" (build from source, the only path); Contributing → "Using this code" with **no promise of issues/PRs/platform support**, stated generously; a reading path into `ARCHITECTURE.md` → `DEVELOPMENT.md` added for the reference audience. Docs + build-config only — zero product-code changes; suite unchanged at **1482 green**. A 5-agent audit (23 remove / 15 reword / 57 keep) verified the retreat is complete and that `bun run app` — the owner's own build path — is byte-for-byte unaffected. | _working tree_ |
| `v0.40.0` | 2026-07-24 | **Windows voice runtime: the 8.19 GB 整合包 → the same scripted recipe as everyone else** — the one-click installer downloaded an 8.19 GB official 整合包 (a `.7z` bundling its own embedded Python, ffmpeg and precompiled C extensions), needed a system 7-Zip, and — because the `sha256` seam used `readFileSync`, which throws above 2 GiB — **hit that ceiling every time, was misread as a checksum mismatch, and DELETED the 8 GB download**: the Windows button was dead. It now runs the SAME recipe as macOS/Linux (~1.6 GB models + code, then a pip venv), plus two Windows-only manifest artifacts fetched from upstream to the user's machine: a python-build-standalone CPython 3.11 (`WIN_PY_DIR`) and a BtbN LGPL **shared** FFmpeg 7.1 (`WIN_FFMPEG_DIR` — torchcodec dlopens the libav* DLLs, so a static exe is useless). `jieba_fast` (sdist-only, no MSVC on a stock box) is replaced by a generated two-file pure-Python shim over `jieba` (verified against real jieba 0.42.1 for every GPT-SoVITS call site); `pyopenjtalk` is excluded by a PEP 508 marker (Japanese input falls to the browser voice via the v0.37.4 ladder). `sha256`/`copy` stream now; disk floor 25→12 GB; 7-Zip, `7zr.exe`, `fetch-7zr.ts` retired. A `recipe` field in provision.json (platform-aware: win32 demands the new recipe, POSIX honours the byte-identical old tree) migrates existing installs without wedging or needlessly re-provisioning mac/linux. +17 desktop tests; suite 1482 green; 4× tsc clean; win sidecar cross-compiles + NSIS packs from macOS. **真机验收 pending Alan's PC.** | _working tree_ |
...
### `v0.45.2` — 2026-08-08 — A word when the song changes (Initiative 32, 3/5)

Status:

- working tree — packaged, smoked, delivered to the Desktop

Fact:

- The initiative's #1 product risk (harassment: a song changes every ~3 minutes) is answered with
  layered discipline, all landed inside the ONE existing decision point (M6 — the detector
  registry died in v0.24.1; this is a new wake REASON in the tick flow, not a new trigger path):
  `maybeFireProactive` considers a music moment only when the global anti-spam rail already said
  yes, the ladder decided nothing THIS tick, and the effective phase is present (engaged /
  idle_watch) — dormant and sleeping stay asleep, nudged keeps its renudge backoff; a song change
  never cuts the escalation line.
- `proactive/musicMoment.ts` — `freshTrackMoment` is pure: a moment exists iff the CURRENT track
  changed ≤120s ago (`FRESH_WINDOW_MS` ≥ 2× the 60s tick — every change is evaluated once or
  twice, then expires; no backlog can fire late) and is still playing. Rapid skip-skip-skip
  leaves only the last track's `changedAtMs` in the window — debounce by construction, no queue.
- The sub-gate stacks ON TOP of the global rail, never replaces it: own cooldown (default 45min,
  `LUNA_MUSIC_PROACTIVE_COOLDOWN_MIN`) + own daily quota (default 3, `LUNA_MUSIC_PROACTIVE_DAILY`,
  local-day rollover on the cadence `dateKey` clock), persisted per-session in three new columns
  (migration `0020_music_moment.sql`), state shaped like `Cadence`. The commit mirrors the
  cadence split: cooldown stamps every music-initiated consideration, quota counts only spoken
  messages. A music fire consumes the GLOBAL account too — `commitProactive`/`commitProactiveSilent`
  layered on the phase commit; no exemptions (test-pinned).
- Expression rides the existing spontaneous framing via the v0.22.0 seed channel (README OQ3
  resolved: seed, no fourth `ProactiveIntent`): `musicSeedFor` hands her the track as a fact plus
  restraint ("light as a passing musing; it needs no reply... if nothing genuine comes, stay
  quiet"), reusing v0.45.1's `trackPhrase`. M3's fail-closed surface gate already covers the
  scary adjacent case: in this proactive turn she cannot touch `music_control` before speaking.
- `LUNA_MUSIC_PROACTIVE=0` kills only this surface; ambient context and tools are untouched.
- Tests 1861 → 1878 (+17). The plan's most important one, three ways: a valid fresh moment
  changes NOTHING when the global rail vetoes (cooldown / quota-exhausted / quiet-hours each
  asserted with zero provider requests). Plus the phase trio (dormant/sleeping/nudged), the
  sub-gates' independence (sub-cooldown blocks while global passes; sub-quota exhausts today,
  recovers tomorrow), both-accounts consumption on a spoken fire, seed content (track yes,
  artwork never), flag-off death, and rapid-skip at-most-one — the second tick after a burst is
  held by the sub-cooldown.
- One real bug caught while testing: `saveMusicMoment` originally used a bare UPDATE, silently
  no-oping for a session with no row yet — the INSERT fallback (mirroring `saveCadence`) fixed
  it, and the test order that exposed it stays in the suite.

Inference:

- The restraint architecture is the product: she comments on a song the way a person on the sofa
  does — occasionally, only when both of you are actually there, never twice in a row, and never
  by interrupting a silence she was already backing out of. Every one of those clauses is a
  mechanical gate with a test, not a hope about model behavior.
- Keeping the moment inside the single decision point preserved v0.24.1's hardest-won invariant
  (one wake path, one lock, one account) — the alternative (a change-event hook) would have
  reopened the double-fire class of bugs the redesign killed.

### `v0.45.1` — 2026-08-08 — She knows without asking (Initiative 32, 2/5)
...
- `packages/server/src/turn/nowPlayingContext.ts` — the weatherContext pattern verbatim:
  `musicAmbientEnabled()` is default-on riding the provider's own gate (`LUNA_MUSIC_AMBIENT=0`
  switches the block off while the tools stay mounted), `buildMusicBlock` is pure, synchronous,
  format-only over the resident provider's in-memory snapshot — the stream pushed the answer here
  before the question; a turn never pays a subprocess or a tool call to know what he's hearing.
- The builder's input is `MusicFacts` — title/artist/album/playing/positionS/durationS/sourceName,
  scalars only. That type IS the artwork ban: `artworkData` (tens of KB of base64 JPEG) cannot
  reach the prompt because no field exists to carry it — absence of a door, not runtime
  discipline. A test enumerates the keys; another asserts no base64/hash trace in the text.
- Progress is three coarse phases (`progressPhase`: start <30s or <15%, end <30s-left or >85%,
  middle between; null duration → no phase) so the same track in the same phase renders the same
  bytes — second-level precision means nothing in conversation and would churn the prompt every
  turn. Paused tracks say "stopped near the start/middle/end"; singles whose album repeats the
  title drop the album clause; no track → `musicBlockFor()` returns null and the prompt carries
  zero residue.
- `runTurn.ts` injects the block immediately after the weather block in the UNCACHED user tail,
  same try/catch-warn-omit shell — a builder failure skips the block, never the turn. The cache
  boundary is asserted: `buildSystemPrompt` is byte-identical across track changes and leaks no
  title (the volatile-never-cached invariant, M5).
- Proactive turns run through this same engine, so the block is already there when SHE initiates
  — the "listening together" baseline that v0.45.2's track-change moments will build on, landed
  here for zero extra code (M5). `trackPhrase` is exported as the wording seed v0.45.2 reuses.
- Tests 1845 → 1861 (+16): phase boundaries, playing/paused/singles wording, the firewall pair,
  the enable matrix (dormant provider wins over any flag), exactly-one-block injection, zero
  residue for idle/dormant/flag-off, cache boundary, and torn-state (stopped event) isolation.

Inference:

- This version is the initiative's thesis made real: presence is knowing without being told. The
  tool (v0.45.0) answers questions; the block removes the need to ask — and because the provider
  pushes, the marginal cost of her always knowing is literally zero network, zero subprocess,
  zero tokens beyond one short line.
- The type-level artwork firewall continues the initiative's "make the failure mode impossible,
  not discouraged" discipline (v0.45.0's unmarked control tool, same idea): the expensive
  mistake — a 40KB base64 blob in every prompt — is unrepresentable, not merely avoided.

### `v0.45.0` — 2026-08-08 — She can hear the room (Initiative 32, 1/5)
...
- Initiative 32 (Listening Together) opens on the locked architecture: no NetEase API wrapping —
  the owner's own client plays, Luna observes from outside via macOS Now Playing (MediaRemote,
  `brew install media-control`), which also buys Apple Music / Spotify / browser compatibility for
  free. The pre-built CLI (`~/Desktop/luna-music-cli`, live-verified 2026-08-07 against NeteaseMusic
  3.1.8.3368) is vendored as `packages/music-cli` — the CI hard constraint (M9): sibling-directory
  `file:` deps explode on runners, so the repo must self-contain. Its 16 tests ride along
  (captured-payload-based, platform-independent, both CI legs run them); imports de-extensioned to
  house style; one-line provenance headers; the `luna-music` CLI entry kept for standalone debugging.
- `packages/server/src/tools/media/nowPlaying.ts` — the resident provider, patterned on the weather
  snapshot: ONE long-lived `watch()` subscription (a single `media-control stream --no-diff` child;
  MediaRemote pushes, so idle cost is zero) maintains `{ track, playing, changedAtMs }` in memory;
  `getNowPlaying()` is the synchronous shared read (null = dormant, distinct from active-but-idle).
  Dormancy trio: `LUNA_MUSIC=1` && darwin && `doctor()` finds the binary — any gate failing is one
  log line and an untouched boot. Stream deaths get bounded exponential-backoff restarts (≤5, cap
  60s, a live event refunds the budget), then dormancy with one warn — never a crash-loop. Both
  shutdown paths call `stopNowPlaying()` so the child cannot orphan.
- `packages/server/src/tools/builtin/music.ts` — the read/control pair, split because risk is
  per-tool: `music_now` (safe-parallel, `proactiveRisk: 'safe'`) answers state/title/artist/album/
  extrapolated position (the CLI's `positionAt` — current without a subprocess); `music_control`
  (`op` zod whitelist play/pause/toggle/next/prev, global-serial, confirms by `send()`'s ~250ms
  read-back) is DELIBERATELY unmarked → the fail-closed gate resolves 'surface': in a proactive
  turn she must speak before touching his playback. Both return recoverable errs when the provider
  is dormant. Descriptions steer her off per-turn polling (the ambient block lands in v0.45.1).
- Wire contract: `music_now` + `music_control` added to the protocol `ToolName` enum. Mounting:
  `withMusic(base, platform)` in `registry.ts` chained in `main.ts` — same boot-frozen instant as
  `withWeather`; the mount gate is the sync flag+platform pair, the async doctor gates only the
  subscription. `LUNA_MUSIC` documented in the desktop ENV_TEMPLATE (a stray backtick in that
  template literal briefly broke the desktop build — caught by tsc, worth remembering).
- Tests 1807 → 1845 (+16 vendored, +22 new): the risk-mark tripwire (the plan's most important
  assertion — a future 'safe' on music_control would let her silently skip songs), mount gating
  incl. the win32 leg CI actually exercises, all three dormancy paths (zero subscriptions, boot
  untouched), restart-budget exhaustion into dormancy, dormant-tool recoverable errs, live-provider
  snapshots through the declared zod shapes. Five packages tsc clean (music-cli joins the wall).
- Real-machine lifecycle, both ways: `LUNA_MUSIC=1` boot → `[music] now-playing observation
  started` + exactly one stream child, SIGTERM → child cleanly gone; flag unset → zero music log
  lines, zero media-control processes. `doctor()` live: binary found, no player reporting (NetEase
  closed) — the idle path answers honestly.

Inference:

- The reversal that makes this initiative buildable at all: the community NetEase API ecosystem is
  dead (its cornerstone archived 2024), so Luna does not speak NetEase — she watches the same
  system surface every player publishes. Zero reverse-engineering to rot, zero credentials in
  code, and the owner's paid client stays the thing that plays.
- The provider is the whole product shape: one push subscription, everything downstream reads
  memory. v0.45.1's ambient block, v0.45.2's track-change moment (`changedAtMs` is recorded but
  unconsumed today, exactly as planned), and v0.45.4's player card all become synchronous
  consumers of state that is already warm.
- The risk split is the companion contract in miniature: knowing what he's listening to is
  presence ('safe', she may glance silently); acting on his player is agency over his space
  ('surface', she must speak first). Encoding that as the fail-closed default rather than a
  convention makes the creepy failure mode a type-level impossibility.

### `v0.44.8` — 2026-08-02 — No seams in her room (owner correction)
...
- The owner's correction, from the packaged menu: "你在菜单页面设置了一块模型区，导致左半边模型被明显的切断了。
  不要设置这种明显的断层处理" — the menu-mode canvas was still the session layout's right-hand slot
  (the chat panel keeps its flex slot at `visibility: hidden`, the v0.44.0 D4 mechanism), so a model
  zoomed past the slot width was clipped at the slot's invisible left edge: a hard vertical seam
  through her sleeve, standing naked in the middle of the open menu room. In session view that same
  edge sits flush against the chat panel and reads as intentional cropping — the menu has no panel
  to justify it.
- The fix has two halves that must hold together. `.menu-wide .model-stage { position: absolute;
  inset: 0 }` spans the canvas across the whole stage while the menu is up — she extends naturally
  and only the WINDOW edge crops her, like any full-bleed art. And the sink gains a frame:
  `setFrame(get)` takes a rect getter, `fit()` centres her inside `[left, left+width]` instead of
  the canvas (`framedBaseX`), so her BASE POSITION stays exactly where the session slot would put
  her. Talk removes the class inside `swapToChat`'s existing 1000ms cleanup — the chat panel is
  already opaque over that edge, so the re-clip is invisible and she never moves.
- The frame is a RECT, not a left inset — the first cut of this fix framed `[panelRight+gap,
  canvasRight]` and she shifted 11px at Talk, because the session slot also stops 22px short of the
  window (the stage's right padding) and centring splits the difference. `app.ts`'s `slotFrame()`
  measures both edges of the real slot (panel right + gap, stage right − padding) live on every
  fit, and degrades to the plain frame in collapsed/column layouts, where the canvas is already
  full-width and there is no slot edge to mimic. Wide mode is gated on `modelState === 'ok'` — the
  empty-state card keeps its normal slot.
- Drag clamps reason about the frame too (`frameClampWidth`, both the pointermove clamp and the
  v0.25.2 heal): a drag that used the whole wide canvas could otherwise strand her outside the slot
  she returns to at Talk.
- The owner's second note, same session: "那个zzz的睡眠特效可以放大一些现在太小了" — the zzz glyphs
  doubled (15/19/24 → 26/33/42px) with drift travel and headroom scaled to match and a soft
  text-shadow for separation; the 3.2s ambient clock is untouched, so the motion audit still passes
  by the same named exemption.
- Measured, not asserted: with a throwaway sidecar on 8901 (`?dev&ws=8901`, scratch DB) her screen
  centre moved **0.16px** menu→Talk and 0.16px Talk→menu — integer `clientWidth` rounding, D4 to
  the sub-pixel. Verified live at zoom 1.8 (the owner's sliced case: sleeve now extends free, no
  seam) and 1.25 (zzz beside her hat, fully in frame). One false alarm chased to ground: a "frozen
  model + stale head anchor" in the test rig turned out to be the hidden browser pane throttling
  rAF (`visibilityState: "hidden"` pauses the model's update loop) — foreground apps are unaffected,
  and the layout self-healed on refront through the live-measuring getter.
- `frame.test.ts` (5 tests) pins the math: plain frame ≡ the old centring expression bit-for-bit
  (every non-lobby surface unchanged), slot frame ≡ the slot-sized canvas position (D4 by
  construction), the 22px-padding case by name, and degenerate frames clamp. Suite 1807 green,
  four packages tsc clean.

Lesson:

- A layout that reads as "cropping" in one composition reads as "amputation" in another. The slot
  edge was always there; only the chat panel made it look deliberate. When a surface is reused
  under a new composition, audit what its edges LOOK like, not just where they are.
- Frame ≠ canvas. The first cut derived the frame's right edge from the canvas and shifted her 11px
  — the session slot's geometry has TWO owner-visible edges, and mimicking a position means
  measuring both, live, from the layout that defines them.

### `v0.44.7` — 2026-08-02 — The close-out, and CI turns green (Initiative 31, 8/8)
...
- **CI turned green** (the owner's named addition). Every push since 7/31 was red and every red
  emailed him; all failures were environmental, and the root cause was this skill's own stale line
  "CI no longer runs" — corrected to: CI runs on every push (ubuntu + windows), red mails the owner,
  local green is not the finish line. Fixes: the three model-file tests (`COSTUME`/`OVERLAYS`/
  `MODEL_ASSETS` vs the real `yumi.cdi3.json`) now `test.skipIf` when the gitignored model is
  absent — NO fixture, because these tests exist to check the REAL installed model and a fake one
  tests nothing; and `browserVoiceRetired`'s `rel()` normalises Windows backslashes — the Windows
  leg CAUGHT the guard's own portability bug, which is exactly why that leg stays.
- D10 becomes an AUDIT (`motionAudit.test.ts`): every duration in the initiative's CSS must belong
  to a declared band (micro 0.2–0.9s phases, stage 1.0–1.8s composed totals, two named ambient
  loops), the JS orchestration constants must compose into the stage band from micro-band phases,
  and strays are listed by value. Passed first run — the discipline had held.
- `prefers-reduced-motion` honored for UI stage transitions (0.2s fades, destinations reachable,
  ambient loops stopped, spring hover → color-only). This **amends the v0.36.0 constitution
  deliberately**: the ban on the MANUAL reduce-motion setting stands (that machinery stays dead,
  still tested), but the OS accessibility preference is a different thing — and the amended test
  now also asserts the media block never reaches her engine-level aliveness selectors. Her idle,
  breath and wake have no off-switch, exactly as v0.36.0 decided.
- README OQ3 lands: the menu shows `LUNA_VERSION` (`version.ts`, bumped at close-out) bottom-left,
  muted 11px. No memory counts — the menu is a front door, not a dashboard.
- `ARCHITECTURE.md` gains **The front door**: the two-half boot, cold-by-construction, wake-in-
  place, the data-surface pages, and the two-band motion rule. Initiative README + master roadmap
  closed out. The old settings panel and the `luna:menu='0'` escape hatch stay for an observation
  week (the conservative default); their retirement is the owner's call.
- +5 tests (motion audit ×3, the amended constitution pair reshaped); the three model-file tests
  skip loudly off-machine.
- Suite **1802 pass / 0 fail**; all four packages tsc clean; **CI run on this push: success** —
  the first green since 7/31.

Inference:

- The CI story is the version's real lesson: a stale sentence in a SKILL caused a month of failure
  mail, because the operating model said the mail could not exist. The fix is one line of prose,
  one `skipIf`, one `replaceAll` — the expensive part was believing the wrong thing confidently.
- The v0.36.0 amendment is the right kind of constitutional change: the original decision's INTENT
  (no user-facing switch that guts her aliveness) survives verbatim and is still tested; what
  changed is recognising that an OS accessibility contract is not that switch.
- Initiative 31 closes with her front door built: cold lobby, wake-in-place, her diary and skills
  readable without waking her, settings that name her parts, and her core editable behind a diff.
  Eight versions, every one packaged, smoked and delivered same-day.

### `v0.44.6` — 2026-08-02 — Modules and the persona editor (Initiative 31, 7/8)
...
- NEW `packages/web/src/ui/modulesConfig.ts`: four uniform module cards — Chat LLM / Embedding /
  Web search / Weather — each a DECLARATION (fields + which probe), so a future module is one more
  entry, not a new UI. Values load through the wizard's `wizardPrefill` bridge (secrets arrive as
  NAMES, never values → rendered as dots); probes are the wizard's own vendor-hinted probes; a
  probe failure never blocks a save (it is advice, not a gate). Secret inputs are masked at rest
  (`sk-gt6U…wxyz`), plaintext only while focused.
- Save rides a NEW `luna:save-config` IPC: whitelist-filtered (`filterWizardFields`) →
  **`luna.env.bak` written first** → `mergeEnvFile` (per-field merge, never a rewrite). No sidecar
  restart, no window swap — **README OQ2 lands here: saved fields take effect after restart**, the
  card says so, and a NEW `lunaPet.relaunch()` (`app.relaunch()+quit()`) makes the restart one
  click. `LUNA_MAX_TOKENS` joins the wizard whitelist for the chat card. A plain browser (no
  bridge) renders Modules read-only and says to edit on the desktop.
- NEW `packages/web/src/ui/personaEditor.ts`: the FIXED core in a plain-text editor over
  `/api/data/soul` + `POST /api/data/soul/fixed` — the v0.31.0 owner capability behind a product
  door. A line-diff **preview is part of the save** (not an optional courtesy), empty text is
  refused client-side AND server-side, and the EVOLVING half renders read-only — she grew it, the
  owner reads it, the workspace reseed stays the advanced exit. **Her `self-edit` tool still cannot
  touch the fixed core; those firewall tests run unchanged in this suite.**
- `settingsPage.ts`: the five categories become seven — **Persona** (it is about HER, not widgets)
  and **Modules** (four cards would have buried System) get their own rail entries.
- +10 tests: the mask shapes (incl. 未配置), changed-fields discipline (blank never clobbers,
  out-of-card keys ignored), per-card probe payloads (weather's per-account host included), every
  secret field declared, the diff classifier, and the no-op-save refusal.
- Suite **1799 pass / 0 fail** (was 1789); all four packages tsc clean.
- Verified live against a copy of her real DB: her 4908-char fixed core loads, a test line previews
  as `+` and saves through the audited store (13 `soul_audit` rows in the copy), **the real
  `luna.sqlite` untouched** (tail compared byte-for-byte); evolving renders read-only; the
  browser's Modules section degrades to the read-only note.

Inference:

- The version's own description held: new doors on old machinery. Every risky primitive here —
  whitelisted env merge, vendor probes, the audited soul store — had already survived the wizard or
  the workspace; what shipped is reach, not mechanism.
- The restart-over-hot-swap decision is the same shape as v0.43.14's silence-over-fallback: a
  single-machine companion does not need the failure modes of a hot-reconfigurable provider, and
  "restart Luna" is one honest click.
- The persona wall is now three-layered and each layer is tested where it lives: the LLM's tool
  cannot touch fixed (tool layer), the editor cannot blank it (client + server), and every write is
  audited (store layer).

### `v0.44.5` — 2026-08-02 — Settings, by which part of her (Initiative 31, 6/8)
...
- NEW `packages/web/src/ui/settingsPage.ts`: five categories — Voice / Expression & Motion /
  Appearance / Behaviour / System — entered from the menu as a full page (transition I), replacing
  the three-tab slide-in panel for lobby boots. `SETTINGS_IA` is the reconciliation artifact: every
  localStorage-backed control mapped to exactly one category.
- **The migration is an ADOPTION, not a rebuild**: the page moves the old panel's live DOM rows
  (the same `<label>` elements, listeners attached at boot and all) into the new categories. The
  live-read contracts physically cannot drift because they are the same nodes. The server-registry
  card and the costume card are adopted whole; their internals are untouched.
- Voice gains a read-only health line (`/api/tts/health` → 在跑/启动中/没有在跑). Behaviour is a
  deliberate placeholder — proactive knobs live in env/DB pins today, and surfacing them is its own
  decision, not a side effect of moving furniture. System holds the adopted server card + the
  workbench entry.
- Lobby boots retire the ⚙ button entirely (`display:none`) — Settings lives behind the menu, one
  hierarchy, no back doors; the panel DOM stays as the donor the page adopts from. **Esc in chat
  returns to the menu** (guarded so it never fights the old panel's Esc-close in bypass boots).
  `luna:menu='0'` keeps the old panel byte-identical — two systems coexist until the initiative
  closes.
- `mainMenu.ts`: `settings` becomes a real page id through the same `pageBody` hook; the
  `openSettings` dep is gone.
- +6 tests: category order, no-key-twice, the exact eleven-key inventory written out as the
  migration checklist, every `PERF_FLAGS` key having exactly one home (the IA cannot drift from the
  engine's own flag list without going red), placement of the idle selector and the costume store,
  and gaze-follow living under Appearance (her eyes in the room, not a performance).
- Suite **1789 pass / 0 fail** (was 1783); all four packages tsc clean.
- Verified live: five categories render; Voice 1 row + health, Expression 7, Appearance 8 rows +
  the costume card, Behaviour the note, System the (WS-empty) server card; toggling the adopted
  Mood memory row writes `luna:affect` exactly as before; the ⚙ is gone in lobby mode.

Inference:

- The adoption strategy is the whole review: a rebuild would have re-implemented eleven controls'
  semantics and prayed the copies matched; moving the elements themselves makes "did the semantics
  survive" a non-question, and shrinks the risk surface to placement — which is what the
  reconciliation test pins.
- 'By which part of her' is a better index than 'by widget type' precisely because the owner's
  question is never "where are the checkboxes" — it is "she is too still, where do I look".

### `v0.44.4` — 2026-08-02 — The skills page: what she learned (Initiative 31, 5/8)
...
- NEW `packages/web/src/ui/skillsPage.ts` (~120 lines): `groupSkills` (alive by recency of use,
  retired folded below by retirement date), `selfTaught` ('saved' and 'dream' are HERS — whether
  saved mid-conversation or distilled by a night's consolidation; everything else was installed),
  `relativeTime` (刚刚/分钟/小时/天 + 「还没用过」 for never, clock-skew safe), and the card mount.
- Cards: name + one-line description + use count + relative last-use + the self-taught mark
  (✦ + a lavender edge — **which ones she taught herself is the point of this page**). Click
  expands the `body` in place (her own operating manual, rendered as plain text — no markdown
  dependency until reading it plain actually hurts); one card open at a time; retired skills grey
  in a folded section. Empty library: 「她还没攒下自己的技能。」; fetch failure: a reachable-state
  message, never a white page.
- Wired through the same `pageBody` hook the diary book rides; the menu shell is untouched.
- +6 tests: shelf ordering both shelves, empty grouping, the self-taught boundary, the three
  relative-time bands + never-used + negative-age clamping.
- Suite **1783 pass / 0 fail** (was 1777); all four packages tsc clean.
- Verified live against a copy of her real DB: `✦ discover-latest-build`(自学, 8 uses, 9h ago),
  `self-architecture` (owner-installed, 5 uses, its 6.7KB body expands readable),
  `search-then-fetch` (2 uses, 18 days); exclusive expansion holds.

Inference:

- M9 made this page cheap: the lifecycle columns (`used_count`, `last_used_ms`, `source`,
  `deprecated_ms`) were added for the skill shelf's own bookkeeping, and they turn out to BE the
  growth record — the page is mostly a projection.
- The self-taught mark carries the page's meaning. Three skills is not a library; "one of these
  three she built herself out of a dream" is a companion growing.

### `v0.44.3` — 2026-08-02 — The diary book: what she writes, and what she digests (Initiative 31, 4/8)
...
- NEW `packages/web/src/ui/diaryBook.ts` (~380 lines), split workbench-style: a pure data half
  (`localDayKey`, `buildBookIndex`, `monthGrid`, `nextLitDay`, `translateStep`/`dreamNarrative`,
  `createTurnQueue`, `pageHeading`) the tests drive, and `mountDiaryBook` assembling the DOM.
- The two-page spread (D6): calendar left — **only days that actually hold something light up**
  (M8's skip-day distribution decides the set), purple dot = diary, crescent = a dream that night,
  both = both; month navigation; per-month and total counts. Content right with the date heading
  and paper-scroll body. Keyboard: arrows travel LIT days only, Enter turns.
- Dreams share the book (D7): a `Diary | Dream` switch appears only on days that have both. The
  **translation layer** (M10) renders `steps[]` as what she did that night — counts extracted from
  the real detail strings (`rated 9 turns` → 回看了 9 个瞬间), pipeline timings never surface, an
  unknown step renders raw instead of crashing, and aborted/empty dreams read as one italic line:
  「这个梦断掉了。」 Week/month digests stay off the calendar — they have no single day to sit on.
- The page turn (E): the old face swings `rotateY(-172°)` around the spine over the new content,
  1.6s with an accelerating curve and a paper-bend shadow; same-day face switches use a lighter
  half turn (−90° swap, 1.0s). Compositor-only (`transform`/`opacity`), `will-change` mounted only
  during the turn. Clicks mid-turn queue with **latest-wins** (the reader wants where they pointed
  last, not a replay of every misclick). Narrow windows collapse to a single stacked page.
- Data: one fetch each of `/api/data/diaries` + `/api/data/dreams` on entry, 300ms skeleton,
  failure renders a "取不到" state instead of a white page. **Zero WS anywhere in the book** —
  reading her diary does not wake her, by architecture (the book is HTTP, the session is WS).
- Wired through v0.44.0's `pageBody` hook — the menu shell needed no change.
- +16 tests: the skip-day lit set, digest exclusion, local-day dream mapping, dual marks,
  July-2026 grid geometry, lit-only arrow travel with clamping, the full translation table against
  real detail strings, the timing-never-surfaces rule, broken-dream semantics, the latest-wins turn
  queue, and date formatting.
- Suite **1777 pass / 0 fail** (was 1761); all four packages tsc clean.
- Verified live against a copy of her real DB: July 2026 shows 19 lit days with dots and crescents;
  her actual 7/31 entry turns in ("Alan told me today I'm closed source now…", 628 chars); the
  Dream face shows that night's TWO dreams — one broken in italics, one fully translated; the
  status badge stayed untouched (zero WS) throughout.

Inference:

- This is the initiative's showpiece: 28 diaries and 59 dream reports that were pure database rows
  become a book she could plausibly own. The design honesty is in what it refuses — no editing
  (it is HER writing), no feelings in the dream lines (stating actions and counts only; anything
  more would be writing her diary for her), and the raw pipeline stays in the workbench/trace where
  it belongs.
- "Reading her diary must not wake her" cost nothing to implement — it fell out of v0.44.2's
  architectural split. The book never touches the socket because there is no socket to touch.
- The translation table's wording is a first draft awaiting the owner's review (README OQ1); the
  TESTS pin the contract (counts, actions, degradation), not the prose, so a wording pass will not
  go red.

### `v0.44.2` — 2026-08-02 — Her memory becomes something you can fetch (Initiative 31, 3/8)
...
- NEW `packages/protocol/src/data.ts`: zod schemas for the read-only data surface — `DiaryEntry`,
  `SkillRecord`, `DreamStep`/`DreamRecord` (steps flattened, `aborted` explicit), `SoulData`
  (deliberately WITHOUT the workspace's `writable` dev-flag). An HTTP face, separate from the WS
  contract; one schema, both ends.
- NEW `packages/server/src/data/dataApi.ts`: `GET /api/data/{diaries,skills,dreams,soul}` +
  `POST /api/data/soul/fixed`. Mounted in `main.ts` **before** the `viewerEnabled` gate (M2 —
  product, not debugging); the loopback bind is the security boundary, same as the WS (S1). Every
  payload is schema-parsed on the way out. `report_json` is flattened server-side — the client
  never touches the raw string, and a malformed row degrades to an aborted-empty record instead of
  taking the list down. `?limit=` exists from day one; her scale (28/3/59) needs no paging today.
- Soul productized (M3): the POST rides the existing audited `updateFixedCore`; the `/_workspace`
  routes stay as aliases, byte-identical. **Luna's own `self-edit` tool still cannot touch the
  fixed core — that firewall lives in the tool layer and this route does not go near it.**
- Desktop (M11): `serve.ts` forwards `/api/data/*` with the `/api/tts` discipline — the upstream
  target is chosen from an exact allowlist (`DATA_ROUTES`, method included), never from the request
  path, so traversal matches nothing. `main.ts` threads the sidecar port in; `dev-server.ts` gains
  the equivalent proxy for browser dev. The packaged smoke now also asserts
  `fetch('/api/data/diaries') === 200` through the real forward.
- +15 tests: endpoint shapes/order/empty-library behaviour, the limit clamp, all three shipped
  `report_json` forms plus the malformed row, soul GET/POST round-trip and the empty-write reject,
  fall-through vs 404 ownership of the prefix, and the forward planner's allowlist (traversal,
  method mismatch, unconfigured port).
- Suite **1761 pass / 0 fail** (was 1746); all four packages tsc clean.
- Verified against a copy of her real DB on a throwaway port: **28 diaries, 3 skills
  (`discover-latest-build`, `search-then-fetch`, `self-architecture`), 59 dreams (20 aborted),
  soul 4908+693 chars**; the workspace alias answers 200; `/api/data/../secret` normalizes away
  from the surface and dies at the WS 426.

Inference:

- This is the initiative's only new server surface, and the reason it is its own version: the shape
  had to be decided once, correctly — product-gated (not dev-gated), loopback-bounded, allowlist-
  forwarded, schema-guarded at both ends. Every later version (the diary book, the skills page, the
  persona editor) is a CONSUMER of what landed here.
- The diary book's "reading her diary must not wake her" property (v0.44.3) falls out of the
  architecture for free: this surface is HTTP, the session is WS, and the menu keeps the WS cold.
- Flattening `report_json` server-side is a small decision with a long tail: the client never
  learns the storage format, so the dream cycle can evolve its report shape without a web release —
  only the flattener moves.

### `v0.44.1` — 2026-08-02 — Talk: she wakes for you (Initiative 31, 2/8)
...
- NEW `packages/web/src/wakeSequence.ts`: the wake as DATA — four beats over existing layers
  (`setState('neutral')` at 0 (leaving `sleeping` IS the eyelid handback — her first blink is the
  blink awake), `headLiftAlert` at 550ms, a brow pulse at 950ms, a light `tender` clip at 1300ms)
  plus the one-beat sleep sequence, run by an injectable scheduler. Zero new engine mechanism —
  this file is choreography, and the direct dividend of v0.43.x. The model's two motion3 files were
  checked and rejected (M7): a wave and a cry, both looping.
- NEW `packages/web/src/returnGate.ts`: the polite disconnect. ← Menu mid-turn waits for
  `turn.result` — and for `proactive.finished`, because a proactive turn deliberately emits no
  `turn.started` (v0.33.2) and tracking only the reactive pair would cut her off exactly when she
  chose to speak unprompted. An `error` frame settles the flight too, so a dead turn cannot wedge
  the return forever.
- `app.ts`: Talk = the menu→chat swap (text column fades left 0.5s, chat reveals 0.6s at 0.3s —
  she never moves), `runSequence(WAKE_STEPS)`, `activateSession()`, and a fire-and-forget TTS warm.
  **The wake gate is the wake itself**: no overlay — an overlay would hide the very animation that
  replaces it; the input is usable throughout (the v0.35.6 no-strand rule), and a first line the
  voice cannot carry is skipped under v0.43.14 semantics.
- ← Menu (a header button, lobby boots only): `returnGate.request` → close the socket, remount the
  menu, one-beat sleep. `activateSession` is now re-entrant — the same `LunaWsClient` reconnects
  (`close()` sets the deliberate latch, `connect()` clears it), `wsLive` is the double-activate belt.
- Dream: activate + `dream.enter` (the outbox buffers it until open) with **no wake sequence** — a
  sleeper does not wake to fall asleep. The wake she skipped plays when the dream ENDS
  (`dream.status` false + a pending flag).
- The packaged smoke's phase 2 now **clicks Talk** instead of reloading with `?menu=0`: `wsStatus
  'open'` now proves the real activation path end to end, and every pre-existing assertion runs
  after the wake.
- +12 tests: the four wake beats in order at the approved offsets (injected scheduler), the
  synchronous first beat, cancel semantics, optional-method tolerance, sleep-vs-wake asymmetry, and
  the full polite-disconnect truth table (idle/mid-turn/proactive/error/re-request).
- Suite **1746 pass / 0 fail** (was 1734); all four packages tsc clean.
- Verified live: Talk from cold (badge null) → talk-leaving + waking + zzz gone on the click frame →
  menu DOM gone, chat visible, WS attempted; ← Menu → menu + zzz back, chat hidden; Talk again →
  the SAME client reconnects. (Dead-port run — the badge honestly says `closed`, the plan's
  "醒来发现信号不好" case.)

Inference:

- Talk is the version's thesis in one interaction: she is not ON a page, so entering conversation
  is not a navigation — it is her waking up where she already stood. The whole wake is composed
  from v0.43.x surfaces (`playAction`, `pulse`, `triggerEmotion`, the state machine), which is the
  payoff of building those as generic layers rather than one-off effects.
- The gate inversion matters beyond cosmetics: a boot gate blocks EVERYTHING to hide a cold start;
  the wake absorbs the same cold start inside an animation that was going to play anyway. Same
  wait, zero blocked seconds.
- The disconnect gate is deliberately not a state machine — it is three booleans over events the
  wire already emits. The subtle half is the proactive pair; missing it would make ← Menu rudest
  precisely in her most personal moments.

### `v0.44.0` — 2026-08-02 — The cold lobby (Initiative 31, 1/8)
...
- `app.ts` boot is now two halves. The **lobby half** runs immediately: layout, the Live2D sink
  (model loads as always), `setState('sleeping')`, the menu DOM. The **session half** —
  `WebAudioSink`, the speech gate, the controller, `new LunaWsClient`, `connect()`,
  `requestGeolocation` — moved into `activateSession()`, called at the exact position in the boot
  sequence where the client used to be constructed, so direct-boot ordering is byte-identical.
  In menu mode none of it runs until Talk (v0.44.1). The boot gate is skipped in lobby mode —
  warming belongs to the wake, not to a menu that makes no sound.
- `wsClient` is the null-until-activated seam. The three lobby-attached listeners that reach the
  socket (send, the two dream buttons, the desktop geo push) guard on it; the geo callback still
  CACHES every fix so `onStatus 'open'` re-sends the newest on connect.
- NEW `packages/web/src/menuMode.ts`: `menuEnabled` — pet / agent-only / `luna:menu='0'` /
  `?menu=0` all boot direct (M12); `?setup` and `?workbench` never reach the decision.
- NEW `packages/web/src/ui/mainMenu.ts` (~230 lines): `menuItems` (Talk/Diary/Skills/Dream primary,
  Settings/Quit secondary — Quit only when the desktop bridge exists), `nextFocusIndex` (arrow keys
  cycle ENABLED items), `springLinear` (a damped spring sampled into CSS `linear()`, k=190 c=11 →
  ~25% overshoot, D2), `mountMainMenu` (DOM + keyboard + transition I + zzz anchored on the
  `--luna-head-x/y` vars the speech bubbles already ride).
- Transition I (D3): menu out 600ms (scale 1.16 + fade), a beat, page up 900ms on the pop ease —
  1.6s total, orchestrated, with her fading in sync. Diary/Skills open placeholder shells;
  Settings opens the existing panel over the menu; Dream renders `aria-disabled`; Talk is a
  deliberate no-op until v0.44.1. `← Menu` reverses everything and re-asserts `sleeping`.
- The menu overlays the chat panel's own grid slot (`visibility: hidden`, space kept) — the D4
  mechanism: she is already standing exactly where the session layout puts her, so Talk can wake
  her without moving her one pixel.
- Desktop: `lunaPet.quit()` (preload + `ipcMain 'luna:quit'` → `app.quit()`) — M6 measured the
  bridge had no quit. The packaged smoke is now **two-phase**: phase 1 asserts the menu (six items,
  Quit present, zzz over a mounted canvas, status badge dataset UNSET = the WS was never
  constructed), then reloads with `?menu=0` and runs every pre-existing assertion unchanged,
  `wsStatus === 'open'` included.
- +16 tests (`menuMode.test.ts`, `mainMenu.test.ts`): the bypass truth table, item order and
  tiering, Quit presence, disabled-skip focus cycling, spring endpoints and overshoot bounds
  (over- vs under-damped), and the 1.6s two-phase orchestration constants.
- Suite **1734 pass / 0 fail** (was 1718); all four packages tsc clean.
- Verified live on the real renderer: menu mode (5 items in a browser — no Quit), Dream grey, zzz
  floating, `status-badge` dataset null (zero WS), chat hidden; Diary transition out and back with
  every class restored; `?menu=0` boots direct with the WS attempted (badge `closed` against a dead
  port — exactly today's behaviour).

Inference:

- Opening the app no longer drops the owner mid-conversation — and the deeper change is that the
  BACKEND is no longer a party to opening the app at all. No WS session means no proactive
  machinery, no presence signal, nothing: the menu is genuinely cold, by construction rather than
  by suppression (M1 — everything proactive rides the socket).
- The surgical risk in this version was boot-order drift for the existing paths, and the defence
  was to keep `activateSession()` AT the old construction site and call it synchronously for every
  direct boot. The diff moves code into a closure; it does not reorder it.
- The menu living in the chat panel's slot is the load-bearing layout decision (D4). Wake must not
  be a page transition — she is the one thing on screen that persists across every mode, which is
  the initiative's whole thesis about her not belonging to any page.

### `v0.43.15` — 2026-08-01 — Intensity stops being a volume knob (Initiative 30, 15/15)
...
- `faceVm.ts` `blendedState`: `out[k] = lerp(base, raw, pb.intensity)` → **`out[k] = raw`**.
  `overlayParams`: `base * w * pb.intensity` → **`base * w`** (`w`, the intro/outro fade, stays —
  it is direction, not strength).
- **`Playback.intensity` and `pending.intensity` are deleted.** With both readers gone they were
  stored-and-never-read, which is exactly how `physicsPassthrough` sat dead for six versions. The
  removal cascaded through `triggerEmotion(id)`, `previewPose(def)`, `Live2DSink.triggerEmotion`,
  `currentPlayback()` and the workbench readout — tsc found every call site.
- **Unchanged on purpose, each now commented against a future "unification"**: `affectToEmotion`
  still reads the RAW wire value (so *omitted never escalates* holds), and `affect.nudge` still
  scales the VAD impulse and its hold — how long a mood lingers is a real judgement, it accumulates
  across messages, and a small value there is brief rather than invisible.
- `message.ts`: the `emotion` describe rewritten. It said `0 = barely visible, 1 = full`, which
  became a lie the moment this shipped; it now says the field picks **which version** of the
  expression, that both play at full strength, and that it also sets how long the mood lingers.
- Workbench copy follows the semantics: the slider reads `Affect intensity 0.70 · escalated variant`,
  and the clip section says *always at full amplitude*.
- Measured after the change — `bright_delight` peaks, identical at every input **including omitted**:

  | emotion | mouthPucker | cheekPuff | 爱心眼 | browLY |
  |---|---|---|---|---|
  | 0.3 / 0.5 / 1.0 / omitted | −0.498 | 0.992 | **1.000** | −1.000 |

  Against the before-table in the plan (0.3 → −0.147 / 0.290 / **0.300** / −0.391).
- **Deleted, not loosened** — both asserted precisely the behaviour this version removes, and both
  are named here as the plan requires:
  - `faceVm.test.ts` `'emotion intensity scales expression strength'`
  - `faceVm.test.ts` `'overlay strength follows the affect intensity, like the pose already did'`
- +7 tests: frame-**identical** peaks at 0.3/0.5/1.0 (`toBe`, no tolerance); a visibility floor
  (heart eyes and cheekPuff > 0.9 at *any* legal intensity, omitted included); the branch still
  reaching `annoyed` / `skeptical` / `awkwardV2` at 0.3 **and** their escalated forms at 0.9; a clip
  triggered by name playing full; the VAD impulse still scaling; the `emotion` describe asserted as
  an ABSENCE (no `barely visible`, no `0 = `); and the widened clamp limit below.
- Suite **1718 pass / 0 fail** (1712 − 2 + 7 + 1); all four packages tsc clean.

Inference:

- The owner's observation was that the model plays it safe, and measurement agreed: at `emotion 0.3`
  a `bright_delight` moved `mouthPucker` by 0.147 while `defaultIdleV1` alone swings `mouthForm` by
  ±0.22 — **the expression was smaller than her breathing**. This is v0.42.4's lesson recurring
  (measurable ≠ visible), except the culprit this time was not a coefficient but a value the model
  chose.
- **The version that was NOT shipped is the interesting one.** Locking the wire value to 1 — the
  first instinct — would have forced the ≥0.7 branch permanently high, making `annoyed`, `skeptical`
  and `awkwardV2` unreachable and taking the swirl-eyes overlay (which hangs only on `awkwardV2`)
  with them: 14 reachable clips back down to 11, undoing half of v0.43.2, and a character who answers
  every irritation with maximum sulk. Amplitude and branch had to split, and `emotion`'s meaning
  narrows from *how loud* to **which of the two performances**.
- **A widened KNOWN LIMIT, recorded rather than discovered later.** While intensity scaled amplitude,
  a model-typical 0.3 kept the most extreme brows well inside their clamp, so they breathed — along
  with an expression nobody could see. At full amplitude twelve brow channels across four clips
  (`adorable`, `fakeFierce`, `annoyed`, `poutyAnnoyed`) sit ON the clamp and provably cannot move.
  A new test enumerates them exactly, so a future clip that saturates a brow has to be added to the
  list deliberately. Recovering both would mean lowering the authored peaks — the owner's taste call,
  not something to smuggle into a semantics fix.
- **The adversarial audit did not run** for this version: all four lenses died on a session limit.
  The three checks it was pointed at were done by hand instead — no surviving `pb.intensity` reader,
  the v0.43.3 zero-frozen-frames invariant still green at full amplitude, and a saturation sweep over
  all 14 clips (the result of which is the KNOWN LIMIT test above). Stated plainly because "audited"
  and "I checked it myself" are not the same claim.

### `v0.43.14` — 2026-08-01 — She has one voice, or none (归一化 runbook 第 1 步)
...
- **DELETED** `packages/web/src/audio/webSpeechSink.ts` (107 lines) and, with it, its only consumer
  `voicePick.ts` (107 lines) + `voicePick.test.ts` — the picker existed solely to choose which system
  voice the browser would borrow.
- `ttsBackend.ts`: the union narrows `'none' | 'browser' | 'http'` → **`'none' | 'http'`**, and the
  default flips from `'browser'` to **`'none'`**. A persisted legacy `'browser'` — from localStorage
  or from luna.env — resolves to `'none'`, never to `'http'`.
- `app.ts`: the pure-browser branch and v0.37.4's second rung are gone. A line the http voice cannot
  speak is now logged and **skipped**; `hasVoice` is `ttsBackend === 'http'`; boot clears the stale
  `luna:voice` key (the v0.36.0 `luna:reduce-motion` precedent).
- **`AudioSink.speak` now reports whether the words reached the room** (`Promise<boolean | void>`;
  `void` = "this sink does not track it" → assume spoken). `SerialQueue.run` became generic so a
  barge-in-skipped task resolves `false` rather than `undefined`. `controller.ts` gates v0.43.13's
  end-of-sentence gesture on it.
- Templates and docs that still *offered* the retired backend: `packages/desktop/src/envfile.ts`
  (the first-run luna.env template wrote `LUNA_TTS_BACKEND=browser` into every new install),
  `.env.example`, `docs/SETUP.md` (§2 + the fallback promise), `README.md`, `README.zh-CN.md`,
  `scripts/app.ts` (told the owner to set `=browser`), `scripts/dev-all.ts`,
  `packages/web/dev-server.ts`, `packages/desktop/src/voicePack.test.ts` fixture.
- `setupWizard.ts`: the voice step offers `http | none`; the default is **`'none'`** and the hydrate
  branch mirrors `resolveTtsBackend` (`backend === 'http' ? 'http' : 'none'`).
- NEW `packages/web/src/audio/browserVoiceRetired.test.ts` — a repo-wide sweep (code **and** prose)
  with two exact allowlists: the files permitted to *name* the browser voice historically, and the
  migration test permitted to *construct* the retired value.
- Suite **1712 pass / 0 fail** (was 1713); all four packages tsc clean.

Test disposition — every one accounted for, per the plan's 不许静默放宽 rule:

| Test | Disposition | Why |
|---|---|---|
| `voicePick.test.ts` (14) | **deleted** | its subject (choosing a system voice to borrow) no longer exists |
| `ttsBackend.test.ts` "defaults to browser" | **rewritten** | asserts `'none'`; the default genuinely inverted |
| `ttsBackend.test.ts` "storage overrides config" | **rewritten** | kept the precedence guarantee, moved the sample off `'browser'` |
| `ttsBackend.test.ts` "unknown value" | **rewritten** | falls through to silence, not to a voice |
| `ttsBackend.test.ts` legacy-`'browser'` migration | **added** (+1) | the new behaviour needed its own pin |
| `setupWizard.test.ts` `modeValues` full-mode | **rewritten + retitled** | see below |
| `webAudioSink.test.ts` ladder (4 names/comments) | **renamed** | assertions unchanged — only the prose named a rung that no longer exists |
| `webAudioSink.test.ts` spoken-report (4) | **added** | the new `speak` contract |
| `controller.test.ts` gesture gating (2) | **added** | a skipped line must not gesture |
| `browserVoiceRetired.test.ts` (6) | **added** | the plan's "pin the absence as a test" |
| `ttsRuntime.test.ts` (2 names/comments) | **renamed** | same: prose only |

1713 − 14 + 1 + 6 + 4 + 2 = **1712**.

Inference:

- The browser voice was a distribution-era answer to a question this project stopped asking. Once
  Luna became one person's single-machine companion (v0.41.0), "a stranger finishes her sentence
  whenever api_v2 hiccups" was all it did. Failure semantics change from *speak it anyway* to
  **skip it and say so in the log** — she has one voice, or none.
- **An adversarial audit of this change found twenty confirmed defects in my first pass, three of
  them blockers, and they were all in the half I had treated as cosmetic.** The first-run luna.env
  template was still minting `LUNA_TTS_BACKEND=browser` into every new install; `.env.example` and
  `docs/SETUP.md` still documented it as the default; and the wizard default I had "fixed" from
  `'browser'` to `'http'` would have written a backend the owner might have no server for — the exact
  promotion `resolveTtsBackend` refuses, undone one layer up. Narrowing a type deletes the code that
  uses a feature. It does not delete the **promises**.
- Hence the guard test scans the repo, not two source directories, and greps prose as well as
  identifiers. Its first draft passed while four live promises shipped; that is the whole argument
  for its current shape.
- `speak` reporting whether it was voiced is the one behavioural addition, and it exists because
  v0.43.13 rode on an assumption this version broke: it fires a question's head-tilt when `speak`
  resolves, which used to mean "she finished saying it". With the fallback gone, resolution also
  covers "that line never happened" — and she was tilting her head after questions the room never
  heard. `void` keeps every existing stub valid, so only a sink that actively reports failure
  suppresses the gesture.
- Landing this while api_v2 is down means she is **silent**, not fallback-voiced. That is the
  intended semantics, and the boot gate and managed-health surfaces still say why.

### `v0.43.13` — 2026-08-01 — Punctuation gestures, and Initiative 30 closes (Initiative 30, 14/14)
...
- NEW `packages/web/src/live2d/utterance.ts` (~75 lines): `classifyUtterance(text)` reads a
  sentence's FINAL punctuation — `？?` → tilt, `！!` → lean in, `……/.../。。。/…` → gaze drifts off,
  `～~` → a light sway — and `UtteranceGestures` alternates the lateral direction across consecutive
  identical gestures. Trailing quotes and brackets are stripped first, so `他说"真的吗？"` still reads
  as a question; a mark mid-sentence does not trigger.
- `faceVm.ts`: NEW `pulseSpeech(pose, ms)` — the same pulse layer behind the speaking-performance
  flag. `addPulse` stays ungated, because the workbench is a debugging surface and a product toggle
  must not make it lie about what it can drive.
- `sinks.ts`: optional `pulse(pose, durationMs)`; `pixiLive2DSink` forwards it to `pulseSpeech`.
- `controller.ts`: classifies each message, then fires against the audio's own timing —
  `onStart` for the `start` gestures, the resolved `speak` promise for the `end` ones. A rejected
  synthesis fires nothing and is swallowed.
- `ARCHITECTURE.md`: the `live2d/` section gains a **Speech and listening** passage — the three
  signals (attention level, audio stress, sentence shape), the one mechanism (additive half-sine
  pulses), the unchanged arbitration rules, and the workbench's reason for existing.
- +30 tests: the full punctuation table in both scripts, all three ellipsis forms, mid-sentence
  non-triggering, wrapper stripping, gesture bounds (≤5 per channel, ≤1000 ms), no eyelid or mouth
  channel, direction alternation and its reset, controller timing (`start` vs `end`), a sink with no
  `pulse` support still speaking, a rejected synthesis firing nothing, and the `pulseSpeech` gate in
  both directions.
- Suite **1713 pass / 0 fail** (was 1689); all four packages tsc clean. The whole-initiative
  regression rides in that number: v0.43.0's blink probe, v0.43.2's outro monotonicity, v0.43.3's
  zero-frozen-frames and v0.43.5's 60 fps equivalence gate all still run and still pass.

Inference:

- **Two signals, not one overloaded one.** The envelope carries no pitch, so a rising question is
  invisible to v0.43.12 no matter how the constants are tuned; punctuation is what is actually
  available without an F0 tracker or an NLU pass. Keeping them separate means each can be wrong on
  its own without corrupting the other.
- Timing is the whole content of this version, and it is why the hook is in the controller rather
  than the engine: only the controller knows *when* a sentence starts and finishes playing. An
  exclamation's emphasis belongs across the utterance; a question's tilt belongs in the pause after
  it, because that is where the rise lands in speech.
- Mirroring only the lateral channels is a small decision that matters: negating a brow lift would
  turn emphasis into a frown. The alternation exists so three questions in a row do not read as a
  stuck animation, not to invert what the face means.
- **This adds a trigger, not a mechanism.** v0.43.12's pulse layer took the gestures unchanged, which
  is the test of whether that layer was built at the right altitude. If the LLM is ever given an
  explicit gesture axis, the landing site already exists — an optional wire field, and this same
  `pulse` / `playAction` surface underneath.

### `v0.43.12` — 2026-08-01 — She nods on the stresses (Initiative 30, 13/14)
...
- NEW `packages/web/src/live2d/accent.ts` (~65 lines): `AccentDetector` over `lip.open`. An
  exponential baseline (τ 600 ms, time-based like v0.43.5's smoothing), a peak test of
  `open > baseline × 1.6 && open > 0.25` measured against the baseline BEFORE the current frame folds
  in, a 550 ms refractory window, and a hard cap of 6 stresses per utterance. Every tunable constant
  is exported from the top of this one file.
- `faceVm.ts`: NEW pulse layer — `Pulse = {startAt, durationMs, pose}`, `addPulse(pose, ms)`,
  `activePulseCount(now?)`, and `applyPulses` sampling a **half-sine** envelope and ADDING it last,
  after the actions. Deliberately generic: v0.43.13's punctuation gestures use it as-is.
- `faceVm.setMouth` now also feeds the detector, and `setMouth(null)` — the sentence boundary the
  audio sink already publishes — resets it. A stress becomes `headPitch 1.2 + 1.6×strength` degrees
  plus a `0.05×strength` brow lift over 380 ms.
- `perfFlags.ts`: NEW `luna:speech-performance` (**Speaking performance**), shared with v0.43.13.
  `pixiLive2DSink` passes it through and publishes `pulses()` on the debug bridge; the workbench
  readout gains a **stress lamp** that lights while a pulse is live.
- +15 tests. The detector: six spaced peaks → exactly six stresses within two frames of each; two
  peaks 300 ms apart → one; silence, sub-floor throat-clearing and a slow swell → zero; 30 peaks → the
  cap; strength monotonic and in 0..1; and the same envelope at 30 fps finding the same stresses as at
  60. The layer: a nod that returns to **zero** residue, the ≤3° bound, the per-sentence fuse holding
  at 6, no eyelid or mouth writes, the flag off meaning no detection at all, and `addPulse` working
  for an arbitrary pose.
- Suite **1689 pass / 0 fail** (was 1674); all four packages tsc clean.
- Verified live: three synthetic peaks produced three pulses, each active immediately after its peak,
  and the workbench lamp lit and cleared with them.

Inference:

- This is the gap Initiative 30 named at its founding and then did not touch for twelve versions:
  *"六个版本做完，她说话的样子和今天一模一样。"* The answer needed no new data source — `lip.open` has
  been a per-frame energy envelope flowing through `setMouth` since the lip-sync shipped, and nothing
  but the mouth had ever read it.
- **Head pitch is written in the pre-physics pass (F26), which is why the hair follows for free.** A
  nod driven at the wrong hook would have moved her head and left her hair behind — the physics is
  doing the expensive half of this feature and it costs nothing.
- Two decisions kept the feature honest against measurement rather than intuition. The peak is
  compared against the baseline *before* the current frame is folded in, because a loud frame drags
  the baseline up and would quietly discount the very peak being tested. And the tests measure
  **differentially** — the same envelope through two VMs, one with the flag off — because the idle
  also drives `headPitch`, so an absolute reading measures the sway. Both of those started as bugs in
  the first draft.
- What is deliberately absent: pitch. There is no F0 here, so a rising question intonation is
  invisible to this layer; v0.43.13 approximates it from the text's punctuation instead — two
  independent signals rather than one overloaded one. And the browser fallback voice has no envelope,
  so this layer stays silent there rather than faking a performance.
- The constants are a starting point, not an answer. The workbench lamp exists precisely so the owner
  can calibrate them against his own voice model by ear; that part is his, and no test can stand in
  for it.

### `v0.43.11` — 2026-08-01 — The half of the conversation where she is not talking (Initiative 30, 12/14)
...
- `faceVm.ts`: NEW `setListening(on)` / `isListening()` and a `LISTENING_BIAS`
  (`headYaw −3`, `bodyYaw −1.5`, `browL/RY +0.05`) **added** to the target under everything else, so
  it rides beneath a playing clip instead of replacing it. Head and body only — the gaze stays with
  whoever owns it.
- `faceVm.ts`: `updateIdleActions` picks its pool and cadence per tick from the state. In `thinking`
  it draws from a NEW `THINKING_ACTION_POOL` at **3.5–8 s** instead of the idle 8–20 s; a reply
  arriving flips the state and it returns to the idle pool with no extra bookkeeping.
  `pickIdleAction` now takes the pool as an argument.
- `faceData.ts`: two new `ActionDef`s, data only — `gazeUpRecall` (1.6 s: gaze up-and-right, head
  follows, brows lift — the posture of retrieval) and `browKnit` (0.9 s: a knit and release).
  Neither declares an eyelid track.
- `perfFlags.ts`: NEW `luna:listening` (**Attentive listening**), gating both halves — they are one
  behaviour seen from two sides. `sinks.ts`: optional `setListening`; `pixiLive2DSink` forwards it and
  passes the flag through. `layout.ts` + `app.ts`: the settings row, and the typing detection —
  `input` sets the level, a 1.5 s debounce and `blur` and `send()` clear it.
- `pickIdleAction` gained a fallback: the no-repeat memory is 3 and the thinking pool has exactly 3
  entries, so applied literally it would starve after three picks and she would freeze mid-thought.
- +8 tests, and **three existing assertions corrected** while writing them: two hardcoded gesture
  counts became derived from `ACTIONS`, and a claim that no thinking gesture drives an eyelid was
  wrong — `lookAwayThenBack` does, legitimately (v0.43.4 rejected stripping it), so the assertion is
  now that it RETURNS them.
- Suite **1674 pass / 0 fail** (was 1666); all four packages tsc clean.
- Verified live: typing shifted `ParamAngleX` by −3.97° toward the panel and `isListening()` went
  true; clearing the field released it; `thinking` fired exactly the three thinking-pool gestures and
  `neutral` went straight back to the idle pool.

Inference:

- The initiative's own open question named this gap: six versions had made her vivid **while quiet**,
  and a conversation is half waiting. This is the cheapest possible answer to it — every turn passes
  through both states, so the appearance rate is 100% and the cost is a bias and a pool swap.
- **Both halves are additive and reuse existing machinery.** The typing bias is `addPose` under the
  existing layers; the choreography is v0.43.4's scheduler with a different pool, because its gate
  (`!playback && !lip && !sleeping`) already let `thinking` through — what was missing was never
  scheduling, it was something worth playing there.
- Listening is a LEVEL, not an event, and that is what makes per-keystroke jitter impossible by
  construction rather than by tuning. The debounce lives in `app.ts` where the keystrokes are; the
  engine only ever sees "yes" or "no".
- The gaze arbitration from v0.13.x is deliberately untouched. Turning toward you is a head-and-body
  act; letting attention grab the eyeballs would have fought the focus controller for the one channel
  the user's own mouse is already driving.

### `v0.43.10` — 2026-08-01 — What she wears is his to decide, and it stays decided (Initiative 30, 11/14)
...
- `faceData.ts`: NEW `COSTUME` table — `eyepatch` / `mic` / `puppy` / `longHair` / `shortHair2`, with
  the two hairstyles sharing a mutual-exclusion `group`. `ParamarmupL/R` is deliberately excluded (a
  raised arm is gesture material), as is `Paramdown1` (already an `adorable` overlay).
- NEW `packages/web/src/live2d/costume.ts` (~75 lines): `COSTUME_KEY = 'luna:costume'`,
  `parseCostume` (drops unknown ids and malformed JSON), `loadCostume`, `saveCostume`,
  `toggleCostume` (applies the exclusion group), and `costumeWrites` — which returns a **total** write
  set, every catalog item with an explicit value, so a removed item is released rather than forgotten.
- `pixiLive2DSink.ts`: applies `costumeWrites(loadCostume())` at construction. **Zero new engine
  code** — v0.43.8's per-frame `setManualParam` is the whole runtime.
- `layout.ts`: a Costume card on the Avatar tab, its own card because it is a different kind of
  switch from the ones above it; `app.ts` fills, persists and re-paints it (a hairstyle turning on
  visibly takes the other off). `ui/workbench.ts`: the bench's five costume rows now write through the
  same persisted state via `costumeIdForParam`, so there is one answer to "what is she wearing".
- +13 tests: catalog vs `yumi.cdi3.json`, **costume and overlay param sets disjoint**, arms excluded,
  storage round-trip, malformed/unknown-id rejection, hairstyle exclusivity in both directions,
  totality of the write set, and the bench↔settings mapping.
- Suite **1666 pass / 0 fail** (was 1653); all four packages tsc clean.
- Verified live: eyepatch + hairstyle on, page reloaded, both still worn and both boxes still checked;
  turning on the second hairstyle zeroed the first and rewrote storage; and playing `adorable`
  mid-costume ran its heart-eyes overlay at 0.95 without disturbing either worn param.

Inference:

- This closes the three-way wall the owner ruled on, and it is now stated in tests rather than in
  prose: **the emotion system never chooses a prop** (v0.43.1, unchanged), **the LLM cannot either**
  (the `message` schema has no such field — nothing was added in v0.43.6), **the owner does as he
  likes** (this version). Three actors, three permissions, no ambiguity about which one acted.
- Disjointness between `COSTUME` and `ALL_OVERLAY_PARAMS` is the load-bearing invariant. Overlays are
  zeroed every frame by the clip layer; if a costume id were also an overlay id, a clip ending would
  quietly undress her, and it would look like a bug in persistence rather than in the tables.
- The total write set is the same discipline as `ALL_OVERLAY_PARAMS`: state changes must write what is
  now FALSE, not only what is now true. Add-blend params never decay on their own.
- Cost to the engine: nothing. v0.43.8 built the runtime for a debugging affordance; making it a
  product feature turned out to be a table, a storage module and a card.

### `v0.43.9` — 2026-08-01 — A new face gets looked at before it gets committed (Initiative 30, 10/14)
...
- `faceVm.ts`: NEW `setComposeMode(pose | null)` + `composeActive()`. While a compose pose is set,
  `tick` skips idle, state bias, mood, clip, gestures, residue and microsaccades, and `rest` is pinned
  to `FACE_VM_DEFAULT_STATE` — but the write path (smoothing → gain → clamp → `setParam`) is unchanged.
  The gaze gate is bypassed in compose so the two gaze sliders work with mouse-follow on.
- `faceVm.ts`: NEW `previewPose(def, intensity)` + a private `defOf(id)` through which all six clip
  lookups now go, so a transient `EmotionDef` plays via the same playback machinery. Its id
  (`__preview__`) is deliberately not a key of `EMOTIONS`, so `triggerEmotion`'s guard can never
  reach it by name.
- `ui/workbench.ts`: NEW `composeChannels()` (bounds read out of `clampStateValue`; the unclamped
  angle channels get the model's ±30 working range), `COMPOSE_GROUPS` (the engine's six groups plus a
  UI-only `pupil` group — `pupil*` is in no engine group by design), `composeFromClip`, `inferOwns`,
  `composeEmotionDef`, `exportEmotionDef`.
- The composer UI: an Enter/Exit compose toggle with a red `COMPOSING` badge, a clip loader, 35
  sliders in collapsible groups (brows + mouth open by default), **A/B play** (exits compose and runs
  the pose as a real clip at `timelineFor`'s SHORT timeline), and **Copy EmotionDef** to clipboard
  with a textarea fallback. `dispose()` clears compose, and `pagehide` calls it.
- +16 tests: the 600-tick silence proof (only composed channels written — no pupil, no sway, no mood),
  the gain-path equality for a clamped and an unclamped channel, the eyelid exception and its release,
  living layers returning after exit, a preview playing and releasing, `__preview__` unreachable by
  name, load→export losslessness against `EMOTIONS.shy`, `owns` inference, default-channel omission,
  and the exported JSON's shape.
- Suite **1653 pass / 0 fail** (was 1637); all four packages tsc clean.
- Verified live: loaded `disappointed`, edited `browLY`/`gazeY`/`mouthForm`, watched her hold it,
  exported a 25-channel `EmotionDef` with `owns: [pose, gaze, eyes, mouth, brows]`, then A/B-played it
  as `__preview__` — the full "human eyes first" pipeline, once, end to end.

Inference:

- This version is the answer to the second half of what the owner asked for: *"哪怕是后面要新加情绪
  资产，也可以先走我一个人类视角看看离不离谱。"* Whether the affect vocabulary should grow (sad /
  tired / expectant — the initiative's open question) stops being a bet and becomes a thing he can
  look at first.
- **The sliders must edit pre-gain state values, and the preview must go through the gain.** That is
  the one constraint the whole version hangs on: `faceData.ts` stores pre-gain numbers, so a preview
  that bypassed `FACE_PARAM_GAIN` would look correct and export numbers meaning something else
  entirely. Freezing the layers is what makes the pose stable; keeping the write path is what makes
  the export true.
- Compose deliberately violates v0.43.0's eyelid invariant, and that is the invariant working as
  written: it says *persistent* layers never write `eyeOpen` while *time-bounded* ones may and must
  release. Compose is bounded by the owner leaving the bench, and half-lidded is a face he has to be
  able to author. The exit path is tested, not assumed.
- A/B play exists because a pose that reads as a still is not the same as a pose that reads arriving
  and leaving. It runs at the short timeline for the same reason — rehearsing at the retired 5.6 s
  beat would evaluate a performance the app no longer gives.
- Not done, on purpose: no new clip was added to `faceData.ts`. This version ships the pipeline; the
  first faces through it are the owner's to author.

### `v0.43.8` — 2026-08-01 — Nine gestures and seventeen assets, reachable by hand (Initiative 30, 9/14)
...
- `faceVm.ts`: NEW `playAction(name, intensity)` — keys the instance `manual:<name>:<t>` into the
  existing `actions` map, so it coexists with a same-named scheduled instance and reuses
  `applyActions` verbatim. NEW `listActions()`, `manualParamIds()`.
- `faceVm.ts`: NEW `setManualParam(pid, value | null)` + a per-frame write loop at the END of `tick`,
  after the overlay loop. `null` deletes the entry and queues **exactly one 0** so an Add-blend asset
  releases instead of latching. NEW read-only `activeOverlayParams(now?)`.
- `sinks.ts`: `Live2DSink` gains optional `playAction` / `setManualParam`, alongside the existing
  manual-trigger methods; `pixiLive2DSink` forwards both.
- `ui/workbench.ts`: NEW `MODEL_ASSETS` — all 17 shipped `.exp3.json` assets with their real param
  ids, split `emotion` (9) / `costume` (8), plus `COSTUME_NOTE`. NEW gesture section derived from
  `Object.keys(ACTIONS)`. Emotional asset rows get a `.driven` class whenever the clip layer is
  actually driving that overlay.
- +15 tests: per-frame write counting (100 ticks → 100 writes), the single release 0, worn-beats-
  overlay write order, `manual:` keying and self-expiry, an unknown gesture as a no-op, the catalog
  checked against the real `yumi.cdi3.json`, and the v0.43.1 prop wall re-asserted.
- Suite **1637 pass / 0 fail** (was 1622); all four packages tsc clean.
- Verified live: eyepatch + microphone worn together read 1/1, removing the eyepatch leaves the mic
  at 1 and drops the eyepatch to 0, and playing `adorable` lights `Paramheart` + `Paramdown1` in the
  asset list — the clip→asset mapping, visible while it happens.

Inference:

- **Per-frame writing is not belt-and-braces, it is the mechanism.** The prop params are outside
  `ALL_OVERLAY_PARAMS` so nothing zeroes them, but pixi-live2d-display's motion update has a
  parameter save/restore cycle that discards a one-shot write. A "wear it once" API would have worked
  in tests and flickered on screen.
- The release path matters as much as the wear path. Add-blend params do not decay, so simply
  forgetting a worn asset leaves her in an eyepatch permanently — the one queued 0 is what makes
  taking it off a real operation.
- The costume wall from v0.43.1 is unchanged and re-asserted here. The rule was never "these assets
  are forbidden", it was "the emotion system must never CHOOSE them" — she does not produce a
  microphone mid-apology. The bench is the owner's hand, which is a different actor.
- `activeOverlayParams` turns the `overlayRefs` table into something observable. Before this, which
  clip carries which drawn asset was knowable only by reading `faceData.ts`.

### `v0.43.7` — 2026-08-01 — A bench for the face (Initiative 30, 8/14)
...
- NEW `packages/web/src/workbenchMode.ts` (27 lines): `isWorkbenchMode` (an exact `?workbench` query
  key, not a substring), `debugBridgeEnabled` (`dev` OR workbench), `workbenchModelUrl` (a `?model=`
  override for an external browser pointed at the desktop shell's static server).
- NEW `packages/web/src/ui/workbench.ts` (~250 lines): the data half — `workbenchSections()` derived
  from `ExpressionKey.options`, `Object.keys(EMOTIONS)`, the four `Live2DState`s and `IDLE_PROFILES`;
  `applyControl` dispatching each kind onto the `Live2DSink` method that performs it; `parseIntensity`;
  `readout`. Then `mountWorkbench` assembling the DOM over it.
- NEW `packages/web/src/live2d/perfFlags.ts`: the five per-tick flag keys, their labels, and
  `flagOn`/`setFlag`. `pixiLive2DSink` now imports them instead of declaring its own copies, so the
  settings card, the engine and the bench cannot disagree about a key.
- `app.ts`: a `?workbench` early-exit branch after `?setup` — mounts the bench, resolves a model,
  mounts the REAL `createPixiLive2DSink` into its stage, returns. No WS, no chat UI, no boot gate.
- `pixiLive2DSink.ts`: the `__lunaDbg` bridge gate widens to `debugBridgeEnabled(location.search)` and
  gains `playback()`. `faceVm.ts`: NEW read-only `currentPlayback(now?)` returning `{id, intensity,
  phase}` or null, the clip-layer counterpart to v0.43.4's `activeActionIds`.
- `expressionMap.ts`: `HIGH_INTENSITY` is exported and re-exported by the bench as `INTENSITY_MARK`,
  so the slider's tick mark IS the branch threshold. `layout.ts`: a `🎛 Live2D workbench` button on the
  Avatar card; `app.ts` stashes the current URL in `sessionStorage` so "← Back" returns to the exact
  instance. `theme.css`: ~40 lines of bench skin.
- +20 tests. Suite **1622 pass / 0 fail** (was 1602); all four packages tsc clean.
- Verified live against the real renderer on a throwaway port: the button chain reaches the engine
  (`ParamCheekpuff` 0.007 → 1.000 across a `poutyAnnoyed` intro→perform), and the slider at **0.69
  performs `annoyed`, at 0.70 `poutyAnnoyed`** with the label flipping to "escalated".

Inference:

- The owner's words were "我实在有点乱了" — six layers, five flags, three trigger sources, and one
  observable surface. This is not a feature for her; it is the instrument that makes the next
  expressive versions decidable by looking instead of by argument.
- **The bench had to be a URL mode on the shipped build, not a separate page.** A bench with its own
  renderer would let him tune a face that the app then performs differently, which is worse than no
  bench — it would manufacture confident wrong conclusions. Same `createPixiLive2DSink`, same
  `FaceVm`, same bundle.
- Adding `currentPlayback` is the one place this version touched the engine, and it is deliberately
  read-only and phase-honest (`inactive` reports as `null`). The alternative — the bench remembering
  what it last clicked — would have shown a clip name for seconds after the clip ended.
- The `perfFlags` extraction was not tidying. Three separate writers of the same five string literals
  is exactly the shape where a toggle silently controls nothing, and this version was about to add
  the third.

### `v0.43.6` — 2026-08-01 — Telling her which levers she has (Initiative 30, 7/14)
...
- `packages/server/src/tools/builtin/message.ts`: both optional performance fields get real describes.
  `expression` now names the four affects that carry a drawn asset (`bright_delight` → heart eyes,
  `playful_brightness` → star eyes, `shy_softness` → blush, mild `awkward_lightness` → swirl eyes) and
  states that a strong affect keeps tinting her resting face for tens of seconds after the clip ends.
- `emotion` changes from `0 = barely visible, 1 = full` to the actual semantics: **≥ 0.7 escalates**
  `annoyed_resistance` / `awkward_lightness` / `guarded_distance` / `gentle_concern` into a different
  clip, and **omitting the field never escalates**.
- Zero behavior change: no `ExpressionKey` variant added, no `expressionMap.ts` threshold or branch
  touched, no `superRefine` change. The Zod shape is byte-identical apart from the description strings.
- +3 tests (`message.test.ts`): the drawn extras and the lingering mood are present on `expression`;
  `0.7` / `escalate` / `Omitting it never escalates` are present on `emotion`; and all four escalating
  affects are named **by their wire key**, so renaming one without updating the schema goes red.
  Keyword assertions, not prose snapshots — the wording gets re-tuned against the workbench.
- Suite **1602 pass / 0 fail** (was 1599); `server` tsc clean.

Inference:

- v0.43.0–v0.43.5 built four levers she had no way of knowing existed. The 2026-07-31 live test made
  that concrete: she told the owner she could not trigger heart eyes (she can — `bright_delight`), and
  that she carries no state between messages (false since v0.42). Those were not lapses in her
  persona; they were accurate reports of everything she is shown.
- **The tool schema is the only text present on literally every turn.** Her `self-architecture` skill
  points at the live2d source, but that is a map to grep when a question forces it — not something she
  consults mid-banter. So the schema is the correct and only reliable place for this, and it costs
  ~60 words inside the cached prefix.
- `emotion`'s old describe was not merely thin, it was actively misleading: "0 = barely visible,
  1 = full" reads as a continuous dimmer, so the sensible move is to leave it off and let the renderer
  pick. Under v0.43.2's branch that means the four escalated clips — the most distinctive poses in the
  set, only just made reachable — stay unreachable in practice. Documenting the threshold is what
  actually finishes v0.43.2.

### `v0.43.5` — 2026-07-31 — Frame-rate independence, and a re-tune measurement vetoed (Initiative 30, 6/6)
...
- `faceVm.ts`: NEW exported `approach(sm, dt)` converts the old fixed per-frame coefficient into the
  equivalent exponential approach over real elapsed time (`rate = −ln(1 − sm)·60`). `tick` now derives
  `dt` from the tick clock, clamped to 0.25 s.
- NEW `SMOOTHING_SCALE`: per-organ multipliers (eyes 1.5, mouth 1.4, brows 1.2, gaze 0.9, pose 0.6),
  applied on top of the state- and arousal-derived rate rather than replacing it, so v0.42.2's
  arousal modulation still works. Behind `smoothingEnabled` (default on).
- `GAIN` in `affectPose.ts` is **unchanged at 2.6**. Re-measured under the full v0.43.x pipeline,
  20 s after a `bright_delight`: `ParamEyeSmileL` 0.406, `ParamBrowYL` 0.255, `ParamMouthForm` 0.206,
  against v0.42.4's 0.371 / 0.229 / 0.337.
- `ARCHITECTURE.md`: the `live2d/` section documents the thaw, the 35-channel count, and the three
  arbitration rules — eyelids belong to the built-in blink, head/body are physics inputs written in a
  separate pre-physics pass, smoothing is time-based.
- +6 tests: the 60 fps equivalence gate across all nine (state × arousal) rate combinations, the
  expression still landing, 60 fps vs 30 fps settling to the same face, the backgrounded-tab clamp,
  eyes settling before posture, and one shared rate again with the flag off.
- Suite **1599 pass / 0 fail** (was 1593); `web`, `server`, `desktop`, `protocol` tsc clean.

Inference:

- The equivalence gate is the whole reason this version is safe. A smoothing change is exactly the
  kind of edit that "looks like a refactor" and quietly re-tunes every hand-adjusted constant
  downstream of it; stating the invariant as an equality between the old and new coefficient makes
  that impossible to do by accident.
- **The most useful thing this version did was not change something.** The plan asserted `GAIN` would
  "almost certainly need recalibrating" — a reasonable prediction, since clips got shorter and the
  thaw landed. Measurement disagreed by ±10%, which is noise next to an idle that swings the mouth by
  ±0.22. Initiative 29 drifted precisely because a coefficient was carried over on reasoning rather
  than measurement; the discipline only counts if it also stops changes.
- What was deliberately NOT done: the plan's 4–6 day recalibration of the 130 lines of idle sine
  amplitudes. It buys frame-rate consistency the owner cannot see on one machine with one screen, and
  it would scramble a feel he tuned by hand over many versions.

### `v0.43.4` — 2026-07-31 — A beat, not a sit; and she fidgets now (Initiative 30, 5/6)
...
- `faceData.ts`: NEW `timelineFor(def, short)` maps the authored `performMs` (5200–6800) linearly onto
  2200–2800, leaving `introMs`/`outroMs` untouched. Both sets coexist; nothing is re-authored.
- `faceVm.ts`: the timeline is resolved ONCE at trigger time, not per frame — a clip that began long
  must finish long, or flipping the flag mid-performance would jump her out of a pose.
- NEW `updateIdleActions` + `pickIdleAction`: one gesture every 8–20 s while no clip plays, no
  lip-sync frame, and not sleeping. Weighted draw over `IDLE_ACTION_POOL` (nine entries, each with a
  v/a/d lean), floor 0.05 so a mood biases the draw and never vetoes an action. `IDLE_ACTION_MEMORY`
  (3) bars recent repeats; intensity 0.72, softer than a clip's 0.95.
- NEW `FaceVm.activeActionIds()` — the action layer's only observation point, since a gesture shares
  channels with the idle and has no unique parameter signature to assert on.
- `physicsPassthrough` is now READ, in both `ownedKeys` and `blendedState`: a clip that declares a key
  neither owns nor writes it, so the built-in controller keeps it. `adorable` is the only clip that
  declares any, and it declares the eyelids.
- `paramMap.ts`/`faceData.ts` unchanged for channels; three new flags — `luna:short-clips`,
  `luna:idle-actions` (both default on) — plus **Brief performances** and **Idle gestures** toggles in
  the Avatar settings card.
- +13 tests: timeline bounds and ordering preserved, intro/outro untouched, flag returns authored
  values exactly, the t=3000 floor rationale asserted, ≥20 gestures in five minutes, no repeat within
  three, nothing while a clip performs, mood-biased selection (warm leans in more than cold does),
  eyelids written by a gesture and released, and both halves of `physicsPassthrough`.
- Suite **1593 pass / 0 fail** (was 1580), `packages/web` tsc clean.

Inference:

- Two plan recommendations were rejected on contact with the code, both for the same reason — they
  were written against v0.43.0's *original* invariant rather than the corrected one. Stripping
  `eyeOpen` from `slowBlinkAffection` and `lookAwayThenBack` would have deleted authored character
  (a slow affectionate blink IS an eyelid gesture) to satisfy a rule that had already been sharpened
  to "persistent layers never, time-bounded layers may".
- The `performMs` floor is a small case of a general principle worth stating: when a change breaks an
  assertion, the first question is whether the assertion was protecting something real. Here it was —
  "a clip has reached its pose 3 s in" is the guarantee that expressions are actually visible — so the
  code moved to fit the test rather than the reverse.
- Gating spontaneous gestures on "no clip playing" is a deliberately cheap answer to an arbitration
  problem that would otherwise need `applyActions` to learn about ownership. It costs nothing real:
  the whole point of these gestures is what she does when the conversation is quiet.
- `physicsPassthrough` was dead scaffolding with a clear author's intent attached. Deleting it would
  have been tidier and would have thrown away a design note; implementing it took nine lines and gave
  `adorable` back its blink.

### `v0.43.3` — 2026-07-31 — The peak stops being a still photograph (Initiative 30, 4/6)
...
- `faceVm.ts`: NEW `applyIdleResidue`, called from `tick` **after** `applyEmotion` (before it, the clip
  would simply overwrite it). For every face channel a clip owns it adds
  `(idle[key] − rest[key]) × RESIDUE_SHARE (0.22)` plus a per-channel sine wobble
  `sin(t·rate + phase) × JITTER_AMPLITUDE (0.035) × min(1, |target − rest|)`.
- `RESIDUE_KEYS` = brows + mouth + specials + eyeSmile/eyeSquint/eyeSize. Head, body and gaze are
  excluded: the built-in `CubismBreath` already adds ±15°/8°/10° to `ParamAngleX/Y/Z` every frame, and
  those channels are physics inputs written in the separate `flushPose` phase. Eyelids are excluded by
  the v0.43.0 invariant.
- `JITTER_RATES` are eight incommensurable values from 0.6 to 1.9 rad/s. A first attempt at 0.31–1.27
  gave the right amplitude but a 20 s period, which does not register as motion at all.
- `applyIdle` now RETURNS the pose it computed, so the residue pass reuses the same frame's idle
  rather than re-evaluating the profile's trig.
- `paramMap.ts`: +2 channels `pupilX`/`pupilY` → `ParamhitomiX`/`ParamhitomiY`, defaults 0, clamped
  ±1. Deliberately **not** added to `FACE_CHANNEL_GROUPS.gaze` — that is what makes them unownable, so
  neither a clip nor the focus controller can take them.
- NEW `updateSaccade`: re-picks a target within ±0.15 every 0.42–1.9 s and holds it. Driven from the
  injectable `rng`, so it is deterministic under test.
- Flag `luna:live-peak` (default on, `'0'` opts out, re-read every tick) + a **Living expressions**
  toggle in the Avatar settings card, wired the same way as Mood memory.
- +10 tests: zero frozen frames on four channels across two clips, visible range > 0.01, provably
  still-photograph with the flag off, peak not diluted (within 8% of the authored value), head/body
  byte-identical, no channel past its clamp across all 14 clips, and three pupil tests including that
  the eyelid invariant survives.
- Suite **1580 pass / 0 fail** (was 1570), `packages/web` tsc clean.

Inference:

- The plan listed the idle-residue as the main lever and `livingJitter` as "only if that is not
  enough". Measuring during implementation inverted that: the residue can only thaw channels the idle
  actually drives, and the default profile drives two. Shipping the plan as written would have
  produced a green suite and a face still frozen everywhere except the mouth.
- The test metric mattered as much as the fix. A per-frame-magnitude threshold is a bad proxy — it
  fails gentle motion and passes twitchy motion, i.e. it optimises for exactly the wrong thing. "Zero
  frames where the value did not change" states the bug's inverse directly and has no tunable in it.
- Sampling had its own trap: mouth channels are written twice per frame (main loop, then the lip-sync
  ownership block), and counting `setParam` calls read the duplicate as a frozen frame. The probe
  takes one sample per tick.
- `ParamhitomiX/Y` is the useful half of the "98 unused parameters" story. Most of that list is
  physics (eyelash, hair, cloth, fingers, ears) which belongs to the physics engine; the pupil offset
  is the one genuinely expressive channel nothing was driving.

### `v0.43.2` — 2026-07-31 — Fourteen faces, and no goodbye flare (Initiative 30, 3/6)
...
- `packages/web/src/live2d/expressionMap.ts` rewritten. `AFFECT_TO_EMOTION` values become
  `EmotionId | null | { low, high }`; `affectToEmotion(key, intensity?)` picks the branch at
  `HIGH_INTENSITY = 0.7`. Four affects branch: `guarded_distance` → `skeptical`/`fakeFierce`,
  `annoyed_resistance` → `annoyed`/`poutyAnnoyed`, `awkward_lightness` → `awkwardV2`/`embarrassed`,
  `gentle_concern` → `tender`/`disappointed`. `listening_attention` moves off `curious` onto `focused`
  and `open_reengagement` off `tender` onto `curious`/`tender`. NEW `reachableEmotions()` exports the
  full reachable set for the coverage test.
- `intensity` is the raw, possibly-`undefined` wire value. `faceVm.setExpression` keeps its 0.95
  fallback for *pose strength* but passes `emotion` through untouched for the *branch* decision —
  an omitted intensity must not mean "maximum", or every unannotated message performs the intense clip.
- `faceVm.ts` `overlayParams`: outro weight is now `1 - stage.weight`, and every overlay is scaled by
  `pb.intensity` to match the pose's existing `lerp(base, raw, intensity)`.
- `affect.ts`: the comment claiming the lookup "collapses these to 11 clips" was wrong before this
  version (it was 10) and is now obsolete; replaced with why the continuous layer keeps its own table.
- `expressionMap.test.ts` rewritten (+9): table covers `ExpressionKey.options` exactly, all 14 clips
  reachable, the four formerly-orphaned ones named explicitly, ≥13 distinct faces, branch behaviour,
  threshold boundary at 0.69/0.70, and `undefined` intensity ≡ 0.
- `faceVm.test.ts` +4: whole-outro monotonic-decrease trace on `shy`'s blush (>100 samples, zero
  rises, ends at exactly 0), the intro still monotonically increasing, the same for a v0.43.1 peak
  asset, and overlay strength tracking intensity.
- Suite **1570 pass / 0 fail** (was 1557); `packages/web` and `packages/server` tsc clean.

Inference:

- The "all her expressions look the same" complaint that started this whole initiative had a large,
  boring cause sitting in a 15-line lookup table: six of the fifteen affects were being performed as
  one of two faces. No amount of engine work would have fixed that, and no amount of engine work was
  needed to fix it.
- The intensity branch is worth more than the extra clips it unlocks. It means the same affect can be
  *two different performances*, which is the first thing in this engine that makes the model's choice
  of intensity change what you see rather than merely how far a slider travels.
- The `undefined` trap is the kind that ships silently: it type-checks, every test that passes an
  explicit intensity passes, and the only symptom is that she is permanently over-acting. Worth the
  dedicated assertion.
- The outro flare had been in every build since overlays existed and nobody named it, because "the
  blush brightens for a second at the end of her message" reads as a deliberate flourish rather than a
  sign error. It took reading `stage()`'s return contract to see that the weight changes meaning
  between phases.

### `v0.43.1` — 2026-07-31 — The artist's peak assets reach the screen (Initiative 30, 2/6)
...
- `packages/web/src/live2d/faceData.ts`: `OVERLAYS` gains four rows — `爱心眼: {Paramheart: 1}`,
  `星星眼: {Paramxingxing: 1}`, `眼泪: {Paramtear: 1}`, `蚊香眼: {Paramwenxiang: 1}`.
- Wired via `overlayRefs`: `adorable` += `爱心眼`, `playful` += `星星眼`, `awkwardV2` += `蚊香眼`,
  `disappointed` += `眼泪` (it already carried `泪汪汪`, so the pair reads as welling *and* falling).
- `ALL_OVERLAY_PARAMS` is no longer a hand-maintained parallel array; it is
  `Object.values(OVERLAYS).flatMap(Object.keys)`. That list is exactly what `flushPose` zeroes every
  frame (`faceVm.ts:258`), so an entry missing from it is an asset that can be switched on and never
  switched off.
- NEW `packages/web/src/live2d/faceData.test.ts` (69 lines): every overlay param must exist in the
  real `yumi.cdi3.json`; no costume/prop id may appear in any overlay; `ALL_OVERLAY_PARAMS` must cover
  the whole table; the four new wirings are asserted by name; no `overlayRefs` entry may dangle; no
  emotion may carry more than three overlays.
- `faceVm.test.ts`: +5 end-to-end tests — heart/star/swirl eyes each exceed 0.5 mid-perform, an
  unrelated emotion leaves all four at exactly 0, and `Paramheart` returns to 0 after the timeline.
- **No exp3 loader, no `suppressParamIds`, no change to `yumi.model3.json`.** The model directory is
  `.gitignore`d, so the plan's "write a re-runnable registration script to work around that" problem
  disappeared along with the design that needed it.
- Suite **1557 pass / 0 fail** (was 1546), `packages/web` tsc clean.

Inference:

- The plan for this version was built on a reasonable-sounding assumption — "the model has 17 unplayed
  expressions, therefore we need an expression subsystem" — that survived a 13-agent investigation and
  died in thirty seconds of actually opening the files. The cheap check should come before the design,
  not after it. It also cut the version from Medium risk / 1–2 days to Low / half a day.
- The honest asset count matters more than the impressive one: nine "unused emotional expressions"
  was really four, because five were already reachable through channels that had simply never been
  driven hard. Recording 4 instead of 9 is what keeps the next planning round from over-promising.
- The costume/prop boundary is the one genuinely irreversible-feeling decision here. An automatic
  VAD-driven selector over all 17 assets would look impressive in a demo and produce nonsense in
  practice — she would don an eyepatch mid-sentence. Hand-written whitelist, enforced by a test that
  fails if a prop ever leaks in.

### `v0.43.0` — 2026-07-31 — She blinks again (Initiative 30, 1/6)
...
- `packages/web/src/live2d/affectPose.ts`: DELETED the eyelid-aperture projection —
  `eyeOpen = aroused*0.08 − calm*0.07 − negative*0.035` and its two `put(pose, 'eyeOpenL'/'eyeOpenR')`
  calls. `eyeSmile` and `eyeSquint` are untouched; `aroused`/`calm` are still consumed by the brow,
  posture and mouth terms.
- `packages/web/src/live2d/faceVm.ts`: DELETED `eyeOpenL: 0.85, eyeOpenR: 0.85` from
  `STATE_BIAS.thinking`. The remaining `headPitch: -6, gazeY: -0.4, browLForm/browRForm: -0.3` carry
  the pensive read. `STATE_BIAS.sleeping`'s `eyeOpenL/R: 0` is deliberately kept.
- Both sites carry a WHY comment naming the invariant and the measurement, since the deletion is not
  self-explaining — the code that was removed looks reasonable in isolation.
- `affectPose.test.ts`: the pre-existing test *"arousal opens the eyes and lifts the brows"* asserted
  the deleted behaviour and was **rewritten, not loosened** — it now asserts arousal lifts the brows
  and that low arousal narrows the eyes through the **squint** channel (a different parameter from the
  blink one). +4 tests: no entry in the 15-affect `AFFECT_VAD` table yields an `eyeOpen` key, nor does
  any of the 27 lattice points of the VAD cube, `restingState` leaves both eyelids at exactly the
  engine default everywhere, and the squint channel still carries mood.
- `faceVm.test.ts`: +6 tests in a new `the eyelid invariant` block, built on a probe that **counts
  every `setParam` on an id** rather than reading the last value — the failure mode is "written every
  frame with the same number", which a last-value assertion cannot see. Covers: mood alone → 0 writes
  over 600 frames; a sour mood → 0; a clip **may** own the eyelids while performing; a clip **must**
  release them after its timeline; `thinking` → 0 (was 501/501); `sleeping` still writes 0.000.
- No flag. This is a regression fix, not a feature — hiding it behind a switch would have meant
  shipping the broken default again.
- Suite **1546 pass / 0 fail** (baseline 1535), `packages/web` tsc clean.

Inference:

- The bug is a clean instance of a general hazard in this engine: `FaceVm` deliberately writes *after*
  the built-in controllers so that expressions cannot be overwritten by auto-idle-motion — which means
  **every parameter FaceVm touches persistently is a parameter the built-ins no longer own**. Eyelids
  were the first casualty; the same reasoning applies to anything else Cubism drives natively.
- Four versions of Initiative 29 were green the whole time. The tests asserted direction, ordering,
  bounds, persistence and byte-identity — but never *write frequency*, and never checked that a layer
  stayed out of a channel it had no business in. v0.42.4 added the missing amplitude assertion; this
  version adds the missing ownership one.
- The plan for this version was itself wrong in a way only implementation could reveal: it asserted a
  blanket "no layer writes eyeOpen", derived from a grep that covered `faceVm.ts` but not
  `faceData.ts`. All 14 clips write eyelids, and the asymmetric ones are signature character. The
  invariant that survived contact is sharper and more useful — persistence, not the channel, is what
  makes a writer illegitimate.
- `adorable` has carried `physicsPassthrough: ['eyeOpenL','eyeOpenR',…]` since it was authored, and
  that field still has **no reader anywhere in the repo**. The author's intent — hand the eyes back
  during a clip — has never been implemented. Now that clip-time eyelid ownership is an explicit,
  tested rule, that field is the natural place to make it selective; deferred to v0.43.3/v0.43.4.

### `v0.42.4` — 2026-07-31 — Perceptibility (Initiative 29 follow-up)
...
- `affectPose.ts`: new `GAIN = 2.6`, applied inside `put()` **before** the angle scale and the ceiling. Nothing else moved — every coefficient, every interaction term, the 0.46 ceiling and the ×8 angle scale are unchanged.
- Measured before (real pipeline, `bright_delight` @ 1.0, sampled 25 s after the clip ended): `ParamMouthForm` +0.130, `ParamEyeSmile` +0.143, `ParamBrowY` +0.089. After: **+0.337 / +0.371 / +0.229**.
- +2 tests in `affectPose.test.ts`: a strong warm mood must move `mouthForm` > 0.2 and `eyeSmileL` > 0.1 (the perceptibility floor), and the ceiling must still bind at all 8 VAD corners.
- Suite 1535 green across three consecutive runs (one earlier run reported a single failure that did not reproduce and was not in the live2d suite). Web tsc clean. `build` → `compile:server` → `pack` → `smoke:packaged` = `ok:true`; `Luna.app` rotated onto the Desktop.

Inference:

- This is the exact failure mode the initiative README warned about and then walked into. Writing "the SDK's magnitudes must be re-tuned against her rig" and then copying them verbatim is a documentation-vs-code gap, and the reason it survived four versions of tests is that **every assertion was about direction, ordering, bounds and persistence — none about amplitude**. A test suite can be entirely green and still not be testing the thing the feature exists to do.
- The new perceptibility test is the cheap guard: it asserts a *floor* on displacement, which is the property a human actually checks. It would have failed on the original values.
- Keeping the fix to one multiplier, rather than re-deriving the coefficients, preserves everything the research bought — the relative weighting between mouth/eyes/brows, the multiplicative interactions, the ceiling discipline. If 2.6 turns out to read as too much in daily use, it is one number to turn, and `reactivity` remains available as a per-user version of the same knob.

### `v0.42.3` — 2026-07-31 — Continuous affect ships ON (Initiative 29, 4/4 — closes it)

Status:

- working tree — default ON, packaged and smoked

Fact:

- `pixiLive2DSink.ts`: `affectEnabled()` inverts to `localStorage['luna:affect'] !== '0'` (default ON, `'0'` opts out); the `?dev` bridge gains `affect` and `mood()` alongside `model`/`faceVm`/`param`.
- `affect.ts`: new pure `describeAffect(vad)` → `"warm · calm · yielding (v+0.55 a−0.15 d−0.40)"`. Bands: valence `cold|reserved|neutral|warm|radiant`, arousal `drowsy|calm|present|alert|electric`, dominance `yielding|even|assertive`; the numeric tail always carries a sign and two decimals (a real minus sign, not a hyphen).
- `ui/layout.ts` + `app.ts`: a **Mood memory** toggle in the Avatar settings card, persisted to `luna:affect`, following the idle-profile precedent (client-side, not the server registry). No sink method needed — the flag is read per tick, so the change lands on the next frame.
- `ARCHITECTURE.md`: the `live2d/` section rewritten to describe the layered per-frame engine, the ownership rule, and the continuous mood half.
- +4 `describeAffect` tests. Suite 1533 green, web tsc clean. `bun run build` → `compile:server` → `pack` → `smoke:packaged` printed `ok:true`; `Luna.app` rotated onto the Desktop with the previous build kept as `Luna-prev.app`.
- **Not built, deliberately**: the planned 0–2 expressiveness slider. See Inference.

Inference:

- Flipping the default was the risky act, and the thing that made it safe was already in place: three exact-stream regressions prove the `'0'` path is byte-identical to a FaceVm with no affect at all, so the escape hatch is not a hope, it is an assertion. Shipping the toggle in the same version means the owner can reach that hatch without a console.
- Dropping the slider was a scope call, not an oversight. The panel is server-registry-driven and has no client-side slider primitive; building one would have cost a new layout helper, CSS and tests to expose a tuning knob for behaviour nobody has lived with for even a day. The right sequence is: live with the designed default, then tune what actually annoys. `reactivity` is already a constructor option, so that later version is a wiring job, not a redesign.
- The `describeAffect` readout exists because this initiative's whole subject is a state you cannot see directly. A layer that only manifests as a few hundredths on thirty-three parameters is exactly the kind of thing that silently breaks; a one-line human-readable projection makes "is she actually in a mood right now" answerable from the console in a second.

### `v0.42.2` — 2026-07-30 — The living baseline (Initiative 29, 3/4)

Status:

- working tree — behind `luna:affect`, still default OFF

Fact:

- `affectPose.ts`: new `restingState(vad)` — `FACE_VM_DEFAULT_STATE` + `affectPose(vad) × 0.6`, each channel through `clampStateValue`. Returns the old default exactly at a zero mood.
- `faceVm.ts`: `tick()` now ticks the affect FIRST (so `rest` and the undertone read the same frame), derives `rest = mood ? restingState(mood) : FACE_VM_DEFAULT_STATE`, seeds `target` from it, and — critically — uses `def = rest[key]` in the smoother/write-gate instead of the constant default. `sm` becomes `clamp(base × (1 + 0.55·arousal), base×0.6, base×1.6)` when a mood is active.
- `affect.ts`: ambient drift — `AMBIENT_MAGNITUDE {0.05, 0.07, 0.03}`, re-picked every `1700 + rng()×4200` ms, added to the chased value only (never to `target`). New options `ambient` (scale, default 1; 0 = still) and `rng` (default `Math.random`), matching the FaceVm convention. `clear()` resets the wander too.
- Tests: +5 ambient-drift cases (bounded on every axis, does not walk the mood away over 5 simulated minutes, genuinely moves her, `ambient: 0` is exactly still, determinism under a seeded rng); +4 `restingState` cases (identity at zero mood, shift equals exactly `offset × 0.6`, per-key clamps hold at all 8 VAD corners, warm rests warmer than sour); +3 FaceVm cases (**mood outlives the clip by 25 s**, sour vs bright rest differently, smoothing follows arousal — probed on `mouthPucker`, the one strongly-driven channel `affectPose` deliberately never touches, so only the rate differs). Suite 1529 green, web tsc clean.
- Two v0.42.1 tests were invalidated *correctly* and rewritten: `runWithAffect` now builds the state with `ambient: 0` (the regression's subject is the layer being silent, not drift being absent), and "a clip that owns a channel is not muddied" became "converges to the same value" — arousal-dependent smoothing changes every channel's path by construction, including owned ones.

Inference:

- The `def` substitution is the part that actually delivers the initiative. Adding an offset to the target only tints a face that is still being pulled back to a fixed neutral; moving the neutral is what makes "at rest" mean "at rest *in this mood*". It is also the riskiest single line in the initiative, because that constant gates whether a parameter is written at all — which is exactly why the flag-off path keeps `rest === FACE_VM_DEFAULT_STATE` by identity rather than by arithmetic.
- Ambient drift needed the "offset, never integrated" discipline to be safe. A wander added into `target` would random-walk her mood over an evening; added only to what `current` chases, it is bounded by construction — a property worth a test precisely because the bug it prevents would take hours of real use to notice.
- Rewriting the two older tests rather than loosening them kept the regression's meaning intact. A test that has to change when behaviour legitimately changes is doing its job; the failure told us exactly which guarantee had moved (path vs endpoint) and the replacement asserts the one that still holds.

### `v0.42.1` — 2026-07-30 — The undertone layer (Initiative 29, 2/4)

Status:

- working tree — behind `luna:affect`, default OFF

Fact:

- NEW `packages/web/src/live2d/affectPose.ts`: pure `affectPose(vad): Pose`. Rectified components (positive/negative, aroused/calm, dominant/submissive) drive mouth form, eye smile/squint/aperture, brow height/form/angle, head pitch/roll, gaze Y and body lift; two multiplicative interaction terms (`positive*submissive` → cheek/blush, `negative*dominant` → brow angle) are the only cross-axis effects. Ceiling 0.46 for normalised channels, ×8 for the six angle channels.
- `faceVm.ts`: `FaceVmOptions` gains `affect?: AffectState` + `affectEnabled?: () => boolean`; a new `addPose()` (accumulating twin of `applyPose`) applies the projection between `STATE_BIAS` and `applyEmotion`, skipping owned keys; `setExpression` additionally calls `affect.nudge(key, intensity)`. The state is ticked even when the layer is disabled, so flipping the flag mid-session cannot snap the face.
- `pixiLive2DSink.ts`: constructs one `AffectState`, passes it in, gates on `localStorage['luna:affect'] === '1'` read per tick.
- `affect.ts`: `DEFAULT_BASELINE` changed from `{0.08, −0.05, 0}` to zero (see Inference).
- +11 `affectPose.test.ts` (empty pose at dead neutral, sign correctness on all six components, multiplicative interactions genuinely needing both factors, the ceiling at all 8 corners of the VAD cube, angle-vs-normalised units, purity) and +7 `faceVm.test.ts` (full-stream equality flag-off; full-stream equality flag-on-at-rest; stream changes when nudged; **mood still present 30 s after the clip ended**; lip-sync still owns the mouth; an owning clip is not muddied). Suite 1517 green, web tsc clean.

Inference:

- Two plan corrections came from reading the rig, and both mattered. The roadmap said to apply the layer with the existing `applyPose`, but that helper **assigns** — it would have replaced the idle's `headRoll: sway*40` with a 0.02 offset and killed the sway that makes her look alive. An undertone has to accumulate, so the layer needed its own applicator. Likewise the plan's coefficients were written as if every channel were normalised; on the six degree-valued channels they would have been invisible. Neither error was findable from the plan alone.
- Moving the baseline to zero was a deliberate trade of expressiveness for provability. With a warm baseline the "flag on, at rest, byte-identical" test cannot pass by construction, and that test is the entire safety argument for a layer that touches every frame of her face. The living baseline is a better idea *with its own before/after*, which is exactly what v0.42.2 is for.
- Ownership arbitration turning out to be free is the payoff of routing through `owned`: the clip suppresses the undertone on precisely the channels it is performing with, which is why a direction assertion during a clip fails and one 30 s later succeeds. That failure was informative, not a bug — the mood is meant to be what remains when the performance is over.

### `v0.42.0` — 2026-07-30 — The affect core (Initiative 29, 1/4)

Status:

- working tree — pure module, no callers, no visual change

Fact:

- NEW `packages/web/src/live2d/affect.ts` (~165 lines): `Vad` (valence/arousal/dominance, each −1..1), `AffectOptions`, `AffectState` (`nudge(key, intensity)` / `tick(now)` / `current` / `baseline` / `setBaseline` / `clear`), `DEFAULT_BASELINE = {0.08, −0.05, 0}`, and `AFFECT_VAD` — all 15 `ExpressionKey`s as distinct VAD coordinates.
- Evolution per tick: `dt` clamped to 0.25 s; `current = lerp(current, target, 1 − exp(−dt·1.35))`; when the hold has expired, `target = lerp(target, baseline, 1 − exp(−dt·0.018))`. An impulse does `target = lerp(target, AFFECT_VAD[key], clamp((0.28 + i·0.58)·reactivity, 0, 0.96))` and sets `hold = max(hold, 6 + i·9)` seconds.
- NEW `affect.test.ts` — 18 tests: rest is *exactly* baseline over 60 s; attack reaches ≥90% of target in 2 s; 20 s after a peak she retains >40% of the excursion; 5 min later she is back near baseline; hold suppresses then releases decay; intensity scales the impulse; `reactivity: 0` is affectively flat; **the same affect from two different prior states lands >0.1 apart** (the owner's "same affect ≠ same face"); 50 max nudges never leave −1..1; determinism; a 10-minute frame gap advances no more than the 0.25 s clamp; `clear()` restarts the clock; `setBaseline` moves the resting point.
- Verified `affect.ts` has zero pixi/live2d/cubism imports (grep count 0) and adds no dependency.
- Deviation from the plan, recorded: the plan's test 2 asked for "within 10% of target after ~1 s", which the chosen rate makes arithmetically impossible (τ≈0.74 s → 74% at 1 s). The test asserts the same *intent* against correct maths: ≥90% at 2 s (1 − exp(−2.7) = 0.933).

Inference:

- The research reframed the problem. The owner said "预设表情映射…僵硬", which sounds like a renderer complaint, but `FaceVm` is already a 33-channel per-frame parameter engine with smoothing and ownership arbitration — the *output* half of a continuous system was built long ago (v0.13.2). What was missing is that emotion had no **state**: a clip played and left nothing behind, so message N+1 always started from neutral no matter what message N was. Continuity is therefore an insertion, not a rewrite.
- Choosing an impulse model over an assignment model is what buys "same affect ≠ same face" for free: because the field remembers, an identical `bright_delight` lands differently depending on whether she was already warm or still guarded — no randomness required, which keeps the module deterministic and testable.
- Landing the equations first, with no renderer attached, is deliberate: constants are the only real risk here, and they are now pinned by arithmetic assertions that run in 125 ms rather than by looking at a face and guessing.

### `v0.41.0` — 2026-07-30 — The distribution layer is retired; the repo becomes readable reference engineering

Status:

- working tree — docs + build-config only, zero product-code changes; suite 1482 green

Fact:

- DELETED `.github/workflows/release.yml` (77 lines) — the entire tag→installer→GitHub-Release pipeline (windows-latest NSIS job + macos-latest dmg job + `softprops/action-gh-release` uploads). Verified beforehand that nothing imports or chains to it, and that no `electron-updater`/`autoUpdater`/`publish` channel exists to unwind.
- `packages/desktop/package.json`: removed the `pack:win`, `pack:mac`, and `compile:server:win` scripts and the electron-builder `win`, `nsis`, and `dmg` blocks. **`pack` (`electron-builder --dir`) and the `mac` block stay** — that is the owner's own build path (`bun run app` → `pack` → `Luna.app` → Desktop), and it is byte-for-byte unaffected.
- `scripts/app.ts`: removed the `--win` cross-build branch (the `WIN` flag, the compile+pack+installer-path block). `readdirSync` stays imported — still used by `newestMtime`/`findApp`.
- `.github/workflows/ci.yml`: removed the "Cross-compile the Windows sidecar" step (it existed only to keep the release target valid). The `[ubuntu-latest, windows-latest]` test matrix and the portable-web-build step REMAIN.
- `README.md` + `README.zh-CN.md`: removed the two download buttons and the unsigned/SmartScreen/Gatekeeper caption; removed the PRs-welcome badge; promoted the Luna TV badge to the sole hero CTA ("see her in motion" / 看她动起来) and added a plain statement that the repo is the engineering published as a reference and that Luna is one person's companion with no installers. `Quick start` → `Run your own` (build from source is the only path, and the honest way to read her). `Contributing` → `Using this code`: personal project, **no promise of issues/PRs/platform support**, MIT, take what's useful. Added a reading path (`ARCHITECTURE.md` → `DEVELOPMENT.md`) for the reference audience.
- `CONTRIBUTING.md`: the "Reporting things" section (bugs/questions welcome, open an issue before a large PR) becomes "What this repo is (and isn't)" — no support promise, and the conventions reframed as load-bearing-for-this-codebase rather than gatekeeping.
- Verification: a 5-agent audit inventoried every distribution surface in the repo (23 remove / 15 reword / 57 keep) before any edit, and confirmed post-hoc that all 23 removals landed and that no test referenced any deleted script or builder block.
- Local (git-invisible) bookkeeping: Initiative 28's plan folder is marked ❄️ FROZEN with the reasoning and the permanently-abandoned list (v0.38.3 pet mode, all real-machine acceptance checklists, Linux, signing, Wayland); `docs/roadmap/README.md` records the posture change as a planning constraint — no future roadmap item may propose distribution, packaging, ports, or acceptance on hardware the owner has to go find.

Inference:

- This closes a cost that was compounding invisibly. Every version since v0.38.0 carried a distribution tax (a second OS to package for, a real-machine checklist that never got run, an unverified installer sitting behind a public download button). Retiring the layer converts that from an open obligation into a finished chapter — and does it *while the repo is in a good state*, which is the difference between an intentional freeze and quiet rot.
- The remove/keep line is the whole design of this version: **distribution surface out, platform-awareness in**. A reference repo that handles `taskkill` vs `SIGTERM` correctly is *more* credible, not less; a reference repo that ships an unverified installer is less. Same code, opposite signals — the fix was never "remove Windows", it was "stop promising Windows".
- An unverified installer behind a live download button was a liability masquerading as an asset. The evidence that the pipeline worked (cross-compiled PE32+, dual-runner CI green, a 108 MB NSIS artifact) is fully preserved in the v0.38.x entries above — prose keeps the proof, the binary carried the risk.
- Where the project goes next follows from this: Luna is a private companion shown by recording/streaming, so the work that matters is depth (memory quality, personality, presence, expressiveness), not reach. The planning constraint recorded in the roadmap is there to keep that honest.

### `v0.40.0` — 2026-07-24 — Windows voice runtime: the 8.19 GB 整合包 → the scripted recipe

Status:

- working tree — code complete + unit-proven + the installer packs from macOS; **真机验收 (install→speak) PENDING Alan's Windows PC**, per the repo's standing "the reference machine is the worst witness" rule.

Fact:

- `ttsProvision.ts` `buildManifest`: deleted the win32 early-return (a single 8.19 GB `.7z`). Windows now falls through to the shared scripted manifest plus `windowsExtras()` — a python-build-standalone CPython 3.11 install_only tarball (`WIN_PY_DIR`, sha256-pinned from the release's own SHA256SUMS) and BtbN's `ffmpeg-n7.1-latest-win64-lgpl-shared` zip (`WIN_FFMPEG_DIR`, unhashed because BtbN rebuilds `latest` daily — integrity is structural, a non-zip body fails extract).
- `ttsProvision.ts` venv stage: the `platform !== 'win32'` guard is gone — Windows builds the venv too, from the provisioned CPython (`recipePython`), with the interpreter path branched via new pure `venvPythonPath` (`.venv\Scripts\python.exe` vs `.venv/bin/python`). New `JIEBA_SHIM_INIT`/`JIEBA_SHIM_POSSEG` (a two-file package re-exporting `jieba` with PEP 562 fall-through) written into `winSitePackages` after the pip install; the pyopenjtalk warm-up is win32-skipped.
- `INFERENCE_REQUIREMENTS`: `jieba_fast` and `pyopenjtalk>=0.4` gain `; sys_platform != 'win32'` markers (precedent: `python-mecab-ko`). Plain `jieba` stays unconditional — it is what the shim re-exports.
- `ttsProvision.ts` validate: no platform exemptions — the venv interpreter and cmudict are checked everywhere; win32 additionally checks the ffmpeg exe and the shim. Closes the silent-crash-loop path (a partial install used to write `ready`, then ttsRuntime spawned a bare `python` and `gave-up` with no error).
- `sha256` seam: streaming (openSync/readSync loop) instead of `readFileSync` — the 2 GiB throw is why the 8.19 GB artifact's checksum "failed" and the `.part` was deleted every time. `copy` uses `copyFileSync` instead of `writeFileSync(readFileSync())`.
- `provision.json` gains a `recipe` field. New pure `markerIsCurrent(marker, platform)`: win32 demands `recipe === RECIPE` (its tree changed shape); mac/linux accept any recipe back to `POSIX_TREE_UNCHANGED_SINCE` (the tree is byte-identical). All FOUR readers route through it — `runProvision`'s short-circuit, `ttsRuntime.provisionReady` (platform threaded through `resolveManagedCheckout`), and `main.ts` `hydrateProvisionStatus` (the one the adversarial review caught still on raw `state === 'ready'`, which would have shown upgraders "Runtime ready ✓" with the deploy button greyed while the voice silently failed).
- Retired with the 整合包: `ArchiveKind`'s `'7z'`, `sevenZipCandidates`, the 7z arm of `extract`, `MIN_FREE_BYTES_WIN`, `LUNA_7ZR_DIR`, `bin/7zr.exe`, `scripts/fetch-7zr.ts`, and `pack:win`'s `fetch:7zr` prestep. `win.extraResources` now ships `luna-server.exe` (was `7zr.exe`).
- Copy/docs: the wizard hint (~5.7 GB, which was never right — the archive is 8.19 GB) → ~1.7 GB on every platform; `docs/SETUP.md` updated. +17 desktop tests (win manifest shape, venv-from-provisioned-python, the shim-as-package, pyopenjtalk skip, ffmpeg shared-not-static, streaming hash implied, recipe migration across platforms). Suite 1468 → 1482.

Inference:

- The headline is a bug fix wearing a feature's clothes: **the Windows one-click button never worked**. The `readFileSync` 2 GiB ceiling meant every Windows user downloaded 8 GB and watched it get deleted under a "checksum" error. Unifying on the scripted recipe removes the single largest, least reliable artifact in the project (8.19 GB over one HTTP stream with no per-segment recovery) and, with it, the 7-Zip dependency and a whole class of "the 整合包 layout is genuinely unverified" caveats the changelog had been carrying since v0.38.4.
- The three substitutions each trade a real cost for a smaller one. jieba_fast's C acceleration is wired into `Tokenizer.__cut_DAG` only — NOT the `posseg` hot path GPT-SoVITS actually uses — so the pure-Python shim costs ~0.001 ms/sentence while deleting a CI wheel pipeline, an unproven MSVC compile, and a per-CPython-minor ABI problem. ffmpeg had to be a SHARED build because torchcodec dlopens the DLLs (the repo's own comments claimed it "shells out", which is false); pointing at BtbN's upstream URL rather than mirroring keeps Luna a downloader, not an FFmpeg redistributor, so LGPLv3's relinking obligations never attach.
- The platform-aware recipe gate is the subtle part. A blanket version bump would have force-re-provisioned every existing macOS/Linux install (~5 GB, minutes, a silent drop to the browser voice) even though only Windows's on-disk tree changed. Scoping invalidation to the platform that actually changed — and routing all four marker readers through one pure function — is what makes the migration safe in the direction we CAN'T test (the installed mac base) while still forcing the re-layout in the direction we can't test either (Windows), without the two disagreeing.
